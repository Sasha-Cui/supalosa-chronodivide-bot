...
The Chrono Divide resource set does not contain one additional file whose
embedded name is simply `Heck Freezes Over`. It contains the following official
variants. Consequently, informal labels such as “Gold Corners” must be mapped
to exact filenames and hashes in every result; no result may use an unqualified
`HFO` label when it is not the LE file used by the existing confirmation.
...
The first five rows are the primary HFO revision suite. The four fixed-pair
rows are secondary geometry controls. They do not create nine independent-map
replications: all nine are reported as one related HFO family.
...
| South Pacific | `[2] South Pacific` | 2 | `cd_2_south_pacific.map` | `5d8122dc9234cc6fbcb68ec428cbea28b31496617253274ee15296a7c2807e2e` |
...
substituted for the requested map. South Pacific and Pacific Heights may expose
land/naval capability gaps; they remain in scope even if a technical or
competitive gate fails.

## Interpretation contract

1. The existing 720-game result applies only to the exact HFO LE bytes in the
   first row. It cannot be copied to any other revision.
...
2026-08-31. Runtime identity remains governed by the private RA2 asset manifest;
copyrighted runtime bytes are not added to the review artifact.
