...
| Baselines and ablations | Weak/mixed | The comparator is a prospectively frozen coordinate-free research reference, not the fork's map-profile-enabled deployed default. Five reverts probe components, but there is no deployed-product baseline, SMAC/irace/NTBEA, or alternative training pipeline. Richoux and Castejón et al. have more algorithm-level comparators. |
