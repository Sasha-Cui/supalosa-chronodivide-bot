# HFO Soviet-west rush-and-guard activation-isolation gate V6
...
on 120 fresh cases. V5 established exact paired equality in all 32 inactive
cells but failed because it also required the winner's absolute guard-anchor
count to be zero. That rule was incompatible with the already deployed Allied
west guard, which appeared identically in both arms.

Before deployment, V6 must prove that the two Soviet mechanisms—conditional
rush production/planning and the west grouped home guard—activate only for
Soviet west-versus-east games. It writes no W/D/L or terminal-building outcome.
...
`4,259,000,000 + 10,000*c + 1,000*s + offset`, offsets 0 through 399, and both
...
resignation counts. Also require equality of winner and default guard-anchor
counts. This permits already deployed behavior only when it is bit-for-bit
unchanged by the Soviet opt-in. The aggregate
