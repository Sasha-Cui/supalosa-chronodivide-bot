...
`semantic space`, `evaluation`, and publication years 2023--2026. Candidate
...
The search justified two citation additions and no scientific-claim change.
The updated paper explicitly classifies opponent-set guidance and semantic-
space neighborhoods as prior synthesis work, then retains the narrower claim:
family-disjoint, provenance-bound evaluation for one scripted Chrono Divide
agent and one pinned opponent. No returned exact-phrase result triggered a
source-attribution repair.

Before submission, the author must still read all 32 primary sources, complete
