...
export const STUDY = path.join(PROJECT, "research-evidence/unified-intent-arbiter-v2/d1");
...
export const schedulerIdentity = () => {
    ...
    if (!/^\d+$/.test(value.jobId) || value.account !== "pi_jss233" || value.partition !== "day" ||
        ...
        throw new Error("D1 scheduler contract failed");
    }
    return value;
};
...
export const parseSchedulerRows = (raw, jobId, taskCount) => {
    const rows = raw.trim().split("\n").filter(Boolean).map((line) => {
        ...
        if (v.length !== 9) throw new Error("D1 scheduler column count drifted");
    ...
    if (!Number.isSafeInteger(n) || n < 1 || selected.length !== n ||
        ...
        new Set(selected.map((r) => r.label)).size !== n) throw new Error("D1 scheduler exact launch count failed");
    for (let i = 0; i < n; i++) {
        ...
        if (!row || !/^\d+$/.test(row.jobId) || row.state !== "COMPLETED" || row.exitCode !== "0:0" ||
            ...
            throw new Error("D1 scheduler completion contract failed");
        }
    }
...
export const reserveDirectory = (directory) => {
    if (!directory.startsWith(EXECUTION + "/")) throw new Error("D1 output outside execution");
...
export const recordLaunch = (directory, value) => {
    ...
    if (fs.existsSync(marker) && fs.readFileSync(marker, "utf8").includes("COMPLETE_D1_V1 ")) {
        throw new Error("D1 launch after completion");
    }
    fs.appendFileSync(marker, "LAUNCH_D1_V1 " + JSON.stringify(value) + "\n", { mode: 0o600 });
};
export const parseLaunchMarker = (text, expectedCompletion = null) => {
    const lines = text.split("\n");
    if (lines.pop() !== "") throw new Error("D1 marker missing newline");
    if (expectedCompletion !== null && lines.pop() !== expectedCompletion) throw new Error("D1 completion marker invalid");
    return lines.map((line) => {
        if (!line.startsWith("LAUNCH_D1_V1 ")) throw new Error("D1 launch marker invalid");
        return JSON.parse(line.slice("LAUNCH_D1_V1 ".length));
    });
};
export const publish = (directory, value, limit = MAX_PAIR_BYTES) => {
    ...
    if (files.length && JSON.stringify(files) !== JSON.stringify(["COMPLETE"])) {
        throw new Error("D1 publication directory invalid");
    }
    const marker = path.join(directory, "COMPLETE");
    const launches = fs.existsSync(marker) ? parseLaunchMarker(fs.readFileSync(marker, "utf8")) : [];
    exact(launches, value.launches, "D1 preserved launch journal");
    const data = JSON.stringify(value) + "\n";
    if (Buffer.byteLength(data) > limit) throw new Error("D1 complete artifact exceeds byte bound");
    writeExclusive(path.join(directory, "record.json"), data);
    fs.appendFileSync(marker, "COMPLETE_D1_V1 " + hash(data) + " " + Buffer.byteLength(data) + "\n", { mode: 0o600 });
...
export const readPublished = (directory, expectedHash = null, limit = MAX_PAIR_BYTES) => {
    exact(fs.readdirSync(directory).sort(), ["COMPLETE", "record.json"], "D1 artifact files");
    const file = path.join(directory, "record.json"), stat = fs.lstatSync(file);
    if (!stat.isFile() || stat.isSymbolicLink() || stat.size > limit) throw new Error("D1 artifact file type/size invalid");
    ...
    const launches = parseLaunchMarker(fs.readFileSync(path.join(directory, "COMPLETE"), "utf8"),
        "COMPLETE_D1_V1 " + sha256 + " " + data.length);
    if (expectedHash !== null && expectedHash !== sha256) throw new Error("D1 artifact checksum invalid");
    const value = JSON.parse(data);
    exact(launches, value.launches, "D1 completed launch journal");
...
export const technicalOnly = (value) => {
    ...
    for (const [key, child] of Object.entries(value)) {
        if (/winner|outcome|score|endpoint|defeated|terminal|inventory|credit|damage|rank|buildingcount|remainingbuilding|actioncontents|statecontents/i.test(key)) {
            throw new Error("D1 prohibited technical field");
        }
        technicalOnly(child);
    }
};
...
export const buildD1RegistrationAudit = ({
    certificate, registrationRoot, registrationAfterUtc, records, supportingMetadata, proposedSeeds, abandonedSelector, od1A1,
}) => {
    const expected = [...Array.from({ length: 200 }, (_, i) => 3350110000 + i),
        3350111000, 3350111001, 3350111002, 3350111003, 3350111100];
    exact(proposedSeeds, expected, "D1 prospective seed population");
    if (proposedSeeds.length !== 205 || new Set(proposedSeeds).size !== 205 ||
        ...
        abandonedSelector.advancingEpisodes !== 0 || od1A1.zeroUpdateInitializations !== 905 ||
        od1A1.advancingEpisodes !== 1818) throw new Error("D1 A1 registration audit contract failed");
    const value = {
        ...
        abandonedSelector, od1A1,
        newSeeds: { count: 205, min: Math.min(...proposedSeeds), max: Math.max(...proposedSeeds),
...
export const buildD1PreparationEnvelope = (header) => {
    const value = { ...header, gameModes: {}, observations: [] };
    technicalOnly(value);
    return value;
};

/** Exact path allowlist: unknown registrations fail closed even inside prior/current study roots. */
export const validateD1RegistrationScan = (discovered, permitted, mandatory) => {
    const allowed = new Set(permitted), found = new Set(discovered);
    if (found.size !== discovered.length || discovered.some(p => !allowed.has(p)) ||
        mandatory.some(p => !found.has(p))) throw new Error("D1 unreviewed or missing registration metadata");
};
export const registeredD1Seeds = (metadata, proposed) => {
    if (!metadata?.complete || !metadata.passed || !Array.isArray(metadata.plan?.cases) ||
        !metadata.plan.cases.length) throw new Error("D1 registration schema unrecognized");
    const plan = metadata.plan;
    if (plan.canaries !== undefined && !Array.isArray(plan.canaries)) throw new Error("D1 canary registration schema invalid");
    const definitions = [...plan.cases, ...(plan.canaries ?? []), ...(plan.smoke ? [plan.smoke] : [])];
    const seeds = definitions.map(c => c.requestedEngineSeed);
    const requested = new Set(proposed);
    if (seeds.some(s => !Number.isSafeInteger(s) || s < 3350000000 || s >= 3351000000 || requested.has(s)) ||
        (plan.tasks !== undefined && (!Array.isArray(plan.tasks) ||
            plan.tasks.some(t => !seeds.includes(t.requestedEngineSeed))))) {
        throw new Error("D1 registered seed collision or schema drift");
    }
    return seeds;
};
