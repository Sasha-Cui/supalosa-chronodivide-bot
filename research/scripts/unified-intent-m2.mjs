...
const STUDY = path.join(PROJECT, "research-evidence", "unified-intent-arbiter-v1", "m2-open-development");
...
const COMPLETE_RE = /^COMPLETE_UNIFIED_INTENT_M2_PAIR_V1 ([0-9a-f]{64}) ([0-9]+)$/;
const SMOKE_COMPLETE_RE = /^COMPLETE_UNIFIED_INTENT_M2_SMOKE_V1 ([0-9a-f]{64}) ([0-9]+)$/;
...
const {
    ...
    DRIVER, "dist", "training", "unifiedIntentM2Plan.js",
)).href);
...
const { runUnifiedIntentM2Episode } = await import(pathToFileURL(path.join(
    DRIVER, "dist", "training", "unifiedIntentM2Episode.js",
)).href);
const { validateUnifiedIntentM2Telemetry } = await import(pathToFileURL(path.join(
    DRIVER, "dist", "training", "unifiedIntentM2Telemetry.js",
)).href);
const {
    ...
    DRIVER, "dist", "training", "unifiedIntentM2Analysis.js",
)).href);
const {
    LITERAL_BUILDING_ELIMINATION_ENDPOINT_SHA256,
    LITERAL_BUILDING_ELIMINATION_ENDPOINT_VERSION,
} = await import(pathToFileURL(path.join(
    DRIVER, "dist", "training", "literalBuildingEliminationEndpoint.js",
)).href);
...
const sourceFiles = {
    m2Protocol: path.join(
        REPO, "research/protocols/method/2026-09-12-unified-intent-arbiter-v1-m2-open-development.md",
    ),
    ...
    gate3Failure: path.join(
        REPO, "research/results/2026-09-10-unified-intent-arbiter-v1-gate-3-failed.md",
    ),
    a1Result: path.join(
        REPO, "research/results/2026-09-11-unified-intent-arbiter-v1-gate-3-diagnostic-a1.md",
    ),
    b1Result: path.join(
        REPO, "research/results/2026-09-12-unified-intent-arbiter-v1-gate-3-b1.md",
    ),
};
...
const programFiles = () => {
    const values = {
        program: PROGRAM,
        plan: path.join(DRIVER, "dist", "training", "unifiedIntentM2Plan.js"),
        episode: path.join(DRIVER, "dist", "training", "unifiedIntentM2Episode.js"),
        telemetry: path.join(DRIVER, "dist", "training", "unifiedIntentM2Telemetry.js"),
        analysis: path.join(DRIVER, "dist", "training", "unifiedIntentM2Analysis.js"),
        ...
        manifestSlurm: path.join(REPO, "research", "slurm", "unified_intent_m2_manifest.sbatch"),
        traceSlurm: path.join(REPO, "research", "slurm", "unified_intent_m2_pair.sbatch"),
        finalizerSlurm: path.join(REPO, "research", "slurm", "unified_intent_m2_finalizer.sbatch"),
    };
...
const verifyPureGate = () => {
    ...
    if (
        ...
        value.tests?.vitest?.files !== 14 ||
        value.tests?.vitest?.tests !== 146 ||
...
const prepare = async () => {
    ...
    const plan = buildUnifiedIntentM2Plan(runtime.maps);
    ...
    const manifest = {
        kind: "unified-intent-m2-manifest-v1",
        complete: true,
        passed: true,
    ...
    writeExclusive(
        ...
        completeLine("COMPLETE_UNIFIED_INTENT_M2_MANIFEST_V1", file),
    );
...
const loadManifest = () => {
    ...
    rejectProhibited(value);
    if (
        value.kind !== "unified-intent-m2-manifest-v1" ||
        !value.complete ||
        !value.passed ||
        !value.developmentOnly ||
        !value.technicalOnly ||
        !value.competitiveFieldsAbsent ||
        value.sourceCommit !== required("SOURCE_COMMIT") ||
        value.programSha256 !== requiredHash("PROGRAM_SHA256") ||
        value.plan.cases.length !== 900 ||
        value.plan.counts.tasks !== 900 ||
        value.plan.counts.games !== 1800 ||
    ...
    validateUnifiedIntentM2Plan(value.plan);
...
const validateM2EpisodeTechnical = (value, arm) => {
    if (
        !value || value.complete !== true || value.technicalPass !== true ||
        ...
        !Number.isSafeInteger(value.updates) || value.updates < 1 || value.updates > 24000 ||
        value.maxUpdates !== 24000 || !SHA256.test(value.action.sha256) ||
        !SHA256.test(value.trajectory.sha256) || value.trajectory.snapshots < 2 ||
        value.quitSuppression.forwarded.candidate !== 0 ||
        value.quitSuppression.forwarded.baseline !== 0
    ) throw new Error("Unified intent M2 episode technical gate failed: " + arm.id);
    if (arm.enabled) {
        validateUnifiedIntentM2Telemetry(value.telemetry);
        if (value.telemetry.updates !== value.updates) {
            throw new Error("Unified intent M2 telemetry duration drifted");
        }
    } else if (value.telemetry !== null) {
        throw new Error("Disabled unified intent M2 episode emitted telemetry");
    }
};

const validateM2Episode = (value, arm) => {
    validateM2EpisodeTechnical(value, arm);
    const expected = {
        candidate_win: ["candidate", 1],
        baseline_win: ["baseline", 0],
        simultaneous_draw: ["draw", 0.5],
        engine_nonliteral_termination_draw: ["draw", 0.5],
        tick_cap_draw: ["draw", 0.5],
    }[value.status];
    if (
        !expected || value.winner !== expected[0] || value.candidateScore !== expected[1] ||
        !Number.isSafeInteger(value.terminalBuildingCounts.candidate) ||
        !Number.isSafeInteger(value.terminalBuildingCounts.baseline) ||
        value.terminalBuildingCounts.candidate < 0 ||
        value.terminalBuildingCounts.baseline < 0 ||
        (value.status === "candidate_win" && value.terminalBuildingCounts.baseline !== 0) ||
        (value.status === "baseline_win" && value.terminalBuildingCounts.candidate !== 0) ||
        (value.status === "simultaneous_draw" &&
            (value.terminalBuildingCounts.candidate !== 0 ||
                value.terminalBuildingCounts.baseline !== 0)) ||
        (value.status === "tick_cap_draw" && value.updates !== 24000)
    ) throw new Error("Unified intent M2 literal outcome gate failed");
};

const loadM2Execution = async () => {
    const runtime = verifyRuntime();
    const manifest = loadManifest();
    exact(runtime, manifest.runtime, "Unified intent M2 runtime drifted");
    await gameApi.cdapi.init(runtime.assetsRoot);
    const factory = await loadBaselineFactory(path.join(REPO, "packages", "chronodivide-bot"));
    if (factory.descriptor.kind !== "external-package") {
        throw new Error("Unified intent M2 baseline is not external");
    }
    const advancedDefinition = loadRa2WebOpponent(
        runtime.advancedFreezeRoot,
        runtime.advancedOpponentId,
    );
    return { runtime, manifest, factory, advancedDefinition };
};

const runM2Arm = async (task, arm, context) => {
    const map = context.runtime.maps.find((value) => value.id === task.mapId);
    if (!map) throw new Error("Unified intent M2 map is unavailable");
    const bots = await createBots(task, arm, context.factory, context.advancedDefinition);
    return runUnifiedIntentM2Episode({
        api: gameApi.cdapi,
        spec: {
            mapName: map.fileName,
...
const runM2Smoke = async () => {
    const source = sourceIdentity();
    verifyCertificate();
    verifyPureGate();
    const context = await loadM2Execution();
    const task = context.manifest.plan.cases[0];
    const directory = required("OUT_DIR");
    if (directory !== path.join(EXECUTION, "smoke") || fs.existsSync(directory)) {
        throw new Error("Unified intent M2 smoke output drifted");
    }
    fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
    const technical = [];
    for (const arm of task.arms) {
        const result = await runM2Arm(task, arm, context);
        validateM2EpisodeTechnical(result, arm);
        technical.push({
            armId: arm.id,
            complete: true,
            technicalPass: true,
            literalAdjudicatorSha256: result.endpointSha256,
            publicCallSha256: result.action.sha256,
            publicStateSha256: result.trajectory.sha256,
            telemetryValid: arm.enabled ? result.telemetry.complete : result.telemetry === null,
        });
    }
    const artifact = {
        kind: "unified-intent-m2-technical-smoke-v1",
        complete: true,
        passed: true,
        technicalOnly: true,
        competitiveFieldsAbsent: true,
        episodePayloadDiscarded: true,
        sourceCommit: source.sourceCommit,
        programSha256: source.programSha256,
        manifestSha256: requiredHash("MANIFEST_SHA256"),
        assignment: task,
    ...
    writeExclusive(
        path.join(directory, "COMPLETE"),
        completeLine("COMPLETE_UNIFIED_INTENT_M2_SMOKE_V1", file),
    );
};

const runM2Pair = async () => {
    const source = sourceIdentity();
    verifyCertificate();
    verifyPureGate();
    const context = await loadM2Execution();
    ...
    if (!Number.isSafeInteger(taskIndex) || taskIndex < 0 || taskIndex >= 900) {
        throw new Error("Unified intent M2 task index drifted");
    }
    const task = context.manifest.plan.cases[taskIndex];
    const directory = required("OUT_DIR");
    if (fs.existsSync(directory)) throw new Error("Unified intent M2 pair output exists");
    fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
    try {
        const arms = [];
        for (const arm of task.arms) {
            const result = await runM2Arm(task, arm, context);
            validateM2Episode(result, arm);
            arms.push({ arm, result });
        }
        const map = context.runtime.maps.find((value) => value.id === task.mapId);
        const artifact = {
            kind: "unified-intent-m2-pair-v1",
            complete: true,
            passed: true,
            developmentOnly: true,
            sourceCommit: source.sourceCommit,
            ...
            scheduler: {
                jobId: process.env.SLURM_JOB_ID,
                arrayJobId: process.env.SLURM_ARRAY_JOB_ID,
                arrayTaskId: process.env.SLURM_ARRAY_TASK_ID,
                account: process.env.SLURM_JOB_ACCOUNT,
                partition: process.env.SLURM_JOB_PARTITION,
            },
        };
        const file = path.join(directory, "pair.json");
        writeExclusive(file, JSON.stringify(artifact) + "\n");
        if (fs.statSync(file).size > TRACE_LIMIT) throw new Error("Unified intent M2 pair is too large");
        writeExclusive(
            path.join(directory, "COMPLETE"),
            completeLine("COMPLETE_UNIFIED_INTENT_M2_PAIR_V1", file),
        );
    } catch (error) {
        const failure = {
            kind: "unified-intent-m2-generic-failure-v1",
            complete: false,
            passed: false,
            technicalOnly: true,
            competitiveFieldsAbsent: true,
...
const parseM2Smoke = (manifestSha256) => {
    const directory = path.join(EXECUTION, "smoke");
    exact(fs.readdirSync(directory).sort(), ["COMPLETE", "smoke.json"], "M2 smoke files drifted");
    const file = path.join(directory, "smoke.json");
    const match = SMOKE_COMPLETE_RE.exec(fs.readFileSync(path.join(directory, "COMPLETE"), "utf8").trim());
    if (!match || match[1] !== sha256File(file) || Number(match[2]) !== fs.statSync(file).size) {
        throw new Error("M2 smoke checksum drifted");
    }
    const value = json(file);
    rejectProhibited(value);
    if (
        value.kind !== "unified-intent-m2-technical-smoke-v1" || !value.complete ||
        !value.passed || !value.technicalOnly || !value.competitiveFieldsAbsent ||
        !value.episodePayloadDiscarded || value.manifestSha256 !== manifestSha256 ||
        value.technical.length !== 2 || value.technical.some((row) => !row.technicalPass)
    ) throw new Error("M2 smoke artifact drifted");
    return value;
};

const parseM2Pair = (directory, task, manifestSha256) => {
    exact(fs.readdirSync(directory).sort(), ["COMPLETE", "pair.json"], "M2 pair files drifted");
    const file = path.join(directory, "pair.json");
    ...
    if (
        ...
        fs.statSync(file).size > TRACE_LIMIT
    ) throw new Error("M2 pair checksum drifted");
    const value = json(file);
    if (
        value.kind !== "unified-intent-m2-pair-v1" || !value.complete || !value.passed ||
        !value.developmentOnly || value.manifestSha256 !== manifestSha256 ||
        value.arms.length !== 2
    ) throw new Error("M2 pair artifact drifted");
    exact(value.assignment, task, "M2 assignment drifted");
    value.arms.forEach((row, index) => {
        exact(row.arm, task.arms[index], "M2 arm drifted");
        validateM2Episode(row.result, row.arm);
        exact(row.result.observedStarts, {
            candidate: task.candidateStart,
            opponent: task.opponentStart,
        }, "M2 observed start drifted");
    });
    return value;
};

const m2Faction = (country) => [
    "Americans", "Alliance", "French", "Germans", "British",
].includes(country) ? "Allied" : "Soviet";
const m2Mean = (values) => values.reduce((sum, value) => sum + value, 0) / values.length;
const m2Median = (values) => {
...
const m2Effect = (rows) => ({
    cases: rows.length,
    pairedScore: m2Mean(rows.map((row) => row.enabledScore - row.disabledScore)),
    pairedLiteralWin: m2Mean(rows.map((row) =>
        Number(row.enabledWinner === "candidate") - Number(row.disabledWinner === "candidate"))),
});
...
const finalizeM2 = () => {
    ...
    const runtime = verifyRuntime();
    const manifest = loadManifest();
    exact(runtime, manifest.runtime, "M2 finalizer runtime drifted");
    const manifestSha256 = requiredHash("MANIFEST_SHA256");
    const arrayJobId = required("ARRAY_JOB_ID");
    const scheduler = schedulerRows(arrayJobId, 900);
    const smoke = parseM2Smoke(manifestSha256);
    if (
    ...
    ) throw new Error("M2 smoke identity drifted");
    const pairs = manifest.plan.cases.map((task) => parseM2Pair(
        path.join(EXECUTION, "cells", "task-" + String(task.taskIndex).padStart(3, "0")),
        task,
        manifestSha256,
    ));
    const schedulerIds = new Set();
    for (const [taskIndex, value] of pairs.entries()) {
        ...
        if (
            ...
            typeof value.scheduler.jobId !== "string"
        ) throw new Error("M2 pair scheduler identity drifted");
        schedulerIds.add(value.scheduler.jobId);
    }
    if (schedulerIds.size !== 900) throw new Error("M2 pair scheduler IDs are not unique");

    const rows = pairs.map((pair) => {
        const task = pair.assignment;
        const disabled = pair.arms[0].result;
        const enabled = pair.arms[1].result;
        return {
            ...
            opponent: task.opponent,
            mapId: task.mapId,
            directionOrdinal: task.directionOrdinal,
            country: task.country,
            faction: m2Faction(task.country),
            candidateSlot: task.candidateSlot,
            requestedEngineSeed: task.requestedEngineSeed,
            disabledWinner: disabled.winner,
            disabledStatus: disabled.status,
            disabledScore: disabled.candidateScore,
            disabledUpdates: disabled.updates,
            disabledCandidateBuildings: disabled.terminalBuildingCounts.candidate,
            disabledOpponentBuildings: disabled.terminalBuildingCounts.baseline,
            enabledWinner: enabled.winner,
            enabledStatus: enabled.status,
            enabledScore: enabled.candidateScore,
            enabledUpdates: enabled.updates,
            enabledCandidateBuildings: enabled.terminalBuildingCounts.candidate,
            enabledOpponentBuildings: enabled.terminalBuildingCounts.baseline,
            pairedScoreDifference: enabled.candidateScore - disabled.candidateScore,
            pairedLiteralWinDifference:
                Number(enabled.winner === "candidate") - Number(disabled.winner === "candidate"),
            transition: disabled.winner + "_to_" + enabled.winner,
            disabledCallSha256: disabled.action.sha256,
            enabledCallSha256: enabled.action.sha256,
            disabledStateSha256: disabled.trajectory.sha256,
            enabledStateSha256: enabled.trajectory.sha256,
        };
    });
    const analysisRows = rows.map((row) => ({
        ...
        disabledWinner: row.disabledWinner,
        enabledWinner: row.enabledWinner,
        disabledScore: row.disabledScore,
        enabledScore: row.enabledScore,
    }));
    const subsets = {
        overall: analysisRows,
        supalosa: analysisRows.filter((row) => row.opponent === "pinned_supalosa"),
        advanced: analysisRows.filter((row) => row.opponent === "ra2web_advanced"),
    };
    const bootstrap = Object.fromEntries(Object.entries(subsets).map(([name, values]) => [
        name,
        unifiedIntentM2ClusterBootstrap(values, name),
    ]));
    const summaries = Object.fromEntries(Object.entries(subsets).map(([name, values]) => [name, {
        disabled: unifiedIntentM2Wdl(values.map((row) => row.disabledWinner)),
        enabled: unifiedIntentM2Wdl(values.map((row) => row.enabledWinner)),
        effect: m2Effect(values),
    }]));
    const familyRows = [...new Set(rows.map((row) => row.familyId))].sort().map((familyId) => {
        const values = rows.filter((row) => row.familyId === familyId);
        return {
            familyId,
            opponent: values[0].opponent,
            mapId: values[0].mapId,
            ...m2Effect(values),
            disabledWdl: unifiedIntentM2Wdl(values.map((row) => row.disabledWinner)),
            enabledWdl: unifiedIntentM2Wdl(values.map((row) => row.enabledWinner)),
        };
    });
    const subgroupRows = [];
    for (const [dimension, values] of Object.entries({
        ...
        slot: ["0", "1"],
        country: [...new Set(rows.map((row) => row.country))].sort(),
        direction: ["0", "1"],
    })) {
        for (const value of values) {
            const selected = rows.filter((row) => String(
                dimension === "slot" ? row.candidateSlot :
                    dimension === "direction" ? row.directionOrdinal : row[dimension],
            ) === value);
            subgroupRows.push({
                dimension,
                value,
                ...m2Effect(selected),
                disabledWdl: unifiedIntentM2Wdl(selected.map((row) => row.disabledWinner)),
                enabledWdl: unifiedIntentM2Wdl(selected.map((row) => row.enabledWinner)),
            });
        }
    }
    const terminalRows = [];
    for (const [subsetName, selected] of Object.entries({
        overall: rows,
        supalosa: rows.filter((row) => row.opponent === "pinned_supalosa"),
        advanced: rows.filter((row) => row.opponent === "ra2web_advanced"),
    })) {
        for (const arm of ["disabled", "enabled"]) {
            const statuses = [
                "candidate_win", "baseline_win", "simultaneous_draw",
                "engine_nonliteral_termination_draw", "tick_cap_draw",
            ];
            for (const status of statuses) {
                const values = selected.filter((row) => row[arm + "Status"] === status)
                    .map((row) => row[arm + "Updates"]);
                terminalRows.push({
                    subset: subsetName,
                    arm,
                    status,
                    count: values.length,
                    meanUpdates: values.length ? m2Mean(values) : null,
                    medianUpdates: values.length ? m2Median(values) : null,
                    minUpdates: values.length ? Math.min(...values) : null,
                    maxUpdates: values.length ? Math.max(...values) : null,
                });
            }
        }
    }
    const transitions = {};
    for (const disabled of ["candidate", "draw", "baseline"]) {
        for (const enabled of ["candidate", "draw", "baseline"]) {
            const key = disabled + "_to_" + enabled;
            transitions[key] = rows.filter((row) => row.transition === key).length;
        }
    }
    const alliedEffect = subgroupRows.find((row) => row.dimension === "faction" && row.value === "Allied");
    const sovietEffect = subgroupRows.find((row) => row.dimension === "faction" && row.value === "Soviet");
    const slot0Effect = subgroupRows.find((row) => row.dimension === "slot" && row.value === "0");
    const slot1Effect = subgroupRows.find((row) => row.dimension === "slot" && row.value === "1");
    const negativeSupalosaFamilies = familyRows.filter((row) =>
        row.opponent === "pinned_supalosa" && row.pairedScore < -0.10).length;
    const advancedEnabledWilsonLower90 = unifiedIntentM2WilsonLower90(
        summaries.advanced.enabled.wins,
        subsets.advanced.length,
    );
    const gates = {
        overallPairedScoreLowerPositive: bootstrap.overall.pairedScore.lower90 > 0,
        overallPairedWinLowerPositive: bootstrap.overall.pairedLiteralWin.lower90 > 0,
        advancedPairedScoreLowerPositive: bootstrap.advanced.pairedScore.lower90 > 0,
        advancedMoreLiteralWins:
            summaries.advanced.enabled.wins > summaries.advanced.disabled.wins,
        supalosaScoreNoninferior: bootstrap.supalosa.pairedScore.lower90 >= -0.02,
        supalosaEnabledWinsExceedLosses:
            summaries.supalosa.enabled.wins > summaries.supalosa.enabled.losses,
        supalosaFamilySafety: negativeSupalosaFamilies <= 2,
        alliedPointNonnegative: alliedEffect.pairedScore >= 0,
        sovietPointNonnegative: sovietEffect.pairedScore >= 0,
        slot0PointNonnegative: slot0Effect.pairedScore >= 0,
        slot1PointNonnegative: slot1Effect.pairedScore >= 0,
        transitionSafety: transitions.candidate_to_baseline <=
            transitions.baseline_to_candidate + transitions.draw_to_candidate,
    };
    const broadPositiveSignal = Object.values(gates).every(Boolean);
    const advancedAbsoluteSuperiority =
        summaries.advanced.enabled.wins > summaries.advanced.enabled.losses &&
        advancedEnabledWilsonLower90 > 0.5;
    const mechanisms = {
        traces: 0, proposedCalls: 0, forwardedOrderCalls: 0, sameUnitConflicts: 0,
        duplicateSuppressions: 0, deferredUnitIds: 0, updatesWithDeferral: 0,
        maxRollingTotalCalls: 0, maxRollingOrderCalls: 0, maxForwardedChunkSize: 0,
    };
    for (const pair of pairs) {
        const value = pair.arms[1].result.telemetry;
        mechanisms.traces += 1;
        for (const key of ["proposedCalls", "forwardedOrderCalls", "sameUnitConflicts",
            "duplicateSuppressions", "deferredUnitIds", "updatesWithDeferral"]) {
            mechanisms[key] += value[key];
        }
        mechanisms.maxRollingTotalCalls = Math.max(
            mechanisms.maxRollingTotalCalls, value.maxRollingTotalCalls,
        );
        mechanisms.maxRollingOrderCalls = Math.max(
            mechanisms.maxRollingOrderCalls, value.maxRollingOrderCalls,
        );
        mechanisms.maxForwardedChunkSize = Math.max(
            mechanisms.maxForwardedChunkSize, value.maxForwardedChunkSize,
        );
    }
    const directory = required("OUT_DIR");
    if (directory !== path.join(EXECUTION, "finalizer") || fs.existsSync(directory)) {
        throw new Error("M2 finalizer output path drifted");
    }
    fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
    const outputs = {
        "pairs.csv": csv(rows),
        "families.csv": csv(familyRows),
        "subgroups.csv": csv(subgroupRows),
        "terminal.csv": csv(terminalRows),
        "scheduler.csv": csv(scheduler),
        "bootstrap.json": JSON.stringify(bootstrap, null, 2) + "\n",
    };
    const descriptors = {};
    for (const [name, content] of Object.entries(outputs)) {
        const file = path.join(directory, name);
    ...
    const filesBeforeAggregate = countFiles(EXECUTION);
    const aggregate = {
        kind: "unified-intent-m2-aggregate-v1",
        complete: true,
        passed: true,
        developmentOnly: true,
        sourceCommit: source.sourceCommit,
        ...
        bootstrap,
        transitions,
        negativeSupalosaFamilies,
        advancedEnabledWilsonLower90,
        gates,
        broadPositiveSignal,
        advancedAbsoluteSuperiority,
        mechanisms,
        filesBeforeAggregate,
        expectedFilesAfter: filesBeforeAggregate + 3,
        outputs: descriptors,
    };
    if (aggregate.expectedFilesAfter >= FILE_LIMIT) throw new Error("M2 file budget exceeded");
    const aggregateFile = path.join(directory, "aggregate.json");
    writeExclusive(aggregateFile, JSON.stringify(aggregate, null, 2) + "\n");
    writeExclusive(aggregateFile + ".sha256", sha256File(aggregateFile) + "  " + aggregateFile + "\n");
    writeExclusive(
        path.join(directory, "COMPLETE"),
        completeLine("COMPLETE_UNIFIED_INTENT_M2_FINALIZER_V1", aggregateFile),
    );
    if (countFiles(EXECUTION) !== aggregate.expectedFilesAfter) {
        throw new Error("M2 final file count drifted");
    }
    console.log(JSON.stringify({
        complete: true,
        passed: true,
...
else if (mode === "smoke") await runM2Smoke();
else if (mode === "pair") await runM2Pair();
else if (mode === "finalize") finalizeM2();
else throw new Error("Unknown unified intent M2 mode");
