...
import { REPO, PROJECT, STUDY, EXECUTION, fileHash, git, writeExclusive, readPublished,
    schedulerRows, json } from "../runtime/unified-intent-d1-io.mjs";
...
const script = path.join(REPO, "research/slurm/unified_intent_d1_stage.sbatch");
const protocol = path.join(REPO, "research/protocols/method/2026-09-22-command-budget-diagnostic-d1.md");
if (fileHash(protocol) !== "151c4cd5719459b8a6dc218f17d55f7fbc563803a11d6a0b7298a0a121247ebe") throw new Error("D1 prospective protocol changed");
...
if (phase !== "pure") {
    const receipt = json(receiptPath("pure")), purePath = path.join(STUDY, "pure-v1/pure.json");
    ...
    if (receipt.sourceCommit !== source || value.sourceCommit !== source || value.scheduler.jobId !== receipt.jobId ||
        !value.complete || !value.passed || value.tests.passed !== 264 || value.tests.od1RuntimePassed !== 16 || value.tests.d1RuntimePassed !== 11 ||
        value.tests.d1AnalysisPassed !== 8 || value.tests.goldenPassed !== 1 ||
        fs.readFileSync(path.join(STUDY, "pure-v1/COMPLETE"), "utf8") !==
            "COMPLETE_UNIFIED_INTENT_D1_PURE_V1 " + digest + " " + fs.statSync(purePath).size + "\n") {
...
if (!["pure", "prepare"].includes(phase)) {
    const m = checkPublished(path.join(EXECUTION, "manifest"), "unified-intent-d1-manifest-v1");
    env.MANIFEST_SHA256 = m.sha256;
}
if (["smoke", "pairs"].includes(phase)) {
    const c = checkPublished(path.join(EXECUTION, "canary-finalizer"), "unified-intent-d1-canary-aggregate-v1");
    schedulerRows(c.value.arrayJobId, 4);
    env.CANARY_GATE_SHA256 = c.sha256;
}
if (phase === "pairs") {
    const s = checkPublished(path.join(EXECUTION, "smoke"), "unified-intent-d1-smoke-v1");
    env.SMOKE_SHA256 = s.sha256;
    console.log(JSON.stringify({ technicalSmokeResources: schedulerRows(s.value.scheduler.jobId)[0] }));
}
const intendedProgram = phase === "pure" ? path.join(REPO, "research/scripts/unified-intent-d1-pure.mjs") : program;
const intendedScript = phase === "pure" ? path.join(REPO, "research/slurm/unified_intent_d1_pure.sbatch") : script;
writeExclusive(reservation, JSON.stringify({ sourceCommit: source, phase, createdAt: new Date().toISOString(),
    programSha256: fileHash(intendedProgram), scriptSha256: fileHash(intendedScript),
    stageProgramSha256: env.PROGRAM_SHA256, protocolSha256: env.PROTOCOL_SHA256 }) + "\n");
...
if (phase === "pure") {
    const pureProgram = path.join(REPO, "research/scripts/unified-intent-d1-pure.mjs");
    const pureScript = path.join(REPO, "research/slurm/unified_intent_d1_pure.sbatch");
    submit("pure", ["--job-name=chrono-intent-d1-pure", "--mem=8G", "--time=01:00:00"],
        { ...env, OUT_DIR: path.join(STUDY, "pure-v1"), PROGRAM_SHA256: fileHash(pureProgram), SCRIPT_SHA256: fileHash(pureScript) }, pureScript);
} else if (phase === "prepare" || phase === "smoke") {
    submit(phase, ["--job-name=chrono-intent-d1-" + phase, "--mem=8G", "--time=" + (phase === "prepare" ? "08:00:00" : "06:00:00")],
        { ...env, MODE: phase }, script);
} else {
    const isCanary = phase === "canary", n = isCanary ? 4 : 200;
    const array = submit(phase, ["--job-name=chrono-intent-d1-" + phase,
        "--mem=8G", "--time=06:00:00", "--array=0-" + (n - 1) + "%" + (isCanary ? 4 : 32)],
        { ...env, MODE: isCanary ? "canary" : "pair" }, script);
    submit(phase + "-finalizer", ["--job-name=chrono-intent-d1-finalizer", "--mem=" + (isCanary ? "8G" : "24G"),
