...
const COMPLETE_RE = /^COMPLETE_UNIFIED_INTENT_GATE3_B1_CASE_V1 ([0-9a-f]{64}) ([0-9]+)$/;
...
const {
    ...
    DRIVER, "dist", "training", "unifiedIntentGate3B1Plan.js",
)).href);
const {
...
const sourceFiles = {
    b1Protocol: path.join(
        REPO, "research/protocols/method/2026-09-11-unified-intent-arbiter-v1-gate-3-ceiling-150-b1.md",
    ),
...
const programFiles = () => {
    const values = {
        program: PROGRAM,
        plan: path.join(DRIVER, "dist", "training", "unifiedIntentGate3B1Plan.js"),
        ...
        manifestSlurm: path.join(REPO, "research", "slurm", "unified_intent_gate3_b1_manifest.sbatch"),
        traceSlurm: path.join(REPO, "research", "slurm", "unified_intent_gate3_b1_trace.sbatch"),
        finalizerSlurm: path.join(REPO, "research", "slurm", "unified_intent_gate3_b1_finalizer.sbatch"),
    };
...
const verifyPureGate = () => {
    ...
    if (
        ...
        value.tests?.vitest?.files !== 10 ||
        value.tests?.vitest?.tests !== 123 ||
...
const prepare = async () => {
    ...
    const plan = buildUnifiedIntentGate3B1Plan(runtime.maps);
    ...
    const manifest = {
        kind: "unified-intent-gate3-b1-manifest-v1",
        complete: true,
        passed: true,
        technicalOnly: true,
    ...
    writeExclusive(
        ...
        completeLine("COMPLETE_UNIFIED_INTENT_GATE3_B1_MANIFEST_V1", file),
    );
...
const loadManifest = () => {
    ...
    rejectProhibited(value);
    if (
        value.kind !== "unified-intent-gate3-b1-manifest-v1" ||
        !value.complete ||
        !value.passed ||
        !value.technicalOnly ||
    ...
    validateUnifiedIntentGate3B1Plan(value.plan);
...
else throw new Error("Unknown Gate 3 B1 mode");
