...
import {buildScreenAllocation,screenAssignment,CENSUS_SHA256} from "../runtime/multimap-screen-allocation-v3.mjs";
...
const phase=process.argv[2];assert.ok(["allocate","cell","finalize"].includes(phase));
...
const plan=buildScreenAllocation(census);
...
function inspectAllocation(){
    ...
    assert.equal(a.censusSha256,CENSUS_SHA256);assert.deepEqual(a.plan,plan);
    return sha;
}
async function runCell(){
    ...
    const allocationSha256=inspectAllocation(),assignment=screenAssignment(plan,taskIndex);
    ...
    const provenance=createExperimentManifest({runId:"multimap-screen-v3-"+taskIndex+"-"+jobId,mixDir,maps:[map.fileName],
    ...
    write(required("OUT_PATH"),{kind:"multimap-v2-screen-cell-amendment-3",complete:true,taskIndex,assignment,map,provenance,
...
function finalize(){
    ...
    for(let i=0;i<900;i++){
        ...
        assert.equal(cell.kind,"multimap-v2-screen-cell-amendment-3");assert.equal(cell.complete,true);
    ...
    const maps=census.maps.map((entry,mapIndex)=>{
        const rows=results.filter(r=>r.mapIndex===mapIndex);
        return {map:entry.map,allocation:plan.maps[mapIndex],...analyzeMapScreen(rows,entry.map.startCount),
    ...
    write(required("OUT_PATH"),{kind:"multimap-v2-screen-aggregate-amendment-3",complete:true,technicalIntegrityPassed:true,
        ...
        taskCount:900,runtimeIdentity,maps,eligibleMapIds:maps.filter(m=>m.eligible).map(m=>m.map.id)});
    console.log(JSON.stringify({complete:true,maps:13,eligibleMapCount:maps.filter(m=>m.eligible).length}));
}
try{
    if(phase==="allocate"){
        write(required("OUT_PATH"),{kind:"multimap-v2-screen-allocation-amendment-3",complete:true,passed:true,outcomeFree:true,
            ...inputIdentity,sourceCommit,schedulerJobId:jobId,schedulerAccount:"pi_jss233",plan});
        console.log(JSON.stringify({complete:true,screenCount:900,confirmationCount:3168,
            changedMemberships:plan.maps.reduce((s,m)=>s+m.addedIndices.length,0)}));
    }else if(phase==="cell")await runCell();else finalize();
