...
const STUDY = path.join(PROJECT, "research-evidence", "unified-intent-arbiter-v2", "gate-2");
...
const COMPLETE_RE = /^COMPLETE_UNIFIED_INTENT_V2_GATE2_RECORD_V1 ([0-9a-f]{64}) ([0-9]+)$/;
const PROHIBITED = /winner|outcome|score|endpoint|defeated|terminal|building|inventory|credit|damage|actioncontents|statecontents|rank|failureupdate/i;
...
const {
    ...
    DRIVER, "dist", "training", "unifiedIntentV2Gate2Plan.js",
)).href);
const {
    runUnifiedIntentV2TechnicalEpisode,
} = await import(pathToFileURL(path.join(
    DRIVER, "dist", "training", "unifiedIntentV2TechnicalEpisode.js",
)).href);
const {
...
const sourceFiles = {
    gate2Protocol: path.join(
        REPO, "research/protocols/method/2026-09-15-unified-intent-separated-lane-v2-gate2.md",
    ),
    v1Gate1Result: path.join(
        REPO, "research/results/2026-09-07-unified-intent-arbiter-v1-gate-1.md",
    ),
    v1Gate2Result: path.join(
        REPO, "research/results/2026-09-09-unified-intent-arbiter-v1-gate-2-complete.md",
    ),
    v2Gate1Result: path.join(
        REPO, "research/results/2026-09-15-unified-intent-separated-lane-v2-gate1.md",
    ),
    c1Result: path.join(
        REPO, "research/results/2026-09-14-unified-intent-m2-long-horizon-c1.md",
    ),
    v2Protocol: path.join(
        REPO, "research/protocols/method/2026-09-14-unified-intent-separated-lane-v2.md",
    ),
};
...
const programFiles = () => {
    const values = {
        program: PROGRAM,
        plan: path.join(DRIVER, "dist", "training", "unifiedIntentV2Gate2Plan.js"),
        episode: path.join(DRIVER, "dist", "training", "unifiedIntentV2TechnicalEpisode.js"),
        v2Telemetry: path.join(DRIVER, "dist", "training", "unifiedIntentV2Telemetry.js"),
        literalAdjudicator: path.join(DRIVER, "dist", "training", "literalBuildingEliminationEndpoint.js"),
        instrumentation: path.join(DRIVER, "dist", "training", "freshDualStudyInstrumentation.js"),
        ...
        manifestSlurm: path.join(REPO, "research", "slurm", "unified_intent_v2_gate2_manifest.sbatch"),
        traceSlurm: path.join(REPO, "research", "slurm", "unified_intent_v2_gate2_record.sbatch"),
        finalizerSlurm: path.join(REPO, "research", "slurm", "unified_intent_v2_gate2_finalizer.sbatch"),
    };
...
const createBots = async (task, arm, factory, advancedDefinition) => {
    const candidateName = "V2Gate2Candidate";
    const opponentName = "V2Gate2Opponent";
    const botOptions = {
        intentArbiter: {
            enabled: true,
            budgetMode: arm.budgetMode,
            commandCeiling: arm.commandCeiling,
        },
    };
...
const verifyPureGate = () => {
    ...
    if (
        ...
        value.tests?.vitest?.files !== 17 ||
        value.tests?.vitest?.tests !== 159 ||
...
const prepare = async () => {
    ...
    const plan = buildUnifiedIntentV2Gate2Plan(runtime.maps);
    ...
    const manifest = {
        kind: "unified-intent-v2-gate2-manifest-v1",
        complete: true,
        passed: true,
        technicalOnly: true,
        ...
        pureGate: {
            ...
            tests: {
                files: pureGate.tests.vitest.files,
                tests: pureGate.tests.vitest.tests,
                runtimeSchemaTests: pureGate.tests.runtimeSchema.tests,
            },
        },
        runtime,
        files: programFiles(),
    ...
    writeExclusive(
        ...
        completeLine("COMPLETE_UNIFIED_INTENT_V2_GATE2_MANIFEST_V1", file),
    );
...
const loadManifest = () => {
    ...
    rejectProhibited(value);
    if (
        value.kind !== "unified-intent-v2-gate2-manifest-v1" ||
        !value.complete ||
        !value.passed ||
        !value.technicalOnly ||
        ...
        value.plan.counts.episodes !== 925 ||
    ...
    validateUnifiedIntentV2Gate2Plan(value.plan);
...
const runTrace = async () => {
    ...
    try {
        ...
        const artifact = {
            kind: "unified-intent-gate3-b1-case-v1",
            complete: true,
            passed: true,
            technicalOnly: true,
            competitiveFieldsAbsent: true,
        ...
        writeExclusive(
            path.join(directory, "COMPLETE"),
            completeLine("COMPLETE_UNIFIED_INTENT_GATE3_B1_CASE_V1", file),
        );
    } catch (error) {
        const failure = {
            kind: "unified-intent-gate3-b1-generic-failure-v1",
            complete: false,
            passed: false,
            technicalOnly: true,
            competitiveFieldsAbsent: true,
...
const parseCase = (directory, task, manifestSha256, role) => {
    ...
    rejectProhibited(value);
    if (
        value.kind !== "unified-intent-gate3-b1-case-v1" ||
        !value.complete ||
        !value.passed ||
        !value.technicalOnly ||
        !value.competitiveFieldsAbsent ||
...
const finalize = () => {
    ...
    if (armRows.length !== 1850) throw new Error("Gate 3 B1 arm trace count drifted");
    ...
    const aggregate = {
        kind: "unified-intent-gate3-b1-aggregate-v1",
        complete: true,
        passed: true,
        technicalOnly: true,
        competitiveFieldsAbsent: true,
    ...
    writeExclusive(
        path.join(directory, "COMPLETE"),
        completeLine("COMPLETE_UNIFIED_INTENT_GATE3_B1_FINALIZER_V1", aggregateFile),
    );
...
const C1_STATUSES = new Set([
    "clean", "budget_contract", "telemetry_schema_contract",
    "adjudicator_contract", "public_integrity_contract", "setup_contract",
]);

const loadC1Execution = async () => {
    const runtime = verifyRuntime();
    const manifest = loadManifest();
    exact(runtime, manifest.runtime, "M2 C1 runtime drifted");
    await gameApi.cdapi.init(runtime.assetsRoot);
    const factory = await loadBaselineFactory(path.join(REPO, "packages", "chronodivide-bot"));
    if (factory.descriptor.kind !== "external-package") {
        throw new Error("M2 C1 baseline is not external");
    }
    return {
        runtime,
        manifest,
        factory,
        advancedDefinition: loadRa2WebOpponent(
            runtime.advancedFreezeRoot,
            runtime.advancedOpponentId,
        ),
    };
...
const diagnosticFromEpisode = (value) => {
    const telemetry = value.telemetry;
    const checks = {
        setup: true,
        adjudicator: true,
        telemetrySchema: Boolean(telemetry && telemetry.complete),
        reserve: Boolean(telemetry && telemetry.gameplayReserveOverflowUpdates === 0),
        totalBudget: Boolean(telemetry && telemetry.totalCeilingOverflowUpdates === 0),
        orderCap: Boolean(telemetry && telemetry.maxRollingOrderCalls <= 115),
        totalCap: Boolean(telemetry && telemetry.maxRollingTotalCalls <= 150),
        chunkCap: Boolean(telemetry && telemetry.maxForwardedChunkSize <= 128),
        oneForward: Boolean(telemetry && telemetry.oneForwardViolationUpdates === 0),
        forwardValidation: Boolean(
            telemetry && telemetry.forwardedValidationViolationUpdates === 0,
        ),
        productionAtomicity: Boolean(
            telemetry && telemetry.partialProductionBatchViolationUpdates === 0,
        ),
        publicCall: Boolean(
            SHA256.test(value.action.sha256) &&
            value.quitSuppression.forwarded.candidate === 0 &&
            value.quitSuppression.forwarded.baseline === 0
        ),
        publicState: SHA256.test(value.trajectory.sha256),
    };
    const status = !checks.telemetrySchema
        ? "telemetry_schema_contract"
        : [checks.reserve, checks.totalBudget, checks.orderCap, checks.totalCap].some((item) => !item)
          ? "budget_contract"
          : [checks.chunkCap, checks.oneForward, checks.forwardValidation,
              checks.productionAtomicity].some((item) => !item)
            ? "telemetry_schema_contract"
            : [checks.publicCall, checks.publicState].some((item) => !item)
              ? "public_integrity_contract"
              : "clean";
    return { status, gateChecks: checks };
};

const diagnosticFromError = (error) => {
    if (!(error instanceof UnifiedIntentM2EpisodeTechnicalError)) throw error;
    if (error.technicalCategory === "setup_contract") {
        return { status: "setup_contract", gateChecks: { setup: false } };
    }
    if (error.technicalCategory === "adjudicator_contract") {
        return { status: "adjudicator_contract", gateChecks: { adjudicator: false } };
    }
    if (error.technicalCategory === "telemetry_schema_contract") {
        return {
            status: "telemetry_schema_contract",
            gateChecks: { telemetrySchema: false },
        };
    }
    if (error.technicalCategory === "public_call_contract") {
        return {
            status: "public_integrity_contract",
            gateChecks: { publicCall: false },
        };
    }
    throw error;
};

const runC1Record = async () => {
    const source = sourceIdentity();
    verifyCertificate();
    verifyPureGate();
    const context = await loadC1Execution();
    const taskIndex = Number(required("TASK_INDEX"));
    if (!Number.isSafeInteger(taskIndex) || taskIndex < 0 || taskIndex >= 900) {
        throw new Error("M2 C1 task index drifted");
    }
    const task = context.manifest.plan.cases[taskIndex];
    const map = context.runtime.maps.find((value) => value.id === task.mapId);
    if (!map) throw new Error("M2 C1 map is unavailable");
    const role = required("TRACE_ROLE");
    const directory = required("OUT_DIR");
    if (!["smoke", "array"].includes(role) || fs.existsSync(directory)) {
        throw new Error("M2 C1 output identity drifted");
    }
    fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
    try {
        const bots = await createBots(task, task.arm, context.factory, context.advancedDefinition);
        let diagnostic;
        try {
            const value = await runUnifiedIntentM2Episode({
                api: gameApi.cdapi,
                spec: {
                    mapName: map.fileName,
                    gameMode: context.manifest.gameModes[map.id],
                    country: task.country,
                    candidateSlot: task.candidateSlot,
                    candidateStart: task.candidateStart,
                    opponentStart: task.opponentStart,
                    candidateStartOrdinal: task.candidateStartOrdinal,
                    opponentStartOrdinal: task.opponentStartOrdinal,
                    requestedEngineSeed: task.requestedEngineSeed,
                    maxUpdates: 24000,
                    enabled: true,
                    diagnosticOnly: true,
            ...
            diagnostic = diagnosticFromEpisode(value);
        } catch (error) {
            diagnostic = diagnosticFromError(error);
        }
        if (
            !C1_STATUSES.has(diagnostic.status) ||
            Object.values(diagnostic.gateChecks).some((value) => typeof value !== "boolean") ||
            (diagnostic.status === "clean" &&
                Object.values(diagnostic.gateChecks).some((value) => value !== true)) ||
            (diagnostic.status !== "clean" &&
                !Object.values(diagnostic.gateChecks).some((value) => value === false))
        ) throw new Error("M2 C1 diagnostic schema drifted");
        const artifact = {
            kind: "unified-intent-m2-c1-record-v1",
            complete: true,
        ...
        writeExclusive(file, JSON.stringify(artifact) + "\n");
        if (fs.statSync(file).size > TRACE_LIMIT) throw new Error("M2 C1 record too large");
        writeExclusive(
            path.join(directory, "COMPLETE"),
            completeLine("COMPLETE_UNIFIED_INTENT_M2_C1_RECORD_V1", file),
        );
    } catch (error) {
        const failure = {
            kind: "unified-intent-m2-c1-runner-failure-v1",
            complete: false,
            passed: false,
            technicalOnly: true,
            competitiveFieldsAbsent: true,
...
const parseC1Record = (directory, task, manifestSha256, role) => {
    exact(fs.readdirSync(directory).sort(), ["COMPLETE", "record.json"], "M2 C1 files drifted");
    const file = path.join(directory, "record.json");
    const match = COMPLETE_RE.exec(fs.readFileSync(path.join(directory, "COMPLETE"), "utf8").trim());
    if (
        !match || match[1] !== sha256File(file) || Number(match[2]) !== fs.statSync(file).size ||
        fs.statSync(file).size > TRACE_LIMIT
    ) throw new Error("M2 C1 checksum drifted");
    const value = json(file);
    rejectProhibited(value);
    if (
        value.kind !== "unified-intent-m2-c1-record-v1" || !value.complete ||
        !value.technicalOnly || !value.competitiveFieldsAbsent || !value.payloadDiscarded ||
        value.role !== role || value.manifestSha256 !== manifestSha256 ||
        !C1_STATUSES.has(value.diagnostic.status) ||
        value.passed !== (value.diagnostic.status === "clean")
    ) throw new Error("M2 C1 record drifted");
    exact(value.assignment, task, "M2 C1 assignment drifted");
    return value;
};

const c1Faction = (country) => [
...
const finalizeC1 = () => {
    ...
    const runtime = verifyRuntime();
    const manifest = loadManifest();
    exact(runtime, manifest.runtime, "M2 C1 finalizer runtime drifted");
    const manifestSha256 = requiredHash("MANIFEST_SHA256");
    const arrayJobId = required("ARRAY_JOB_ID");
    const scheduler = schedulerRows(arrayJobId, 900);
    const smoke = parseC1Record(
        path.join(EXECUTION, "smoke"),
        manifest.plan.cases[0],
        manifestSha256,
        "smoke",
    );
    if (
        smoke.sourceCommit !== source.sourceCommit || smoke.programSha256 !== source.programSha256 ||
        smoke.scheduler.account !== "pi_jss233" || smoke.scheduler.partition !== "day"
    ) throw new Error("M2 C1 smoke drifted");
    const records = manifest.plan.cases.map((task) => parseC1Record(
    ...
    for (const [taskIndex, value] of records.entries()) {
        const map = runtime.maps.find((row) => row.id === value.assignment.mapId);
        if (
            value.sourceCommit !== source.sourceCommit || value.programSha256 !== source.programSha256 ||
            !map || value.mapSha256 !== map.sha256 || value.scheduler.arrayJobId !== arrayJobId ||
            String(value.scheduler.arrayTaskId) !== String(taskIndex) ||
            value.scheduler.account !== "pi_jss233" || value.scheduler.partition !== "day" ||
            typeof value.scheduler.jobId !== "string"
        ) throw new Error("M2 C1 scheduler identity drifted");
        schedulerIds.add(value.scheduler.jobId);
    }
    if (schedulerIds.size !== 900) throw new Error("M2 C1 scheduler IDs are not unique");
    const rows = records.map((value) => ({
        taskIndex: value.assignment.taskIndex,
        caseIndex: value.assignment.caseIndex,
        familyId: value.assignment.familyId,
        opponent: value.assignment.opponent,
        mapId: value.assignment.mapId,
        directionOrdinal: value.assignment.directionOrdinal,
        faction: c1Faction(value.assignment.country),
    ...
    const statusCounts = Object.fromEntries([...C1_STATUSES].map((status) => [
        status,
        rows.filter((row) => row.status === status).length,
    ]));
    const gateFailures = {};
    for (const row of rows) {
        for (const [name, passed] of Object.entries(row.gateChecks)) {
            if (!passed) gateFailures[name] = (gateFailures[name] ?? 0) + 1;
        }
    }
    const contingency = rows.map((row) => ({
        opponent: row.opponent,
        mapId: row.mapId,
        directionOrdinal: row.directionOrdinal,
        faction: row.faction,
        country: row.country,
    ...
    const hardCeilingEligible = statusCounts.clean === 900;
    const directory = required("OUT_DIR");
    if (directory !== path.join(EXECUTION, "finalizer") || fs.existsSync(directory)) {
        throw new Error("M2 C1 finalizer output drifted");
    }
    fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
    const outputs = {
        "records.csv": csv(rows),
    ...
    const filesBeforeAggregate = countFiles(EXECUTION);
    const aggregate = {
        kind: "unified-intent-m2-c1-aggregate-v1",
        complete: true,
        passed: true,
        technicalOnly: true,
        competitiveFieldsAbsent: true,
        payloadsDiscarded: true,
        hardCeilingEligible,
        sourceCommit: source.sourceCommit,
        programSha256: source.programSha256,
        manifestSha256,
    ...
    rejectProhibited(aggregate);
    if (aggregate.expectedFilesAfter >= FILE_LIMIT) throw new Error("M2 C1 file budget exceeded");
    const aggregateFile = path.join(directory, "aggregate.json");
    writeExclusive(aggregateFile, JSON.stringify(aggregate, null, 2) + "\n");
    writeExclusive(aggregateFile + ".sha256", sha256File(aggregateFile) + "  " + aggregateFile + "\n");
    writeExclusive(
        path.join(directory, "COMPLETE"),
        completeLine("COMPLETE_UNIFIED_INTENT_M2_C1_FINALIZER_V1", aggregateFile),
    );
    if (countFiles(EXECUTION) !== aggregate.expectedFilesAfter) {
        throw new Error("M2 C1 final file count drifted");
    }
    console.log(JSON.stringify({
        complete: true,
        passed: true,
...
const loadV2Execution = async () => {
    const runtime = verifyRuntime();
    const manifest = loadManifest();
    exact(runtime, manifest.runtime, "V2 Gate 2 runtime drifted");
    await gameApi.cdapi.init(runtime.assetsRoot);
    const factory = await loadBaselineFactory(path.join(REPO, "packages", "chronodivide-bot"));
    if (factory.descriptor.kind !== "external-package") {
        throw new Error("V2 Gate 2 baseline is not external");
    }
    return {
        runtime,
        manifest,
...
const runV2Record = async () => {
    const source = sourceIdentity();
    verifyCertificate();
    verifyPureGate();
    const context = await loadV2Execution();
    const taskIndex = Number(required("TASK_INDEX"));
    if (!Number.isSafeInteger(taskIndex) || taskIndex < 0 || taskIndex >= 925) {
        throw new Error("V2 Gate 2 task index drifted");
    }
    const task = context.manifest.plan.tasks[taskIndex];
    const map = context.runtime.maps.find((value) => value.id === task.mapId);
    if (!map) throw new Error("V2 Gate 2 map is unavailable");
    const role = required("TRACE_ROLE");
    const directory = required("OUT_DIR");
    if (!["smoke", "array"].includes(role) || fs.existsSync(directory)) {
        throw new Error("V2 Gate 2 output identity drifted");
    }
    fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
    try {
        const bots = await createBots(task, task.arm, context.factory, context.advancedDefinition);
        const result = await runUnifiedIntentV2TechnicalEpisode({
            api: gameApi.cdapi,
            spec: {
                mapName: map.fileName,
                gameMode: context.manifest.gameModes[map.id],
        ...
        const candidateEssentialCalls = V2_ESSENTIAL_METHODS.reduce(
            (sum, method) => sum + (result.publicCall.bySideAndMethod["candidate." + method] ?? 0),
            0,
        );
        const checks = {
            mode: result.telemetry.budgetMode === "separated_lanes_v2",
            commandCeiling: result.telemetry.commandCeiling === 115,
            rollingWindow: result.telemetry.rollingUpdates === 900,
            commandCap: result.telemetry.maxRollingCommandCalls <= 115 &&
                result.telemetry.commandCeilingOverflowUpdates === 0,
            chunkCap: result.telemetry.maxForwardedChunkSize <= 128,
            oneForward: result.telemetry.oneForwardViolationUpdates === 0,
            forwardValidation: result.telemetry.forwardedValidationViolationUpdates === 0,
            productionAtomicity: result.telemetry.partialProductionBatchViolationUpdates === 0,
            essentialForwarding: candidateEssentialCalls === result.telemetry.essentialCalls,
            publicCall: SHA256.test(result.publicCall.sha256),
            publicState: SHA256.test(result.publicState.sha256),
            telemetryDigest: SHA256.test(result.telemetry.telemetrySha256),
            resignation: result.quitSuppression.forwarded.candidate === 0 &&
                result.quitSuppression.forwarded.baseline === 0,
            adjudicator: result.literalAdjudicatorVersion === 5 &&
                SHA256.test(result.literalAdjudicatorSha256),
        };
        if (Object.values(checks).some((value) => value !== true)) {
            throw new Error("V2 Gate 2 technical check failed");
        }
        const technical = {
            budgetMode: result.telemetry.budgetMode,
            commandCeiling: result.telemetry.commandCeiling,
            rollingUpdates: result.telemetry.rollingUpdates,
            checks,
            essentialCalls: result.telemetry.essentialCalls,
            productionBatches: result.telemetry.productionBatches,
            proposedCalls: result.telemetry.proposedCalls,
            forwardedOrderCalls: result.telemetry.forwardedOrderCalls,
            sameUnitConflicts: result.telemetry.sameUnitConflicts,
            duplicateSuppressions: result.telemetry.duplicateSuppressions,
            deferredUnitIds: result.telemetry.deferredUnitIds,
            updatesWithDeferral: result.telemetry.updatesWithDeferral,
            maxRollingCommandCalls: result.telemetry.maxRollingCommandCalls,
            maxForwardedChunkSize: result.telemetry.maxForwardedChunkSize,
            publicCallSha256: result.publicCall.sha256,
            publicStateSha256: result.publicState.sha256,
            telemetrySha256: result.telemetry.telemetrySha256,
            literalAdjudicatorSha256: result.literalAdjudicatorSha256,
        };
        technical.technicalSha256 = hash(canonical(technical));
        const artifact = {
            kind: "unified-intent-v2-gate2-record-v1",
            complete: true,
            passed: true,
            technicalOnly: true,
            competitiveFieldsAbsent: true,
        ...
        const file = path.join(directory, "record.json");
        writeExclusive(file, JSON.stringify(artifact) + "\n");
        if (fs.statSync(file).size > TRACE_LIMIT) throw new Error("V2 Gate 2 record too large");
        writeExclusive(
            path.join(directory, "COMPLETE"),
            completeLine("COMPLETE_UNIFIED_INTENT_V2_GATE2_RECORD_V1", file),
        );
    } catch (error) {
        const failure = {
            kind: "unified-intent-v2-gate2-generic-failure-v1",
            complete: false,
            passed: false,
            technicalOnly: true,
            competitiveFieldsAbsent: true,
...
const parseV2Record = (directory, task, manifestSha256, role) => {
    exact(fs.readdirSync(directory).sort(), ["COMPLETE", "record.json"], "V2 Gate 2 files drifted");
    const file = path.join(directory, "record.json");
    const match = COMPLETE_RE.exec(fs.readFileSync(path.join(directory, "COMPLETE"), "utf8").trim());
    if (!match || match[1] !== sha256File(file) ||
        Number(match[2]) !== fs.statSync(file).size || fs.statSync(file).size > TRACE_LIMIT) {
        throw new Error("V2 Gate 2 checksum drifted");
    }
    const value = json(file);
    rejectProhibited(value);
    if (value.kind !== "unified-intent-v2-gate2-record-v1" || !value.complete ||
        !value.passed || !value.technicalOnly || !value.competitiveFieldsAbsent ||
        !value.payloadDiscarded || value.role !== role ||
        value.manifestSha256 !== manifestSha256 ||
        !SHA256.test(value.technical.technicalSha256) ||
        Object.values(value.technical.checks).some((check) => check !== true)) {
        throw new Error("V2 Gate 2 record drifted");
    }
    exact(value.assignment, task, "V2 Gate 2 assignment drifted");
    const comparable = { ...value.technical };
    const digest = comparable.technicalSha256;
    delete comparable.technicalSha256;
    if (hash(canonical(comparable)) !== digest) throw new Error("V2 technical digest drifted");
    return value;
};

const finalizeV2Gate2 = () => {
    const source = sourceIdentity();
    verifyCertificate();
    verifyPureGate();
    const runtime = verifyRuntime();
    const manifest = loadManifest();
    exact(runtime, manifest.runtime, "V2 Gate 2 finalizer runtime drifted");
    const manifestSha256 = requiredHash("MANIFEST_SHA256");
    const arrayJobId = required("ARRAY_JOB_ID");
    const scheduler = schedulerRows(arrayJobId, 925);
    const smoke = parseV2Record(
        path.join(EXECUTION, "smoke"),
        manifest.plan.tasks[0],
        manifestSha256,
        "smoke",
    );
    if (smoke.sourceCommit !== source.sourceCommit ||
        smoke.programSha256 !== source.programSha256 ||
        smoke.scheduler.account !== "pi_jss233" || smoke.scheduler.partition !== "day") {
        throw new Error("V2 Gate 2 smoke drifted");
    }
    const records = manifest.plan.tasks.map((task) => parseV2Record(
        path.join(EXECUTION, "cells", "task-" + String(task.taskIndex).padStart(4, "0")),
        task,
        manifestSha256,
        "array",
    ...
    for (const [taskIndex, value] of records.entries()) {
        const map = runtime.maps.find((row) => row.id === value.assignment.mapId);
        if (value.sourceCommit !== source.sourceCommit ||
            value.programSha256 !== source.programSha256 || !map ||
            value.mapSha256 !== map.sha256 || value.scheduler.arrayJobId !== arrayJobId ||
            String(value.scheduler.arrayTaskId) !== String(taskIndex) ||
            value.scheduler.account !== "pi_jss233" || value.scheduler.partition !== "day" ||
            typeof value.scheduler.jobId !== "string") {
            throw new Error("V2 Gate 2 scheduler identity drifted");
        }
        schedulerIds.add(value.scheduler.jobId);
    }
    if (schedulerIds.size !== 925) throw new Error("V2 Gate 2 scheduler IDs are not unique");
    const duplicates = manifest.plan.tasks.slice(900).map((task) => {
        const original = records[task.duplicateOfTaskIndex];
        const duplicate = records[task.taskIndex];
        const identical = canonical(original.technical) === canonical(duplicate.technical);
        return {
            originalTaskIndex: task.duplicateOfTaskIndex,
            duplicateTaskIndex: task.taskIndex,
            caseIndex: task.caseIndex,
            ...
            identical,
            technicalSha256: duplicate.technical.technicalSha256,
        };
    });
    if (duplicates.length !== 25 || duplicates.some((value) => !value.identical)) {
        throw new Error("V2 Gate 2 duplicate determinism failed");
    }
    const rows = records.map((value) => ({
        taskIndex: value.assignment.taskIndex,
        caseIndex: value.assignment.caseIndex,
        executionReplicateOrdinal: value.assignment.executionReplicateOrdinal,
        familyId: value.assignment.familyId,
        opponent: value.assignment.opponent,
        mapId: value.assignment.mapId,
        directionOrdinal: value.assignment.directionOrdinal,
        ...
        candidateSlot: value.assignment.candidateSlot,
        requestedEngineSeed: value.assignment.requestedEngineSeed,
        essentialCalls: value.technical.essentialCalls,
        productionBatches: value.technical.productionBatches,
        proposedCalls: value.technical.proposedCalls,
        forwardedOrderCalls: value.technical.forwardedOrderCalls,
        sameUnitConflicts: value.technical.sameUnitConflicts,
        duplicateSuppressions: value.technical.duplicateSuppressions,
        deferredUnitIds: value.technical.deferredUnitIds,
        updatesWithDeferral: value.technical.updatesWithDeferral,
        maxRollingCommandCalls: value.technical.maxRollingCommandCalls,
        maxForwardedChunkSize: value.technical.maxForwardedChunkSize,
        technicalSha256: value.technical.technicalSha256,
    }));
    const mechanisms = {
        episodes: records.length,
        essentialCalls: rows.reduce((sum, row) => sum + row.essentialCalls, 0),
        productionBatches: rows.reduce((sum, row) => sum + row.productionBatches, 0),
        proposedCalls: rows.reduce((sum, row) => sum + row.proposedCalls, 0),
        forwardedOrderCalls: rows.reduce((sum, row) => sum + row.forwardedOrderCalls, 0),
        sameUnitConflicts: rows.reduce((sum, row) => sum + row.sameUnitConflicts, 0),
        duplicateSuppressions: rows.reduce((sum, row) => sum + row.duplicateSuppressions, 0),
        deferredUnitIds: rows.reduce((sum, row) => sum + row.deferredUnitIds, 0),
        updatesWithDeferral: rows.reduce((sum, row) => sum + row.updatesWithDeferral, 0),
        maxRollingCommandCalls: Math.max(...rows.map((row) => row.maxRollingCommandCalls)),
        maxForwardedChunkSize: Math.max(...rows.map((row) => row.maxForwardedChunkSize)),
    };
    for (const key of ["essentialCalls", "productionBatches", "proposedCalls",
        "forwardedOrderCalls", "sameUnitConflicts", "duplicateSuppressions",
        "deferredUnitIds", "updatesWithDeferral"]) {
        if (mechanisms[key] < 1) throw new Error("V2 Gate 2 mechanism did not activate: " + key);
    }
    if (mechanisms.maxRollingCommandCalls > 115 || mechanisms.maxForwardedChunkSize > 128) {
        throw new Error("V2 Gate 2 aggregate cap failed");
    }
    const directory = required("OUT_DIR");
    if (directory !== path.join(EXECUTION, "finalizer") || fs.existsSync(directory)) {
        throw new Error("V2 Gate 2 finalizer output drifted");
    }
    fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
    const outputs = {
        "records.csv": csv(rows),
    ...
    const filesBeforeAggregate = countFiles(EXECUTION);
    const aggregate = {
        kind: "unified-intent-v2-gate2-aggregate-v1",
        complete: true,
        passed: true,
        technicalOnly: true,
        competitiveFieldsAbsent: true,
    ...
    rejectProhibited(aggregate);
    if (aggregate.expectedFilesAfter >= FILE_LIMIT) throw new Error("V2 Gate 2 file budget exceeded");
    const aggregateFile = path.join(directory, "aggregate.json");
    writeExclusive(aggregateFile, JSON.stringify(aggregate, null, 2) + "\n");
    writeExclusive(aggregateFile + ".sha256", sha256File(aggregateFile) + "  " + aggregateFile + "\n");
    writeExclusive(
        path.join(directory, "COMPLETE"),
        completeLine("COMPLETE_UNIFIED_INTENT_V2_GATE2_FINALIZER_V1", aggregateFile),
    );
    if (countFiles(EXECUTION) !== aggregate.expectedFilesAfter) {
        throw new Error("V2 Gate 2 final file count drifted");
    }
    console.log(JSON.stringify({
        complete: true,
        passed: true,
...
else if (mode === "record") await runV2Record();
else if (mode === "finalize") finalizeV2Gate2();
else throw new Error("Unknown V2 Gate 2 mode");
