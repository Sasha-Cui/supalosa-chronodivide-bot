...
const COMPLETE_RE = /^COMPLETE_UNIFIED_INTENT_M2_C1_RECORD_V1 ([0-9a-f]{64}) ([0-9]+)$/;
const PROHIBITED = /winner|outcome|score|endpoint|defeated|terminal|building|inventory|unit|credit|damage|action|trajectory|rank|updatecount|failureupdate/i;
...
const {
    ...
    DRIVER, "dist", "training", "unifiedIntentM2C1Plan.js",
)).href);
const {
    runUnifiedIntentM2Episode,
    UnifiedIntentM2EpisodeTechnicalError,
} = await import(pathToFileURL(path.join(
...
const sourceFiles = {
    c1Protocol: path.join(
        REPO, "research/protocols/method/2026-09-13-unified-intent-m2-long-horizon-diagnostic-c1.md",
    ),
    ...
    m2Failure: path.join(
        REPO, "research/results/2026-09-13-unified-intent-m2-cancelled-technical-failure.md",
    ),
    a1Result: path.join(
...
const programFiles = () => {
    const values = {
        program: PROGRAM,
        plan: path.join(DRIVER, "dist", "training", "unifiedIntentM2C1Plan.js"),
        episode: path.join(DRIVER, "dist", "training", "unifiedIntentM2Episode.js"),
        diagnosticTelemetry: path.join(DRIVER, "dist", "training", "unifiedIntentM2Telemetry.js"),
        ...
        manifestSlurm: path.join(REPO, "research", "slurm", "unified_intent_m2_c1_manifest.sbatch"),
        traceSlurm: path.join(REPO, "research", "slurm", "unified_intent_m2_c1_record.sbatch"),
        finalizerSlurm: path.join(REPO, "research", "slurm", "unified_intent_m2_c1_finalizer.sbatch"),
    };
...
const verifyPureGate = () => {
    ...
    if (
        ...
        value.tests?.vitest?.files !== 15 ||
        value.tests?.vitest?.tests !== 150 ||
...
const prepare = async () => {
    ...
    const plan = buildUnifiedIntentM2C1Plan(runtime.maps);
    ...
    const manifest = {
        kind: "unified-intent-m2-c1-manifest-v1",
        complete: true,
        passed: true,
        technicalOnly: true,
    ...
    writeExclusive(
        ...
        completeLine("COMPLETE_UNIFIED_INTENT_M2_C1_MANIFEST_V1", file),
    );
...
const loadManifest = () => {
    ...
    rejectProhibited(value);
    if (
        value.kind !== "unified-intent-m2-c1-manifest-v1" ||
        !value.complete ||
        !value.passed ||
        !value.technicalOnly ||
    ...
    validateUnifiedIntentM2C1Plan(value.plan);
...
else if (mode === "record") await runC1Record();
else if (mode === "finalize") finalizeC1();
else throw new Error("Unknown M2 C1 mode");
