...
const ROOT = path.join(path.dirname(REPO), "research-evidence/unified-intent-arbiter-v2/d1");
...
const assertSource = () => {
    if (process.version !== "v20.13.1" || git("branch", "--show-current") !== "main" ||
        ...
        fileHash(path.join(REPO, "research/slurm/unified_intent_d1_pure.sbatch")) !== scriptHash ||
        fileHash(path.join(REPO, "research/protocols/method/2026-09-22-command-budget-diagnostic-d1.md")) !== "151c4cd5719459b8a6dc218f17d55f7fbc563803a11d6a0b7298a0a121247ebe") {
        throw new Error("D1 pure source identity drifted");
    }
};
assertSource();
if (!out.startsWith(ROOT + "/") || fs.existsSync(out) || process.env.SLURM_JOB_ACCOUNT !== "pi_jss233" ||
    ...
    Number(process.env.SLURM_RESTART_COUNT ?? 0) !== 0) throw new Error("D1 pure scheduler/output invalid");
...
try {
    ...
    if (!results.success || results.testResults.length !== 30 || results.numTotalTests !== 264 ||
        results.numPassedTests !== 264 || results.numFailedTests !== 0 || results.numPendingTests !== 0 ||
        results.numTodoTests !== 0) throw new Error("D1 pure test population or results failed");
    ...
    if (!/^# pass 1$/m.test(runtime) || !/^# fail 0$/m.test(runtime)) throw new Error("D1 runtime test failed");
    ...
    if (!/^# pass 16$/m.test(od1Runtime) || !/^# fail 0$/m.test(od1Runtime)) throw new Error("D1 orchestration/analysis tests failed");
    const d1Checks = [ ["d1Runtime", "runtime", 11], ["d1Analysis", "analysis", 8], ["golden", "golden", 1] ];
    const d1Evidence = {};
    for (const [label, name, expected] of d1Checks) {
        const file = path.join(REPO, "research/tests/unified-intent-d1-" + name + ".test.mjs");
        const log = execFileSync(process.execPath, ["--test", file], { cwd: REPO, encoding: "utf8", maxBuffer: 32 * 1024 * 1024 });
        exclusive(label + ".log", log);
        if (!log.includes("# pass " + expected + "\n") || !log.includes("# fail 0\n")) throw new Error("D1 pure check failed");
        d1Evidence[label] = { passed: expected, testSha256: fileHash(file), logSha256: hash(log) };
    }
    assertSource();
    const artifact = {
        kind: "unified-intent-d1-pure-v1", complete: true, passed: true,
        technicalOnly: true, sourceCommit: source, programSha256: programHash, scriptSha256: scriptHash,
        tests: { build: true, files: 30, passed: 264, runtimePassed: 1, od1RuntimePassed: 16,
            d1RuntimePassed: 11, d1AnalysisPassed: 8, goldenPassed: 1, d1Evidence,
    ...
    exclusive("COMPLETE", "COMPLETE_UNIFIED_INTENT_D1_PURE_V1 " + hash(data) + " " + Buffer.byteLength(data) + "\n");
    console.log(JSON.stringify({ complete: true, tests: 301, sha256: hash(data) }));
} catch (error) {
