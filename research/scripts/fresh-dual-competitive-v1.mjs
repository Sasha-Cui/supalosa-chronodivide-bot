...
const ROOT = path.join(STUDY_ROOT, "execution-v1/competitive-v1");
...
const fileBindings = () => {
    const relative = [
        "research/scripts/fresh-dual-competitive-v1.mjs",
        "research/slurm/fresh_dual_competitive_v1.sbatch",
...
const loadManifest = () => {
    ...
    const manifest = sealed(MANIFEST_DIR, "manifest", "COMPLETE_FRESH_DUAL_COMPETITIVE_MANIFEST_V1");
...
const main = async () => {
    ...
    if (mode === "prepare") {
        ...
        write(MANIFEST_FILE, {
            kind: "fresh-dual-competitive-manifest-v1",
            complete: true,
            passed: true,
            sourceCommit,
            ...
            analysis: {
                moduleSha256: fileBindings()["research/runtime/fresh-dual-analysis-v1.mjs"],
    ...
    assert.equal(
        loaded.manifest.files["research/scripts/fresh-dual-competitive-v1.mjs"],
        programSha256,
    );
    const common = {
        sourceCommit,
    ...
    for (let gameIndex = 0; gameIndex < 2700; gameIndex += 1) {
        ...
        const cell = sealed(directory, "case", "COMPLETE_FRESH_DUAL_COMPETITIVE_CELL_V1");
    ...
    const aggregate = {
        kind: "fresh-dual-competitive-aggregate-v1",
        complete: true,
        passed: true,
        ...common,
        arrayJobId,
...
main().catch((error) => {
    const failure = {
        kind: "fresh-dual-competitive-technical-failure-v1",
        complete: false,
