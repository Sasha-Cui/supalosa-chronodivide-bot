...
export const MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_EPISODE_SCHEMA_VERSION = 1 as const;

export type MissionNativeCloseoutOpenDevelopmentEpisodeSpec = {
    schemaVersion: typeof MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_EPISODE_SCHEMA_VERSION;
    episodeId: string;
    familyId: string;
    mapName: string;
    mapSha256: string;
...
export type MissionNativeCloseoutOpenDevelopmentWinner = "candidate" | "baseline" | "draw";
export type MissionNativeCloseoutOpenDevelopmentOutcomeStatus =
    | "candidate_win"
    | "baseline_win"
    | "simultaneous_draw"
...
export type MissionNativeCloseoutOpenDevelopmentEpisodeResult = {
    schemaVersion: typeof MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_EPISODE_SCHEMA_VERSION;
    episodeId: string;
    familyId: string;
    mapName: string;
    ...
    policyId: string;
    policySha256: string;
    candidateCore: MissionNativeCloseoutExperimentPolicy["candidateCore"];
    missionPolicyId: MissionNativeCloseoutExperimentPolicy["missionPolicyId"];
    policyInformationBoundary: "none" | "visible_memory" | "public_complete_state";
    ...
    wallTimeMs: number;
    shortGame: false;
    outcomeStatus: MissionNativeCloseoutOpenDevelopmentOutcomeStatus;
    winner: MissionNativeCloseoutOpenDevelopmentWinner | null;
    ...
    policyTelemetry: BuildingEliminationTelemetryEvent[];
};

const SHA256_PATTERN = /^[0-9a-f]{64}$/;
const IDENTIFIER_PATTERN = /^[A-Za-z0-9._-]+$/;

export const validateMissionNativeCloseoutOpenDevelopmentEpisodeSpec = (
    spec: MissionNativeCloseoutOpenDevelopmentEpisodeSpec,
): MissionNativeCloseoutOpenDevelopmentEpisodeSpec => {
    if (spec.schemaVersion !== MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_EPISODE_SCHEMA_VERSION) {
        throw new Error(`Mission-native closeout open-development episode schemaVersion must be ${MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_EPISODE_SCHEMA_VERSION}`);
    }
    for (const [label, value] of [["episodeId", spec.episodeId], ["familyId", spec.familyId]] as const) {
    ...
    if (spec.mapName !== spec.mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(spec.mapName)) {
        throw new Error(`Mission-native closeout open-development mapName must be a .map or .mpr basename; got ${spec.mapName}`);
    }
    if (!SHA256_PATTERN.test(spec.mapSha256)) throw new Error("Mission-native closeout open-development mapSha256 must be lowercase SHA-256");
    const policy = validateMissionNativeCloseoutExperimentPolicy(spec.policy);
    if (missionNativeCloseoutExperimentPolicySha256(policy) !== spec.policyId) {
        throw new Error("Mission-native closeout open-development policyId does not equal the canonical policy SHA-256");
    }
    if (!Number.isSafeInteger(spec.seedBlockIndex) || spec.seedBlockIndex < 0) {
        throw new Error("Mission-native closeout open-development seedBlockIndex must be a nonnegative integer");
    }
    if (!Number.isSafeInteger(spec.requestedEngineSeed) || spec.requestedEngineSeed < 0 || spec.requestedEngineSeed > 0xffff_ffff) {
        throw new Error("Mission-native closeout open-development requestedEngineSeed must be uint32");
    }
    if (spec.candidateSlot !== 0 && spec.candidateSlot !== 1) {
        throw new Error("Mission-native closeout open-development candidateSlot must be 0 or 1");
    }
    if (!Object.values(Countries).includes(spec.candidateCountry)) {
        throw new Error("Mission-native closeout open-development candidateCountry is invalid");
    }
    if (!Object.values(Countries).includes(spec.baselineCountry)) {
        throw new Error("Mission-native closeout open-development baselineCountry is invalid");
    }
    if (!Number.isSafeInteger(spec.maxTicks) || spec.maxTicks < 1 || spec.maxTicks > 100_000) {
        throw new Error("Mission-native closeout open-development maxTicks must be in [1, 100000]");
    }
    return { ...spec, policy };
};
...
const score = (winner: MissionNativeCloseoutOpenDevelopmentWinner | null): 0 | 0.5 | 1 | null =>
    winner === "candidate" ? 1 : winner === "baseline" ? 0 : winner === "draw" ? 0.5 : null;

export const createMissionNativeCloseoutOpenDevelopmentExperimentCandidate = (
    factory: BaselineFactory,
    name: string,
    country: Countries,
    policy: MissionNativeCloseoutExperimentPolicy,
    telemetry: (event: BuildingEliminationTelemetryEvent) => void,
): InspectableBaselineBot => policy.candidateCore === "external_supalosa"
    ? factory.create(name, country)
    : createMissionNativeCloseoutCandidate(factory, name, country, policy.missionPolicy!, telemetry);

/** Run one predeclared game with no retry, outcome filter, or endpoint substitution. */
export const runMissionNativeCloseoutOpenDevelopmentEpisode = async (
    rawSpec: MissionNativeCloseoutOpenDevelopmentEpisodeSpec,
    baselineFactory: BaselineFactory,
): Promise<MissionNativeCloseoutOpenDevelopmentEpisodeResult> => {
    const spec = validateMissionNativeCloseoutOpenDevelopmentEpisodeSpec(rawSpec);
    const startedAt = Date.now();
    const candidateName = `MissionNativeCandidate_${spec.seedBlockIndex}_${spec.candidateSlot}`;
    const baselineName = `MissionNativeBaseline_${spec.seedBlockIndex}_${spec.candidateSlot}`;
    const policyTelemetry: BuildingEliminationTelemetryEvent[] = [];
    const candidate = createMissionNativeCloseoutOpenDevelopmentExperimentCandidate(
        baselineFactory,
        candidateName,
    ...
    return withSeededOfflineGame(
        ...
        async (game) => {
            ...
            const capDraw: LiteralEndpointCapDraw | null = !terminal && !technicalFailure && ticks >= spec.maxTicks
                ...
                : null;
            const outcomeStatus: MissionNativeCloseoutOpenDevelopmentOutcomeStatus = technicalFailure
                ? "technical_failure"
                : terminal
                  ? terminal.status
                  : "tick_cap_draw";
            const winner: MissionNativeCloseoutOpenDevelopmentWinner | null = technicalFailure
                ? null
                : terminal
                  ? terminal.winner
                  : "draw";
            ...
            return {
                ...
                policySha256: missionNativeCloseoutExperimentPolicySha256(spec.policy),
                candidateCore: spec.policy.candidateCore,
                missionPolicyId: spec.policy.missionPolicyId,
                policyInformationBoundary: spec.policy.missionPolicy?.informationInterface ?? "none",
                endpointVersion: LITERAL_BUILDING_ELIMINATION_ENDPOINT_VERSION,
                endpointSha256: LITERAL_BUILDING_ELIMINATION_ENDPOINT_SHA256,
