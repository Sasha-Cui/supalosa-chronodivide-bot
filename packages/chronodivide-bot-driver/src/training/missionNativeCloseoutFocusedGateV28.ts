...
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_ENGINE_SEED_BASE = 4_292_000_000 as const;
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_COUNTRIES = [Countries.USA, Countries.LIBYA] as const;
...
export const validateMissionNativeCloseoutFocusedGateV28Telemetry = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
    country: Countries,
): void => {
    ...
    if (progress.some((event) =>
        event.schemaVersion !== 2 || !Number.isFinite(event.damage) || event.damage <= 0,
    ...
    if (activation.length === 0 || activation.some((event) =>
        event.schemaVersion !== 22 || event.compatibleAttackerCount < 0 ||
        ...
        event.directObjectiveFeasible !== (
            event.targetId !== null && event.blockerId === null &&
            event.estimatedBuildingCompletionTicks !== null
        ) ||
        event.completeRouteFeasible !== (
            event.targetId !== null && event.blockerId !== null &&
        ...
        (event.directObjectiveFeasible && event.phase !== "building_ready") ||
        (event.completeRouteFeasible && event.phase !== "blocker_ready") ||
        (event.phase === "building_ready" && !event.directObjectiveFeasible) ||
        (event.phase === "blocker_ready" && event.blockerId === null)
    )) throw new Error(`Invalid schema-22 objective arbitration for ${country}`);
    ...
    if (capability.length === 0) {
        ...
        const blockedDelegation = activation.some((event) =>
            event.schemaVersion === 22 && event.phase === "blocked" && event.enemyBuildingCount > 1 &&
            event.directObjectiveFeasible === false && event.completeRouteFeasible === false &&
            event.partialBlockerLaunchPermitted === false &&
            event.activePredecessorCompatibleAttackerCount > 0,
        );
    ...
    if (
        launch.tick !== activated[0].tick || launch.tick < firstPhysical.tick ||
        launchEvaluation?.schemaVersion !== 22 ||
        ...
        throw new Error(`Launch does not match schema-22 objective feasibility for ${country}`);
    }
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Focused V28 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("Focused V28 gate map bytes drifted");
    }
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const policy = buildMissionNativeCloseoutPolicyV28(true);
    await cdapi.init(path.join(process.cwd(), "data"));
    const rows: Array<Record<string, unknown>> = [];
    for (const [index, country] of MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_COUNTRIES.entries()) {
        const requestedEngineSeed = derivePairedEngineSeed(
            MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_ENGINE_SEED_BASE,
            index,
        );
        ...
        for (const [label, trace] of [["first", first], ["repeat", repeat]] as const) {
            try {
                validateMissionNativeCloseoutFocusedGateV28Telemetry(trace.telemetry, country);
            } catch (error) {
        ...
        rows.push({
            ...
            observedMaximumPrelaunchCertificateTankCount: activationTelemetry.reduce(
                (maximum, event) => Math.max(maximum, event.schemaVersion === 22 ? event.assaultTankCount : 0), 0,
            ),
            observedMaximumPrelaunchCertificateScreenCount: activationTelemetry.reduce(
                (maximum, event) => Math.max(maximum, event.schemaVersion === 22 ? event.assaultScreenCount : 0), 0,
            ),
            ...
            blockedDelegatedEvaluationCount: activationTelemetry.filter((event) =>
                event.schemaVersion === 22 && event.phase === "blocked" && event.enemyBuildingCount > 1 &&
                !event.directObjectiveFeasible && !event.completeRouteFeasible &&
                !event.partialBlockerLaunchPermitted && event.activePredecessorCompatibleAttackerCount > 0,
            ).length,
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-focused-gate-v28-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-focused-gate-v28",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_SCHEMA_VERSION,
            policyId: missionNativeCloseoutPolicyV28Sha256(policy),
            countries: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_COUNTRIES,
            candidateSlot: 0,
        ...
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("Focused V28 gate provenance or coverage failed");
    ...
    if (!rows.some((row) =>
        Array.isArray(row.capabilityTelemetry) && row.capabilityTelemetry.length === 0 &&
        typeof row.blockedDelegatedEvaluationCount === "number" && row.blockedDelegatedEvaluationCount > 0,
    )) {
        coverageErrors.push("no preterminal route-infeasible row retained active predecessor delegation");
    }
    const passed = rows.every((row) => row.passed === true) && coverageErrors.length === 0;
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28",
    ...
    if (!passed) throw new Error("Focused V28 gate failed its outcome-blind technical contract");
};
