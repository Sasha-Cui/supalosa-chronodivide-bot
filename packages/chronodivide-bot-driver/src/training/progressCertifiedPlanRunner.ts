...
import {
    ...
    ProgressCertifiedEpisodeSpec,
    runProgressCertifiedEpisode,
} from "./progressCertifiedEpisode.js";
import {
...
export type ProgressCertifiedPlanEpisode = {
    episodeId: string;
    armId: string;
    policyId: string;
    candidateSlot: 0 | 1;
...
export type ProgressCertifiedRunPlan = {
    ...
    maxTicks: number;
    arms: ProgressCertifiedPlanArm[];
    episodes: ProgressCertifiedPlanEpisode[];
};
...
export const parseProgressCertifiedRunPlan = (value: unknown): ProgressCertifiedRunPlan => {
    if (!isRecord(value)) throw new Error("Progress-certified run plan must be an object");
    exactKeys(value, [
    ...
    ) throw new Error("Progress-certified plan endpoint binding drifted");
    if (!isRecord(value.family)) throw new Error("Progress-certified plan family is invalid");
    exactKeys(value.family, ["familyId", "mapName", "mapSha256"], "Progress-certified family");
    ...
    if (mapName !== mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(mapName)) {
        throw new Error("Progress-certified mapName must be a map basename");
    }
    const country = value.country;
    if (typeof country !== "string" || !Object.values(Countries).includes(country as Countries)) {
        throw new Error("Progress-certified plan country is invalid");
    }
    if (!Array.isArray(value.arms) || value.arms.length < 1) throw new Error("Progress-certified plan has no arms");
    const arms = value.arms.map((raw, index): ProgressCertifiedPlanArm => {
        if (!isRecord(raw)) throw new Error(`Progress-certified arm ${index} is invalid`);
        exactKeys(raw, ["armId", "policyId", "policy"], `Progress-certified arm ${index}`);
        ...
        const policy = validateProgressCertifiedExperimentPolicy(raw.policy as ProgressCertifiedExperimentPolicy);
        if (progressCertifiedExperimentPolicySha256(policy) !== policyId) {
            throw new Error(`Progress-certified arm ${armId} policy hash drifted`);
        }
    ...
    ) throw new Error("Progress-certified plan arm identities must be unique");
    if (!Array.isArray(value.episodes)) throw new Error("Progress-certified plan episodes are invalid");
    const armById = new Map(arms.map((arm) => [arm.armId, arm]));
    const episodes = value.episodes.map((raw, index): ProgressCertifiedPlanEpisode => {
        if (!isRecord(raw)) throw new Error(`Progress-certified episode ${index} is invalid`);
        exactKeys(raw, ["episodeId", "armId", "policyId", "candidateSlot"], `Progress-certified episode ${index}`);
        ...
        if ((raw.candidateSlot !== 0 && raw.candidateSlot !== 1) || armById.get(armId)?.policyId !== policyId) {
            throw new Error(`Progress-certified episode ${index} has an invalid arm or slot binding`);
        }
    ...
    ) throw new Error("Progress-certified plan episode count or identities are invalid");
    for (const arm of arms) {
        const slots = episodes.filter(({ armId }) => armId === arm.armId).map(({ candidateSlot }) => candidateSlot).sort();
        if (slots.join(",") !== "0,1") throw new Error(`Progress-certified arm ${arm.armId} lacks reciprocal slots`);
    }
    ...
    if (derivePairedEngineSeed(engineSeedBase, seedBlockIndex) !== requestedEngineSeed) {
        throw new Error("Progress-certified requested engine seed drifted from its paired derivation");
    }
    return {
...
export const serializeProgressCertifiedRunPlan = (plan: ProgressCertifiedRunPlan): string =>
    `${JSON.stringify(parseProgressCertifiedRunPlan(plan), null, 2)}\n`;
...
export const runProgressCertifiedPlanFromEnvironment = async (): Promise<void> => {
    ...
    if (path.resolve(process.cwd()) !== driverRoot) throw new Error(`Progress-certified runner must start in ${driverRoot}`);
    const planPath = requireEnvPath("PROGRESS_CERTIFIED_PLAN");
    const outDir = requireEnvPath("OUT_DIR");
    if (fs.existsSync(outDir)) throw new Error(`Refusing to reuse Progress-certified OUT_DIR ${outDir}`);
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Progress-certified execution requires the pinned external baseline environment");
    }
    ...
    const manifest = createExperimentManifest({
        ...
        effectiveConfig: {
            runner: "progressCertifiedPlanRunner-v2",
            planPath,
            planSha256,
    ...
    ) throw new Error("Progress-certified runtime, source, baseline, or scheduler provenance drifted");
    const runtimeMap = manifest.inputs.maps[0];
    if (!runtimeMap?.exists || runtimeMap.sha256 !== plan.family.mapSha256) {
        throw new Error("Progress-certified runtime map bytes drifted from the plan");
    }
    const policyFiles = [
    ...
    ) throw new Error("Progress-certified policy violates its declared complete-state or endpoint boundary");
    ...
    for (let launchIndex = 0; launchIndex < plan.episodes.length; launchIndex++) {
        ...
        const arm = armById.get(episode.armId) as ProgressCertifiedPlanArm;
        const spec: ProgressCertifiedEpisodeSpec = {
        ...
        try {
            const result = await runProgressCertifiedEpisode(spec, baselineFactory);
...
if (import.meta.url === invoked) {
    runProgressCertifiedPlanFromEnvironment().catch((error) => {
        console.error(error);
        process.exitCode = 1;
    });
}
