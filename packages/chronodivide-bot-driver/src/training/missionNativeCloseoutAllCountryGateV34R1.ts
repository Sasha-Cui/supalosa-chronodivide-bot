...
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_ENGINE_SEED_BASE = 4_294_850_000 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_RUNS_PER_COUNTRY_SLOT = 4 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_COUNTRIES = Object.freeze(
...
export type MissionNativeCloseoutAllCountryV34R1CoverageRow = {
    country: Countries;
    candidateSlot: 0 | 1;
    passed: boolean;
...
export const validateMissionNativeCloseoutV34R1ObjectiveRaceTelemetry = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
): void => {
    ...
    for (const allocation of allocations) {
        ...
        if (!audit) {
            throw new Error("V34-R1 objective-race telemetry did not cover a throttled live allocation");
        }
    }
    for (const event of objectiveRace) {
        if (event.allocationMode !== "boundedScreen") {
            throw new Error("V34-R1 objective-race allocation did not execute boundedScreen mode");
        }
        if (
            ...
            throw new Error("V34-R1 objective-race allocation partition or terminal state was inconsistent");
        }
        if (event.terminalPriorityActive && event.blockerId === null) {
            if (event.blockerAttackerCount !== 0 || event.buildingAttackerCount !== event.assignedAttackerCount) {
                throw new Error("V34-R1 feasible terminal strike did not receive the full compatible force");
            }
            ...
            if (
                ...
                throw new Error("V34-R1 blocker clearance consumed more than the bounded screen");
            }
        } else if (
            event.blockerAttackerCount !== 0 || event.buildingAttackerCount !== event.assignedAttackerCount
        ) {
            throw new Error("V34-R1 direct building strike did not retain the full compatible force");
        }
    }
};
...
export const validateMissionNativeCloseoutV34R1QuitAudits = (
...
const convertedAfterCompositionBlock = (row: MissionNativeCloseoutAllCountryV34R1CoverageRow): boolean =>
...
export const validateMissionNativeCloseoutAllCountryV34R1Coverage = (
    rows: readonly MissionNativeCloseoutAllCountryV34R1CoverageRow[],
): string[] => {
    const errors: string[] = [];
    if (rows.length !== MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_COUNTRIES.length * 2) {
        errors.push("all-country matrix does not contain exactly 18 country-slot cells");
    }
    for (const country of MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_COUNTRIES) {
    ...
    const compositionExposure = (row: MissionNativeCloseoutAllCountryV34R1CoverageRow): boolean =>
        row.preterminalCompositionBlockedEvaluationCount > 0;
    for (const [label, predicate] of [
        ["Allied", (row: MissionNativeCloseoutAllCountryV34R1CoverageRow) => alliedCountries.has(row.country)],
        ["Soviet", (row: MissionNativeCloseoutAllCountryV34R1CoverageRow) => !alliedCountries.has(row.country)],
        ["slot 0", (row: MissionNativeCloseoutAllCountryV34R1CoverageRow) => row.candidateSlot === 0],
        ["slot 1", (row: MissionNativeCloseoutAllCountryV34R1CoverageRow) => row.candidateSlot === 1],
    ] as const) {
    ...
    if (rows.some((row) => row.productionReservationEventCount !== 0)) {
        errors.push("V34-R1 emitted destructive production-reservation telemetry");
    }
    for (const [label, predicate, expectedName] of [
        ["Allied", (row: MissionNativeCloseoutAllCountryV34R1CoverageRow) => alliedCountries.has(row.country), "GAPILE"],
        ["Soviet", (row: MissionNativeCloseoutAllCountryV34R1CoverageRow) => !alliedCountries.has(row.country), "NAHAND"],
    ] as const) {
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("All-country V34-R1 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("All-country V34-R1 gate map bytes drifted");
    }
    ...
    const coverageRows: MissionNativeCloseoutAllCountryV34R1CoverageRow[] = [];
    ...
    for (const country of MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_COUNTRIES) {
        for (const candidateSlot of [0, 1] as const) {
            ...
            if (directDigest !== disabledDigest) validationErrors.push("disabled V34-R1 adapter drifted");
            if (disabled.telemetry.length !== 0) validationErrors.push("disabled V34-R1 adapter emitted telemetry");
            for (const [label, trace] of [["first", first], ["repeat", repeat]] as const) {
                ...
                try {
                    validateMissionNativeCloseoutV34R1ObjectiveRaceTelemetry(trace.telemetry);
                } catch (error) {
                    validationErrors.push(`${label}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            ...
            if (digest(first.actions) === digest(direct.actions)) {
                validationErrors.push("enabled V34-R1 policy did not change commands");
            }
            validationErrors.push(...validateMissionNativeCloseoutV34R1QuitAudits(
                direct.quitAttempts,
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-all-country-gate-v34-r1-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-all-country-gate-v34-r1",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_SCHEMA_VERSION,
            ...
            countries: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_COUNTRIES,
            reciprocalSlots: [0, 1],
            runsPerCountrySlot: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_RUNS_PER_COUNTRY_SLOT,
            maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
            outcomeInspection: false,
        },
        baseline: factory.descriptor,
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("All-country V34-R1 gate provenance or matrix coverage failed");
    const globalValidationErrors = validateMissionNativeCloseoutAllCountryV34R1Coverage(coverageRows);
    const passed = coverageRows.every((row) => row.passed) && globalValidationErrors.length === 0;
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1",
        ...
        disabledPolicyId: missionNativeCloseoutPolicyV34Sha256(disabledPolicy),
        enabledPolicyId: missionNativeCloseoutPolicyV34Sha256(enabledPolicy),
        countryCount: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_COUNTRIES.length,
        reciprocalSlotCount: 2,
        gameCount: rows.length * MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_R1_RUNS_PER_COUNTRY_SLOT,
        maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
    ...
    if (!passed) throw new Error("All-country V34-R1 gate failed; preserved outcome-free artifact");
};
