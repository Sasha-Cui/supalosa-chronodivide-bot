...
export const PROGRESS_CERTIFIED_EPISODE_SCHEMA_VERSION = 1 as const;

export type ProgressCertifiedEpisodeSpec = {
    schemaVersion: typeof PROGRESS_CERTIFIED_EPISODE_SCHEMA_VERSION;
    episodeId: string;
    familyId: string;
    mapName: string;
    mapSha256: string;
...
export type ProgressCertifiedWinner = "candidate" | "baseline" | "draw";
export type ProgressCertifiedOutcomeStatus =
    | "candidate_win"
    | "baseline_win"
    | "simultaneous_draw"
...
export type ProgressCertifiedEpisodeResult = {
    schemaVersion: typeof PROGRESS_CERTIFIED_EPISODE_SCHEMA_VERSION;
    episodeId: string;
    familyId: string;
    mapName: string;
    ...
    policyId: string;
    policySha256: string;
    candidateCore: ProgressCertifiedExperimentPolicy["candidateCore"];
    ...
    wallTimeMs: number;
    shortGame: false;
    outcomeStatus: ProgressCertifiedOutcomeStatus;
    winner: ProgressCertifiedWinner | null;
...
export const validateProgressCertifiedEpisodeSpec = (
    spec: ProgressCertifiedEpisodeSpec,
): ProgressCertifiedEpisodeSpec => {
    if (spec.schemaVersion !== PROGRESS_CERTIFIED_EPISODE_SCHEMA_VERSION) {
        throw new Error(`Progress-certified episode schemaVersion must be ${PROGRESS_CERTIFIED_EPISODE_SCHEMA_VERSION}`);
    }
    ...
    if (spec.mapName !== spec.mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(spec.mapName)) {
        throw new Error(`Progress-certified mapName must be a .map or .mpr basename; got ${spec.mapName}`);
    }
    if (!SHA256_PATTERN.test(spec.mapSha256)) throw new Error("Progress-certified mapSha256 must be lowercase SHA-256");
    const policy = validateProgressCertifiedExperimentPolicy(spec.policy);
    if (progressCertifiedExperimentPolicySha256(policy) !== spec.policyId) {
        throw new Error("Progress-certified policyId does not equal the canonical policy SHA-256");
    }
    if (!Number.isSafeInteger(spec.seedBlockIndex) || spec.seedBlockIndex < 0) {
        throw new Error("Progress-certified seedBlockIndex must be a nonnegative integer");
    }
    if (!Number.isSafeInteger(spec.requestedEngineSeed) || spec.requestedEngineSeed < 0 || spec.requestedEngineSeed > 0xffff_ffff) {
        throw new Error("Progress-certified requestedEngineSeed must be uint32");
    }
    if (spec.candidateSlot !== 0 && spec.candidateSlot !== 1) {
        throw new Error("Progress-certified candidateSlot must be 0 or 1");
    }
    if (!Object.values(Countries).includes(spec.candidateCountry)) {
        throw new Error("Progress-certified candidateCountry is invalid");
    }
    if (!Object.values(Countries).includes(spec.baselineCountry)) {
        throw new Error("Progress-certified baselineCountry is invalid");
    }
    if (!Number.isSafeInteger(spec.maxTicks) || spec.maxTicks < 1 || spec.maxTicks > 100_000) {
        throw new Error("Progress-certified maxTicks must be in [1, 100000]");
    }
    return { ...spec, policy };
};
...
const score = (winner: ProgressCertifiedWinner | null): 0 | 0.5 | 1 | null =>
    winner === "candidate" ? 1 : winner === "baseline" ? 0 : winner === "draw" ? 0.5 : null;

export const createProgressCertifiedExperimentCandidate = (
    factory: BaselineFactory,
    name: string,
    country: Countries,
    policy: ProgressCertifiedExperimentPolicy,
    telemetry: (event: TerminalObjectiveTelemetry) => void,
): InspectableBaselineBot => createTerminalObjectiveCandidate(
    factory,
    name,
    country,
...
export const runProgressCertifiedEpisode = async (
    rawSpec: ProgressCertifiedEpisodeSpec,
    baselineFactory: BaselineFactory,
): Promise<ProgressCertifiedEpisodeResult> => {
    const spec = validateProgressCertifiedEpisodeSpec(rawSpec);
    const startedAt = Date.now();
    const candidateName = `ProgressCandidate_${spec.seedBlockIndex}_${spec.candidateSlot}`;
    const baselineName = `ProgressBaseline_${spec.seedBlockIndex}_${spec.candidateSlot}`;
    ...
    return withSeededOfflineGame(
        ...
        async (game) => {
            ...
            const capDraw: LiteralEndpointCapDraw | null = !terminal && !technicalFailure && ticks >= spec.maxTicks
                ...
                : null;
            const outcomeStatus: ProgressCertifiedOutcomeStatus = technicalFailure
                ? "technical_failure"
                : terminal
                  ? terminal.status
                  : "tick_cap_draw";
            const winner: ProgressCertifiedWinner | null = technicalFailure
                ? null
                : terminal
                  ? terminal.winner
                  : "draw";
            ...
            return {
                ...
                policySha256: progressCertifiedExperimentPolicySha256(spec.policy),
