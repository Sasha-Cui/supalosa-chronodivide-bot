...
const KIND = "objective-aware-attack-replacement-complete-open-screen-v1" as const;
const STATUS = "FROZEN_OBJECTIVE_AWARE_REPLACEMENT_COMPLETE_OPEN_SCREEN_V1" as const;
const COUNTRIES = [
...
type ArmId = "external_supalosa_control" | "unchanged_v5" |
    "v5_plus_replacement_distance" | "v5_plus_replacement_forces_first" |
    "v5_plus_replacement_buildings_first";
type Arm = {
    armId: ArmId;
...
const commitmentFor = (arm: Omit<Arm, "policySha256">) => {
    ...
    const replacement = buildObjectiveAwareReplacementPolicy(arm.priority, arm.replacementEnabled);
    return {
        architecture: arm.kind === "control" ? "exact_external_supalosa" :
            "exact_external_supalosa_then_optional_objective_aware_attack_replacement_then_strict_v5",
        armId: arm.armId,
        v5: v5Policy,
        v5Sha256: progressCertifiedConversionPolicyV5Sha256(v5Policy),
        replacement,
        replacementSha256: objectiveAwareReplacementPolicySha256(replacement),
    };
};
const buildArms = (): Arm[] => ([
    ...
    { armId: "v5_plus_replacement_distance", kind: "replacement", priority: "distance", replacementEnabled: true },
    { armId: "v5_plus_replacement_forces_first", kind: "replacement", priority: "strategic", replacementEnabled: true },
    { armId: "v5_plus_replacement_buildings_first", kind: "replacement", priority: "objective", replacementEnabled: true },
...
const validateCampaign = (value: unknown): Campaign => {
    if (!isRecord(value) || value.schemaVersion !== 1 || value.kind !== KIND || value.status !== STATUS ||
        ...
        value.armCount !== 5 || value.shardCount !== 90 || value.launchedGameCount !== 900 ||
...
const createCandidate = (
    ...
    return createObjectiveAwareAttackReplacementCandidate(
        factory, name, country, buildProgressCertifiedConversionPolicyV5(),
        buildObjectiveAwareReplacementPolicy(arm.priority, arm.replacementEnabled),
        { v5: (event) => v5Telemetry.push(event), attackFactory: (event) => attackFactoryTelemetry.push(event) },
    );
};
...
const runEpisode = async (campaign: Campaign, shard: Shard, arm: Arm, slot: 0 | 1,
    ...
    const candidateName = `TargetCandidate_${shard.familyOrdinal}_${shard.countryOrdinal}_${slot}`;
    const baselineName = `TargetBaseline_${shard.familyOrdinal}_${shard.countryOrdinal}_${slot}`;
    const v5Telemetry: TerminalObjectiveTelemetry[] = [];
    const attackFactoryTelemetry: AttackMissionFactoryTelemetry[] = [];
    const candidate = createCandidate(factory, candidateName, shard.country, arm, v5Telemetry, attackFactoryTelemetry);
    ...
    return withSeededOfflineGame(cdapi, settings(shard.mapName, candidate, baseline, slot),
        ...
        async (game) => {
            ...
            return {
                ...
                v5Telemetry, attackFactoryTelemetry,
            };
        });
};
...
const generate = async (): Promise<void> => {
    ...
    const compatibilityMissionCounts = isRecord(compatibility) &&
        isRecord(compatibility.priorityMissionCounts) ? compatibility.priorityMissionCounts : null;
    if (!isRecord(compatibility) || compatibility.kind !==
        "objective-aware-attack-replacement-outcome-blind-gate" || compatibility.passed !== true ||
        compatibility.outcomeFree !== true || compatibility.schedulerAccount !== "pi_jss233" ||
        !/^\d+$/.test(String(compatibility.controllerJobId)) || !compatibilityMissionCounts ||
        !(["distance", "strategic", "objective"] as const).every((priority) =>
            Number(compatibilityMissionCounts[priority]) > 0)) {
        throw new Error("Compatibility gate did not pass every replacement priority");
    }
    ...
    const manifest = createExperimentManifest({ runId: "objective-aware-attack-replacement-open-screen-generator",
    ...
    const campaign: Campaign = {
        ...
        familyCount: 10, countryCount: 9, reciprocalSlotCount: 2, armCount: 5, shardCount: 90,
        launchedGameCount: 900, maxTicks: MAX_TICKS, seedBase: SEED_BASE, countries: COUNTRIES,
    ...
    console.log(JSON.stringify({ campaignSha256: fileHash(outFile), launchedGameCount: 900 }));
};
...
const cell = async (): Promise<void> => {
    ...
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const manifest = createExperimentManifest({ runId: `target-open-${taskIndex}-${process.env.SLURM_JOB_ID}`,
    ...
    append(eventsPath, { event: "run_start", campaignSha256, taskIndex, requestedLaunches: 10 });
    ...
    const summary = { schemaVersion: 1, status: technicalFailures === 0 ? "COMPLETE_OBJECTIVE_AWARE_REPLACEMENT_OPEN_SHARD" :
        "FAILED_OBJECTIVE_AWARE_REPLACEMENT_OPEN_SHARD", campaignSha256, taskIndex, requestedLaunches: 10,
        accountedLaunches: completed + technicalFailures, completed, technicalFailures,
        complete: completed + technicalFailures === 10, technicallyClean: technicalFailures === 0,
...
type Observation = {
    ...
    replacement: AttackMissionFactoryTelemetry[];
};
const validateResult = (value: unknown, campaign: Campaign, shard: Shard, arm: Arm, slot: 0 | 1): Observation => {
    if (!isRecord(value) || value.schemaVersion !== 1 || value.familyId !== shard.familyId ||
        ...
        !Array.isArray(value.v5Telemetry) || !Array.isArray(value.attackFactoryTelemetry)) {
    ...
    const replacement = value.attackFactoryTelemetry as AttackMissionFactoryTelemetry[];
    const v5 = value.v5Telemetry as TerminalObjectiveTelemetry[];
    if (arm.kind !== "replacement" && replacement.length !== 0) throw new Error("Comparator emitted replacement telemetry");
    ...
    const policy = buildObjectiveAwareReplacementPolicy(arm.priority, arm.replacementEnabled);
    ...
    for (const event of replacement) {
        if (event.schemaVersion !== 1 || event.event !== "attack_mission_created" ||
            event.informationBoundary !== "public_complete_state" ||
            event.targetPriority !== policy.targetPriority || !/^attack_\d+$/.test(event.missionName) ||
            missionNames.has(event.missionName) || !isRecord(event.composition) ||
            ...
            throw new Error("Replacement-factory telemetry validation failed");
        }
        missionNames.add(event.missionName);
    }
    return { armId: arm.armId, familyId: shard.familyId, country: shard.country, candidateSlot: slot,
        ...
        nonterminalDraw: winner === "draw" && ["tick_cap_draw", "engine_nonliteral_termination_draw"]
            .includes(String(value.outcomeStatus)), replacement };
};
...
const finalize = (): void => {
    ...
    for (const shard of campaign.shards) {
        ...
        if (!isRecord(summary) || summary.status !== "COMPLETE_OBJECTIVE_AWARE_REPLACEMENT_OPEN_SHARD" ||
            summary.campaignSha256 !== campaignSha256 || summary.taskIndex !== shard.taskIndex ||
            summary.requestedLaunches !== 10 || summary.accountedLaunches !== 10 || summary.completed !== 10 ||
    ...
    const evaluations = campaign.arms.filter(({ kind }) => kind === "replacement").map((arm) => {
        ...
        const missionCreations = armObservations.reduce((sum, row) => sum + row.replacement.length, 0);
        ...
        if (missionCreations === 0) eligibilityFailures.push("No replacement attack mission was created");
        return { arm, absolute, versusSupalosa, versusV5, missionCreations,
    ...
    const artifact = { schemaVersion: 1, kind: "objective-aware-attack-replacement-complete-open-screen-finalizer-v1",
        ...
        artifactRows: observations.map(({ replacement: _replacement, ...row }) => row) };
