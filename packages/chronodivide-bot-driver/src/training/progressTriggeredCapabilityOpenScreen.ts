...
const KIND = "progress-triggered-capability-complete-open-screen-v1" as const;
...
const commitmentFor = (arm: Omit<Arm, "policySha256">) => {
    ...
    return {
        architecture: arm.kind === "control" ? "exact_external_supalosa" :
            "exact_external_supalosa_then_optional_progress_triggered_capability_then_strict_v5",
        armId: arm.armId,
        v5: v5Policy,
...
const generate = async (): Promise<void> => {
    ...
    const completeExposure = exposure !== null && requiredVariants.every((id) => {
        ...
        return isRecord(row) && Number(row.replacement) > 0 && Number(row.missions) > 0 &&
            (id === "early_distance" || Number(row.capabilityRequests) > 0);
    });
    if (!isRecord(compatibility) || compatibility.kind !==
        "progress-triggered-capability-outcome-blind-gate" || compatibility.passed !== true ||
        ...
        throw new Error("Compatibility gate did not pass every capability variant");
    }
    ...
    const manifest = createExperimentManifest({ runId: "progress-triggered-capability-open-screen-generator",
...
const validateResult = (value: unknown, campaign: Campaign, shard: Shard, arm: Arm, slot: 0 | 1): Observation => {
    ...
    for (const event of capabilityEvents) if (event.schemaVersion !== 1 ||
        event.informationBoundary !== "public_complete_state" || event.country !== shard.country ||
...
const finalize = (): void => {
    ...
    const evaluations = campaign.arms.filter(({ kind }) => kind === "replacement").map((arm) => {
        ...
        if (arm.airTargetCount > 0 && capabilityReleases === 0) eligibilityFailures.push("No capability unit was released");
        return { arm, absolute, versusSupalosa, versusV5, replacementCount, missionCreations,
            capabilityRequests, capabilityReleases,
    ...
    const artifact = { schemaVersion: 1, kind: "progress-triggered-capability-complete-open-screen-finalizer-v1",
