...
import { buildTerminalRaceArms, terminalRacePolicySha256 } from "./terminalRacePolicy.js";

export const TERMINAL_RACE_SMOKE_MAX_TICKS = 2_400 as const;
export const TERMINAL_RACE_SMOKE_ENGINE_SEED_BASE = 4_145_000_000 as const;
...
const run = async (
    ...
    arm: ReturnType<typeof buildTerminalRaceArms>[number],
): Promise<Record<string, unknown>> => {
    if (!arm.policy.enabled) throw new Error("Terminal-race smoke requires an enabled arm");
    const smokePolicy = {
        ...arm.policy,
        activationMode: "fixed_tick" as const,
        minTick: 0,
    };
    ...
    await withSeededOfflineGame(
        ...
        async (game) => {
            for (let tick = 1; tick <= TERMINAL_RACE_SMOKE_MAX_TICKS; tick += 1) {
    ...
    if (!telemetry.some(({ event }) => event === "decision" || event === "search_orders")) {
        throw new Error(`Terminal-race bridge emitted no decision for ${country} slot ${candidateSlot}`);
    }
    ...
    return {
        ...
        armId: arm.armId,
        requestedEngineSeed,
        policyId: terminalRacePolicySha256(smokePolicy),
        maxTicks: TERMINAL_RACE_SMOKE_MAX_TICKS,
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Terminal-race all-country smoke requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("Terminal-race smoke map bytes drifted");
    }
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const enabledArms = buildTerminalRaceArms().filter(({ policy }) => policy.enabled);
    ...
    for (const arm of enabledArms) {
        for (const country of Object.values(Countries)) for (const candidateSlot of [0, 1] as const) {
            rows.push(await run(
                ...
                country,
                candidateSlot,
                derivePairedEngineSeed(TERMINAL_RACE_SMOKE_ENGINE_SEED_BASE, index++),
                arm,
            ));
        }
    }
    const manifest = createExperimentManifest({
        runId: `terminal-race-smoke-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-terminal-race-all-country-live-bridge-smoke",
            enabledArms: enabledArms.map(({ armId, policyId }) => ({ armId, policyId })),
        ...
        gameSeedBase: TERMINAL_RACE_SMOKE_ENGINE_SEED_BASE,
    });
    if (
        ...
        manifest.software.baseline.trackedDirty !== false || rows.length !== enabledArms.length * 18
    ) throw new Error("Terminal-race all-country smoke provenance or coverage failed");
    const output = {
        schemaVersion: 1,
        status: "PASS_OUTCOME_FREE_TERMINAL_RACE_ALL_COUNTRY_LIVE_BRIDGE_SMOKE",
        ...
        enabledArmCount: enabledArms.length,
