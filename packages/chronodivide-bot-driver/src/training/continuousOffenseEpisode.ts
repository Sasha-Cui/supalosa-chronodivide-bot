...
import { createContinuousOffenseChampionCandidate } from "./continuousOffenseCandidate.js";
import {
...
export const CONTINUOUS_OFFENSE_EPISODE_SCHEMA_VERSION = 1 as const;

export type ContinuousOffenseEpisodeSpec = {
    schemaVersion: typeof CONTINUOUS_OFFENSE_EPISODE_SCHEMA_VERSION;
    episodeId: string;
    familyId: string;
    mapName: string;
    mapSha256: string;
...
export type ContinuousOffenseWinner = "candidate" | "baseline" | "draw";
export type ContinuousOffenseOutcomeStatus =
    | "candidate_win"
    | "baseline_win"
    | "simultaneous_draw"
...
export type ContinuousOffenseEpisodeResult = {
    schemaVersion: typeof CONTINUOUS_OFFENSE_EPISODE_SCHEMA_VERSION;
    episodeId: string;
    familyId: string;
    mapName: string;
    ...
    policyId: string;
    policySha256: string;
    candidateCore: ContinuousOffenseExperimentPolicy["candidateCore"];
    macroPolicyId: ContinuousOffenseExperimentPolicy["macroPolicyId"];
    ...
    wallTimeMs: number;
    shortGame: false;
    outcomeStatus: ContinuousOffenseOutcomeStatus;
    winner: ContinuousOffenseWinner | null;
...
export const validateContinuousOffenseEpisodeSpec = (
    spec: ContinuousOffenseEpisodeSpec,
): ContinuousOffenseEpisodeSpec => {
    if (spec.schemaVersion !== CONTINUOUS_OFFENSE_EPISODE_SCHEMA_VERSION) {
        throw new Error(`Continuous-offense episode schemaVersion must be ${CONTINUOUS_OFFENSE_EPISODE_SCHEMA_VERSION}`);
    }
    ...
    if (spec.mapName !== spec.mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(spec.mapName)) {
        throw new Error(`Continuous-offense mapName must be a .map or .mpr basename; got ${spec.mapName}`);
    }
    if (!SHA256_PATTERN.test(spec.mapSha256)) throw new Error("Continuous-offense mapSha256 must be lowercase SHA-256");
    const policy = validateContinuousOffenseExperimentPolicy(spec.policy);
    if (continuousOffenseExperimentPolicySha256(policy) !== spec.policyId) {
        throw new Error("Continuous-offense policyId does not equal the canonical policy SHA-256");
    }
    if (!Number.isSafeInteger(spec.seedBlockIndex) || spec.seedBlockIndex < 0) {
        throw new Error("Continuous-offense seedBlockIndex must be a nonnegative integer");
    }
    if (!Number.isSafeInteger(spec.requestedEngineSeed) || spec.requestedEngineSeed < 0 || spec.requestedEngineSeed > 0xffff_ffff) {
        throw new Error("Continuous-offense requestedEngineSeed must be uint32");
    }
    if (spec.candidateSlot !== 0 && spec.candidateSlot !== 1) {
        throw new Error("Continuous-offense candidateSlot must be 0 or 1");
    }
    if (!Object.values(Countries).includes(spec.candidateCountry)) {
        throw new Error("Continuous-offense candidateCountry is invalid");
    }
    if (!Object.values(Countries).includes(spec.baselineCountry)) {
        throw new Error("Continuous-offense baselineCountry is invalid");
    }
    if (!Number.isSafeInteger(spec.maxTicks) || spec.maxTicks < 1 || spec.maxTicks > 100_000) {
        throw new Error("Continuous-offense maxTicks must be in [1, 100000]");
    }
    return { ...spec, policy };
};
...
const score = (winner: ContinuousOffenseWinner | null): 0 | 0.5 | 1 | null =>
    winner === "candidate" ? 1 : winner === "baseline" ? 0 : winner === "draw" ? 0.5 : null;

export const createContinuousOffenseExperimentCandidate = (
    factory: BaselineFactory,
    name: string,
    country: Countries,
    policy: ContinuousOffenseExperimentPolicy,
    telemetry: (event: TerminalObjectiveTelemetry) => void,
): InspectableBaselineBot => policy.candidateCore === "external_supalosa"
    ? factory.create(name, country)
    : createContinuousOffenseChampionCandidate(name, country, policy.objectivePolicy, telemetry);

/** Run one predeclared game with no retry, outcome filter, or endpoint substitution. */
export const runContinuousOffenseEpisode = async (
    rawSpec: ContinuousOffenseEpisodeSpec,
    baselineFactory: BaselineFactory,
): Promise<ContinuousOffenseEpisodeResult> => {
    const spec = validateContinuousOffenseEpisodeSpec(rawSpec);
    const startedAt = Date.now();
    const candidateName = `ContinuousCandidate_${spec.seedBlockIndex}_${spec.candidateSlot}`;
    const baselineName = `ContinuousBaseline_${spec.seedBlockIndex}_${spec.candidateSlot}`;
    const policyTelemetry: TerminalObjectiveTelemetry[] = [];
    const candidate = createContinuousOffenseExperimentCandidate(
        baselineFactory,
        candidateName,
    ...
    return withSeededOfflineGame(
        ...
        async (game) => {
            ...
            const capDraw: LiteralEndpointCapDraw | null = !terminal && !technicalFailure && ticks >= spec.maxTicks
                ...
                : null;
            const outcomeStatus: ContinuousOffenseOutcomeStatus = technicalFailure
                ? "technical_failure"
                : terminal
                  ? terminal.status
                  : "tick_cap_draw";
            const winner: ContinuousOffenseWinner | null = technicalFailure
                ? null
                : terminal
                  ? terminal.winner
                  : "draw";
            ...
            return {
                ...
                policySha256: continuousOffenseExperimentPolicySha256(spec.policy),
                candidateCore: spec.policy.candidateCore,
                macroPolicyId: spec.policy.macroPolicyId,
                policyInformationBoundary: spec.policy.objectivePolicy.enabled
                    ? spec.policy.objectivePolicy.informationInterface
                    : "none",
                endpointVersion: LITERAL_BUILDING_ELIMINATION_ENDPOINT_VERSION,
                endpointSha256: LITERAL_BUILDING_ELIMINATION_ENDPOINT_SHA256,
