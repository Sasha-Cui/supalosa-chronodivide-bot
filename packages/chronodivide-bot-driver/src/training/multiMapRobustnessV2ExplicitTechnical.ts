...
export const MULTIMAP_V2_MAPS: readonly MultiMapV2Spec[] = [
    ...
    startCount, family, seedBase: 3_002_000_000 + index * 100_000 })) as readonly MultiMapV2Spec[];
...
export type SelectedCase = { mapCaseIndex: number; countryOrdinal: number; country: Countries; candidateSlot: 0 | 1;
...
export const buildMultiMapV2ExplicitCases = (spec: MultiMapV2Spec, starts: string[]): SelectedCase[] => {
    if (starts.length !== spec.startCount || new Set(starts).size !== starts.length) throw new Error("Explicit start metadata invalid");
    ...
    for (const [countryOrdinal, country] of MULTIMAP_V2_COUNTRIES.entries()) for (const candidateSlot of [0, 1] as const)
        for (let a = 0; a < starts.length; a += 1) for (let b = 0; b < starts.length; b += 1) if (a !== b)
            for (let repeatIndex = 0; repeatIndex < (starts.length === 2 ? 5 : 1); repeatIndex += 1) {
                const mapCaseIndex = cases.length;
                cases.push({ mapCaseIndex, countryOrdinal, country, candidateSlot, candidateStartOrdinal: a,
                    opponentStartOrdinal: b, candidateStart: starts[a], opponentStart: starts[b], repeatIndex,
                    requestedEngineSeed: spec.seedBase + mapCaseIndex, seedOffset: mapCaseIndex,
                    stage1Screen: multiMapV2IsScreenCase(spec, countryOrdinal, a, b, repeatIndex) });
            }
    return cases;
};
export const multiMapV2ExplicitCoverage = (spec: MultiMapV2Spec, cases: SelectedCase[]) => {
    const screen = cases.filter((row) => row.stage1Screen);
    const count = (rows: SelectedCase[], key: (row: SelectedCase) => string) => {
        const result: Record<string, number> = {};
        for (const row of rows) { const label = key(row); result[label] = (result[label] ?? 0) + 1; }
        return result;
    };
    const byCountryStartSlot = count(screen, (row) => [row.countryOrdinal, row.candidateStartOrdinal, row.candidateSlot].join(":")),
        byOpponentStart = count(screen, (row) => String(row.opponentStartOrdinal)),
        byCyclicOffset = count(screen, (row) => String((row.opponentStartOrdinal - row.candidateStartOrdinal + spec.startCount) % spec.startCount)),
        expectedFull = 9 * 2 * spec.startCount * (spec.startCount - 1),
        full = count(cases, (row) => [row.countryOrdinal,row.candidateSlot,row.candidateStartOrdinal,row.opponentStartOrdinal].join(":"));
    if (Object.keys(full).length !== expectedFull || Object.values(full).some((n) => n !== (spec.startCount === 2 ? 5 : 1)) ||
        Object.keys(byCountryStartSlot).length !== multiMapV2ScreenCaseCount(spec) ||
        Object.values(byCountryStartSlot).some((n) => n !== 1) ||
        Object.keys(byOpponentStart).length !== spec.startCount || Object.values(byOpponentStart).some((n) => n !== 18) ||
        Object.keys(byCyclicOffset).length !== spec.startCount - 1) throw new Error("Explicit case coverage mismatch");
    return { completeDirectedPairCoverage: true, originalScreenMembershipPreserved: true, byCountryStartSlot, byOpponentStart,
        byCyclicOffset, cyclicOffsetCountsUniform: new Set(Object.values(byCyclicOffset)).size === 1,
        screenAuthorized: false, screenHoldReason: "Opponent-class balance requires an explicit outcome-blind decision before gameplay" };
};
...
const commonInputs = () => { if (process.env.SLURM_JOB_ACCOUNT !== "pi_jss233") throw new Error("Multi-map V2 requires pi_jss233");
    ...
    const censusAmendmentPath = requiredPath("CENSUS_AMENDMENT_PATH"),
        censusAmendmentSha256 = requiredText("CENSUS_AMENDMENT_SHA256", SHA256),
        collisionAuditPath = requiredPath("COLLISION_AUDIT_PATH"),
        collisionAuditSha256 = requiredText("COLLISION_AUDIT_SHA256", SHA256),
        compatibilityPath = requiredPath("COMPATIBILITY_PATH"), compatibilitySha256 = hashFile(compatibilityPath),
        collision = JSON.parse(fs.readFileSync(collisionAuditPath, "utf8")),
        compatibility = JSON.parse(fs.readFileSync(compatibilityPath, "utf8"));
    if (hashFile(censusAmendmentPath) !== censusAmendmentSha256 || hashFile(collisionAuditPath) !== collisionAuditSha256 ||
        collision.kind !== "multimap-v2-seed-reservation-audit" || collision.complete !== true || collision.passed !== true ||
        collision.schedulerAccount !== "pi_jss233" || !/^[0-9a-f]{40}$/.test(collision.sourceCommit) ||
        String(collision.schedulerJobId) !== requiredText("COLLISION_AUDIT_JOB_ID", /^\d+$/) ||
        collision.censusAmendmentSha256 !== censusAmendmentSha256 || collision.collisions.length !== 0 || collision.errors.length !== 0 ||
        collision.scannedFileCount < 1 || JSON.stringify(collision.reservedInterval) !== "[3002000000,3003300000]" ||
        compatibilitySha256 !== "51ab624f2e8aa7a094bff51d1fe675298c1eb30b8c72672dfefb041df3cdd752" ||
        compatibility.complete !== true || compatibility.passed !== true || compatibility.initializedGameCount !== 1264)
        throw new Error("Explicit census prerequisite mismatch");
    const originalRuntimeSha256 = "dd398f5c8c2b4c3e3d6eb0f9ca6d7549bf70fee16c5950cd8902616ac922497d",
        effectiveRuntimeSha256 = "4ad4a5dd7a6a8ae53a7e671a29d7dd0a5fbad1916d94e52c69c9eda133a30f0c",
        method = (globalThis as any)[Symbol.for("chrono.research.explicit-start.v1")],
        repo = sourceIdentity().repo;
    if (method?.originalSha256 !== originalRuntimeSha256 ||
        compatibility.effectiveRuntimeSha256 !== effectiveRuntimeSha256 ||
        hashFile(requiredPath("CHRONO_GAME_API_PATH")) !== originalRuntimeSha256 ||
        hashFile(path.join(repo,"research/runtime/explicit-start-loader-v1.mjs")) !== "9cd22887ab3ae3206b0c6cb8d91a15593dd2a2dd073ad36d9b5b9827dac46cac" ||
        hashFile(path.join(repo,"research/runtime/explicit-start-transform-v1.mjs")) !== "d36371064697371a53ecece243ddc086db1b481d210d84c549dcd5d0eb9ea32a")
        throw new Error("Explicit census effective runtime mismatch");
    return { mixDir, protocolSha256, inventorySha256, assetManifestSha256, censusAmendmentSha256, collisionAuditSha256,
        compatibilitySha256, interfaceIdentity: { method: method.method, originalRuntimeSha256, effectiveRuntimeSha256 } }; };
...
const zeroUpdate = async (spec: MultiMapV2Spec, baseline: BaselineFactory, country: Countries, slot: 0 | 1, seed: number,
    suffix: string, positions: [number, number]) => { const candidateName = `MMSelectCandidate_${spec.id}_${suffix}`, opponentName = `MMSelectOpponent_${spec.id}_${suffix}`,
        candidate = new StrongBot(candidateName, country, [], false), opponent = baseline.create(opponentName, country);
    (candidate as any).chronoResearchStartPos = positions[0]; (opponent as any).chronoResearchStartPos = positions[1];
    return withSeededOfflineGame(cdapi, settings(spec, candidate, opponent, slot), seed,
        [{ agent: candidate, identity: "candidate" }, { agent: opponent, identity: "opponent" }], async () => { const api = candidate.lastGameApi;
            if (!api || api.getCurrentTick() !== 0) throw new Error("Multi-map zero-update API/tick mismatch"); return { candidateStart: point(api.getPlayerData(candidateName).startLocation),
...
const oneUpdate = async (spec: MultiMapV2Spec, baseline: BaselineFactory, seed: number) => { const candidateName = `MMSmokeCandidate_${spec.id}`,
    ...
    (candidate as any).chronoResearchStartPos = 0; (opponent as any).chronoResearchStartPos = 1;
    return withSeededOfflineGame(cdapi, settings(spec, candidate, opponent, 0), seed,
        [{ agent: candidate, identity: "candidate" }, { agent: opponent, identity: "opponent" }], async (game) => { const api = candidate.lastGameApi;
            if (!api || api.getCurrentTick() !== 0) throw new Error("Multi-map smoke API/tick mismatch"); const before = stateSnapshot(api, candidateName, opponentName), mapCensus = census(api);
            await game.update(); const after = stateSnapshot(api, candidateName, opponentName);
            if (api.getCurrentTick() !== 1) throw new Error("Multi-map smoke exceeded one update");
...
const runCell = async () => { const taskIndex = Number(requiredText("TASK_INDEX", /^\d+$/)), out = requiredPath("OUT_PATH"),
    ...
    const mapText = fs.readFileSync(path.join(inputs.mixDir, spec.fileName), "utf8"),
        section = /\[Waypoints\]\s*([\s\S]*?)(?=\n\[|$)/i.exec(mapText)?.[1];
    if (!section) throw new Error("Explicit map waypoints missing");
    const rows = [...section.matchAll(/^(\d+)\s*=\s*(\d+)\s*$/gm)].filter((row) => +row[1] < 8)
        .sort((a,b) => +a[1] - +b[1]);
    if (rows.length !== spec.startCount || rows.some((row,index) => +row[1] !== index)) throw new Error("Explicit waypoint indices invalid");
    const canonicalStarts = rows.map((row) => String(+row[2] % 1000) + "," + String(Math.floor(+row[2] / 1000))),
        cases = buildMultiMapV2ExplicitCases(spec,canonicalStarts);
    let initializedGameCount = 0;
    for (const row of cases) {
        const observed = await zeroUpdate(spec,baseline,row.country,row.candidateSlot,row.requestedEngineSeed,
            String(row.mapCaseIndex),[row.candidateStartOrdinal,row.opponentStartOrdinal]);
        initializedGameCount += 1;
        if (JSON.stringify(observed.starts) !== JSON.stringify(canonicalStarts) || observed.candidateStart !== row.candidateStart ||
            observed.opponentStart !== row.opponentStart) throw new Error("Explicit live start coordinates mismatch");
    }
    const coverage = multiMapV2ExplicitCoverage(spec,cases);
    if (cases.length !== multiMapV2SelectedCaseCount(spec) || cases.filter((row) => row.stage1Screen).length !==
        ...
        throw new Error("Explicit census case count mismatch");
    ...
    const artifact = { schemaVersion: 1, kind: "multimap-v2-explicit-technical-selection-cell", status: "PASS_MULTIMAP_V2_EXPLICIT_TECHNICAL_CELL",
        ...
        screenCaseCount: cases.filter((row) => row.stage1Screen).length, cases, coverage, deterministicSmoke: smokeA, provenance,
        zeroUpdateInitializations: initializedGameCount, oneUpdateSmokeInitializations: 2, screenAuthorized: false,
        censusAmendmentSha256: inputs.censusAmendmentSha256, collisionAuditSha256: inputs.collisionAuditSha256,
        compatibilitySha256: inputs.compatibilitySha256, interfaceIdentity: inputs.interfaceIdentity };
...
const finalize = () => { const root = requiredPath("RESULTS_ROOT"), out = requiredPath("OUT_PATH"),
    ...
    if (fs.existsSync(out)) throw new Error("Multi-map aggregate exists"); const { commit } = sourceIdentity();
    let tasks = completedTasks(arrayJobId);
    for (let attempt = 0; tasks.size !== MULTIMAP_V2_TASK_COUNT && attempt < 10; attempt += 1) {
        execFileSync("sleep", ["2"]); tasks = completedTasks(arrayJobId);
    }
    ...
    for (let taskIndex = 0; taskIndex < MULTIMAP_V2_TASK_COUNT; taskIndex += 1) { const spec = MULTIMAP_V2_MAPS[taskIndex],
        ...
        if (fs.readFileSync(path.join(taskRoot,"COMPLETE"),"utf8").trim() !== "COMPLETE_MULTIMAP_V2_EXPLICIT_TECHNICAL_CELL")
            throw new Error("Explicit completion marker mismatch");
        if (hashFile(file) !== expected) throw new Error(`Multi-map checksum ${taskIndex}`); const cell = JSON.parse(fs.readFileSync(file, "utf8"));
        if (cell.kind !== "multimap-v2-explicit-technical-selection-cell" || cell.complete !== true || cell.passed !== true ||
            ...
            cell.censusAmendmentSha256 !== inputs.censusAmendmentSha256 || cell.collisionAuditSha256 !== inputs.collisionAuditSha256 ||
            cell.compatibilitySha256 !== inputs.compatibilitySha256 || JSON.stringify(cell.interfaceIdentity) !== JSON.stringify(inputs.interfaceIdentity) ||
            cell.zeroUpdateInitializations !== cell.selectedCaseCount || cell.oneUpdateSmokeInitializations !== 2 ||
            cell.selectedCaseCount !== multiMapV2SelectedCaseCount(spec) || cell.screenCaseCount !== multiMapV2ScreenCaseCount(spec))
            throw new Error(`Multi-map identity ${taskIndex}`);
        const rebuilt = buildMultiMapV2ExplicitCases(spec,cell.deterministicSmoke.mapCensus.starts);
        if (JSON.stringify(rebuilt) !== JSON.stringify(cell.cases) ||
            JSON.stringify(multiMapV2ExplicitCoverage(spec,cell.cases)) !== JSON.stringify(cell.coverage))
            throw new Error("Explicit selected population mismatch");
        assertOutcomeFree(cell); maps.push(cell); }
    ...
    const artifact = { schemaVersion: 1, kind: "multimap-v2-explicit-technical-census-selection",
        status: "PASS_MULTIMAP_V2_EXPLICIT_TECHNICAL_CENSUS_SELECTION", complete: true, passed: true, outcomeFree: true,
        ...
        censusAmendmentSha256: inputs.censusAmendmentSha256, collisionAuditSha256: inputs.collisionAuditSha256,
        compatibilitySha256: inputs.compatibilitySha256, interfaceIdentity: inputs.interfaceIdentity,
        zeroUpdateInitializations: maps.reduce((n,row) => n + row.zeroUpdateInitializations,0),
        oneUpdateSmokeInitializations: maps.reduce((n,row) => n + row.oneUpdateSmokeInitializations,0),
        ...
        maps: maps.map((row) => ({ map: row.map, availableGameModes: row.availableGameModes,
            ...
            screenCaseCount: row.screenCaseCount, cases: row.cases, coverage: row.coverage,
            deterministicSmoke: row.deterministicSmoke })) };
    assertOutcomeFree(artifact); fs.mkdirSync(path.dirname(out), { recursive: true, mode: 0o700 });
    fs.writeFileSync(out, JSON.stringify(artifact, null, 2) + "\n", { flag: "wx", mode: 0o600 });
