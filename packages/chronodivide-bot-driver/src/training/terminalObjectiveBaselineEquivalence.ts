...
import { buildTerminalObjectiveArms } from "./terminalObjectivePolicy.js";

export const TERMINAL_OBJECTIVE_EQUIVALENCE_MAX_TICKS = 1_200 as const;
export const TERMINAL_OBJECTIVE_EQUIVALENCE_ENGINE_SEED_BASE = 3_740_000_000 as const;
...
const run = async (
    ...
    const control = buildTerminalObjectiveArms().find(({ armId }) => armId === "selected_prior");
    if (!control) throw new Error("Terminal-objective selected_prior is unavailable");
    ...
    const candidate = wrapped
        ? createTerminalObjectiveCandidate(factory, candidateName, country, control.policy, () => {
            throw new Error("Disabled terminal-objective control emitted telemetry");
        })
    ...
    await withSeededOfflineGame(
        ...
        async (game) => {
            for (let tick = 1; tick <= TERMINAL_OBJECTIVE_EQUIVALENCE_MAX_TICKS; tick += 1) {
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Terminal-objective equivalence requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("Terminal-objective equivalence map bytes drifted");
    }
    ...
    for (const country of Object.values(Countries)) for (const candidateSlot of [0, 1] as const) {
        const requestedEngineSeed = derivePairedEngineSeed(
            TERMINAL_OBJECTIVE_EQUIVALENCE_ENGINE_SEED_BASE,
            index++,
        );
        ...
        if (!equal) throw new Error(`Disabled terminal-objective layer drifted for ${country} slot ${candidateSlot}`);
        rows.push({ country, candidateSlot, requestedEngineSeed, wrapped, unwrapped, equal });
    }
    const manifest = createExperimentManifest({
        runId: `terminal-objective-equivalence-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-terminal-objective-external-baseline-identity",
            countries: Object.values(Countries),
            reciprocalSlots: [0, 1],
            maxTicks: TERMINAL_OBJECTIVE_EQUIVALENCE_MAX_TICKS,
            outcomeInspection: false,
        },
        baseline: factory.descriptor,
        gameSeedBase: TERMINAL_OBJECTIVE_EQUIVALENCE_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("Terminal-objective equivalence provenance or coverage failed");
    const output = {
        schemaVersion: 1,
        status: "PASS_OUTCOME_FREE_TERMINAL_OBJECTIVE_EXTERNAL_BASELINE_IDENTITY",
        ...
        maxTicks: TERMINAL_OBJECTIVE_EQUIVALENCE_MAX_TICKS,
        implementationInvariant: "disabled terminal-objective candidate returns baselineFactory.create exactly",
        rows,
    };
