...
import {
    ...
    ContinuousOffenseEpisodeSpec,
    runContinuousOffenseEpisode,
} from "./continuousOffenseEpisode.js";
import {
...
export const CONTINUOUS_OFFENSE_PLAN_SCHEMA_VERSION = 2 as const;
export const CONTINUOUS_OFFENSE_PLAN_KIND = "continuous-offense-open-development-literal-endpoint" as const;

export type ContinuousOffensePlanArm = {
    armId: string;
    policyId: string;
    policy: ContinuousOffenseExperimentPolicy;
};

export type ContinuousOffensePlanEpisode = {
    episodeId: string;
    armId: string;
    policyId: string;
    candidateSlot: 0 | 1;
...
export type ContinuousOffenseRunPlan = {
    schemaVersion: typeof CONTINUOUS_OFFENSE_PLAN_SCHEMA_VERSION;
    kind: typeof CONTINUOUS_OFFENSE_PLAN_KIND;
    runId: string;
    ...
    maxTicks: number;
    arms: ContinuousOffensePlanArm[];
    episodes: ContinuousOffensePlanEpisode[];
};
...
export const parseContinuousOffenseRunPlan = (value: unknown): ContinuousOffenseRunPlan => {
    if (!isRecord(value)) throw new Error("Continuous-offense run plan must be an object");
    exactKeys(value, [
    ...
    ], "Continuous-offense run plan");
    if (value.schemaVersion !== CONTINUOUS_OFFENSE_PLAN_SCHEMA_VERSION || value.kind !== CONTINUOUS_OFFENSE_PLAN_KIND) {
        throw new Error("Continuous-offense plan identity is invalid");
    }
    if (
    ...
    ) throw new Error("Continuous-offense plan endpoint binding drifted");
    if (!isRecord(value.family)) throw new Error("Continuous-offense plan family is invalid");
    exactKeys(value.family, ["familyId", "mapName", "mapSha256"], "Continuous-offense family");
    ...
    if (mapName !== mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(mapName)) {
        throw new Error("Continuous-offense mapName must be a map basename");
    }
    const country = value.country;
    if (typeof country !== "string" || !Object.values(Countries).includes(country as Countries)) {
        throw new Error("Continuous-offense plan country is invalid");
    }
    if (!Array.isArray(value.arms) || value.arms.length < 1) throw new Error("Continuous-offense plan has no arms");
    const arms = value.arms.map((raw, index): ContinuousOffensePlanArm => {
        if (!isRecord(raw)) throw new Error(`Continuous-offense arm ${index} is invalid`);
        exactKeys(raw, ["armId", "policyId", "policy"], `Continuous-offense arm ${index}`);
        ...
        const policy = validateContinuousOffenseExperimentPolicy(raw.policy as ContinuousOffenseExperimentPolicy);
        if (continuousOffenseExperimentPolicySha256(policy) !== policyId) {
            throw new Error(`Continuous-offense arm ${armId} policy hash drifted`);
        }
    ...
    ) throw new Error("Continuous-offense plan arm identities must be unique");
    if (!Array.isArray(value.episodes)) throw new Error("Continuous-offense plan episodes are invalid");
    const armById = new Map(arms.map((arm) => [arm.armId, arm]));
    const episodes = value.episodes.map((raw, index): ContinuousOffensePlanEpisode => {
        if (!isRecord(raw)) throw new Error(`Continuous-offense episode ${index} is invalid`);
        exactKeys(raw, ["episodeId", "armId", "policyId", "candidateSlot"], `Continuous-offense episode ${index}`);
        ...
        if ((raw.candidateSlot !== 0 && raw.candidateSlot !== 1) || armById.get(armId)?.policyId !== policyId) {
            throw new Error(`Continuous-offense episode ${index} has an invalid arm or slot binding`);
        }
    ...
    ) throw new Error("Continuous-offense plan episode count or identities are invalid");
    for (const arm of arms) {
        const slots = episodes.filter(({ armId }) => armId === arm.armId).map(({ candidateSlot }) => candidateSlot).sort();
        if (slots.join(",") !== "0,1") throw new Error(`Continuous-offense arm ${arm.armId} lacks reciprocal slots`);
    }
    ...
    if (derivePairedEngineSeed(engineSeedBase, seedBlockIndex) !== requestedEngineSeed) {
        throw new Error("Continuous-offense requested engine seed drifted from its paired derivation");
    }
    return {
...
export const serializeContinuousOffenseRunPlan = (plan: ContinuousOffenseRunPlan): string =>
    `${JSON.stringify(parseContinuousOffenseRunPlan(plan), null, 2)}\n`;
...
export const runContinuousOffensePlanFromEnvironment = async (): Promise<void> => {
    const repoRoot = gitRoot();
    const driverRoot = path.join(repoRoot, "packages", "chronodivide-bot-driver");
    if (path.resolve(process.cwd()) !== driverRoot) throw new Error(`Continuous-offense runner must start in ${driverRoot}`);
    const planPath = requireEnvPath("CONTINUOUS_OFFENSE_PLAN");
    const outDir = requireEnvPath("OUT_DIR");
    if (fs.existsSync(outDir)) throw new Error(`Refusing to reuse Continuous-offense OUT_DIR ${outDir}`);
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Continuous-offense execution requires the pinned external baseline environment");
    }
    const planSha256 = sha256File(planPath);
    const plan = parseContinuousOffenseRunPlan(JSON.parse(fs.readFileSync(planPath, "utf8")));
    ...
    ) throw new Error("Continuous-offense runtime, source, baseline, or scheduler provenance drifted");
    const runtimeMap = manifest.inputs.maps[0];
    if (!runtimeMap?.exists || runtimeMap.sha256 !== plan.family.mapSha256) {
        throw new Error("Continuous-offense runtime map bytes drifted from the plan");
    }
    const policyFiles = [
    ...
    const publicStateSources = policyFiles.filter((filePath) => filePath.includes("terminalRacePublicState"));
    const prohibitedPolicySources = policyFiles.filter((filePath) => !filePath.includes("terminalRacePublicState"));
    if (
        prohibitedPolicySources.some((filePath) => /\bgetAllUnits\s*\(/.test(fs.readFileSync(filePath, "utf8"))) ||
        publicStateSources.some((filePath) => {
            const source = fs.readFileSync(filePath, "utf8");
            return /LiteralBuildingElimination|Disposition|ObjectDestroy|winner|outcome/i.test(source);
        })
    ) throw new Error("Continuous-offense policy violates its declared complete-state or endpoint boundary");
    ...
    for (let launchIndex = 0; launchIndex < plan.episodes.length; launchIndex++) {
        ...
        const arm = armById.get(episode.armId) as ContinuousOffensePlanArm;
        const spec: ContinuousOffenseEpisodeSpec = {
        ...
        try {
            const result = await runContinuousOffenseEpisode(spec, baselineFactory);
    ...
    const summary = {
        schemaVersion: 2,
        status: technicalFailures === 0 ? "COMPLETE_CONTINUOUS_OFFENSE_OPEN_DEVELOPMENT_SHARD" : "FAILED_CONTINUOUS_OFFENSE_TECHNICAL_SHARD",
...
if (import.meta.url === invoked) {
    runContinuousOffensePlanFromEnvironment().catch((error) => {
        console.error(error);
        process.exitCode = 1;
    });
}
