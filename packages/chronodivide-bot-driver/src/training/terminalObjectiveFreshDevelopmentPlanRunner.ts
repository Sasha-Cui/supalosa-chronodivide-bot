...
export const TERMINAL_FRESH_DEVELOPMENT_PLAN_SCHEMA_VERSION = 2 as const;
export const TERMINAL_FRESH_DEVELOPMENT_PLAN_KIND = "terminal-objective-fresh-development-literal-endpoint" as const;

export type TerminalFreshDevelopmentPlanArm = {
    armId: string;
    policyId: string;
    policy: TerminalObjectivePolicy;
};

export type TerminalFreshDevelopmentPlanEpisode = {
    episodeId: string;
    armId: string;
    policyId: string;
    candidateSlot: 0 | 1;
...
export type TerminalFreshDevelopmentRunPlan = {
    schemaVersion: typeof TERMINAL_FRESH_DEVELOPMENT_PLAN_SCHEMA_VERSION;
    kind: typeof TERMINAL_FRESH_DEVELOPMENT_PLAN_KIND;
    runId: string;
    ...
    maxTicks: number;
    arms: TerminalFreshDevelopmentPlanArm[];
    episodes: TerminalFreshDevelopmentPlanEpisode[];
};
...
export const parseTerminalFreshDevelopmentRunPlan = (value: unknown): TerminalFreshDevelopmentRunPlan => {
    if (!isRecord(value)) throw new Error("Fresh terminal-objective run plan must be an object");
    exactKeys(value, [
    ...
    ], "Fresh terminal-objective run plan");
    if (value.schemaVersion !== TERMINAL_FRESH_DEVELOPMENT_PLAN_SCHEMA_VERSION || value.kind !== TERMINAL_FRESH_DEVELOPMENT_PLAN_KIND) {
        throw new Error("Fresh terminal-objective plan identity is invalid");
    }
    if (
    ...
    ) throw new Error("Fresh terminal-objective plan endpoint binding drifted");
    if (!isRecord(value.family)) throw new Error("Fresh terminal-objective plan family is invalid");
    exactKeys(value.family, ["familyId", "mapName", "mapSha256"], "Fresh terminal-objective family");
    ...
    if (mapName !== mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(mapName)) {
        throw new Error("Fresh terminal-objective mapName must be a map basename");
    }
    const country = value.country;
    if (typeof country !== "string" || !Object.values(Countries).includes(country as Countries)) {
        throw new Error("Fresh terminal-objective plan country is invalid");
    }
    if (!Array.isArray(value.arms) || value.arms.length < 1) throw new Error("Fresh terminal-objective plan has no arms");
    const arms = value.arms.map((raw, index): TerminalFreshDevelopmentPlanArm => {
        if (!isRecord(raw)) throw new Error(`Fresh terminal-objective arm ${index} is invalid`);
        exactKeys(raw, ["armId", "policyId", "policy"], `Fresh terminal-objective arm ${index}`);
        ...
        if (terminalObjectivePolicySha256(policy) !== policyId) {
            throw new Error(`Fresh terminal-objective arm ${armId} policy hash drifted`);
        }
    ...
    ) throw new Error("Fresh terminal-objective plan arm identities must be unique");
    if (!Array.isArray(value.episodes)) throw new Error("Fresh terminal-objective plan episodes are invalid");
    const armById = new Map(arms.map((arm) => [arm.armId, arm]));
    const episodes = value.episodes.map((raw, index): TerminalFreshDevelopmentPlanEpisode => {
        if (!isRecord(raw)) throw new Error(`Fresh terminal-objective episode ${index} is invalid`);
        exactKeys(raw, ["episodeId", "armId", "policyId", "candidateSlot"], `Fresh terminal-objective episode ${index}`);
        ...
        if ((raw.candidateSlot !== 0 && raw.candidateSlot !== 1) || armById.get(armId)?.policyId !== policyId) {
            throw new Error(`Fresh terminal-objective episode ${index} has an invalid arm or slot binding`);
        }
    ...
    ) throw new Error("Fresh terminal-objective plan episode count or identities are invalid");
    for (const arm of arms) {
        const slots = episodes.filter(({ armId }) => armId === arm.armId).map(({ candidateSlot }) => candidateSlot).sort();
        if (slots.join(",") !== "0,1") throw new Error(`Fresh terminal-objective arm ${arm.armId} lacks reciprocal slots`);
    }
    ...
    if (derivePairedEngineSeed(engineSeedBase, seedBlockIndex) !== requestedEngineSeed) {
        throw new Error("Fresh terminal-objective requested engine seed drifted from its paired derivation");
    }
    return {
...
export const serializeTerminalFreshDevelopmentRunPlan = (plan: TerminalFreshDevelopmentRunPlan): string =>
    `${JSON.stringify(parseTerminalFreshDevelopmentRunPlan(plan), null, 2)}\n`;
...
export const runTerminalFreshDevelopmentPlanFromEnvironment = async (): Promise<void> => {
    const repoRoot = gitRoot();
    const driverRoot = path.join(repoRoot, "packages", "chronodivide-bot-driver");
    if (path.resolve(process.cwd()) !== driverRoot) throw new Error(`Fresh terminal-objective runner must start in ${driverRoot}`);
    const planPath = requireEnvPath("TERMINAL_FRESH_DEVELOPMENT_PLAN");
    const outDir = requireEnvPath("OUT_DIR");
    if (fs.existsSync(outDir)) throw new Error(`Refusing to reuse Fresh terminal-objective OUT_DIR ${outDir}`);
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Fresh terminal-objective execution requires the pinned external baseline environment");
    }
    const planSha256 = sha256File(planPath);
    const plan = parseTerminalFreshDevelopmentRunPlan(JSON.parse(fs.readFileSync(planPath, "utf8")));
    ...
    const manifest = createExperimentManifest({
        ...
        effectiveConfig: {
            runner: "terminalObjectiveFreshDevelopmentPlanRunner-v1",
            planPath,
            planSha256,
            ...
            outcomeAccess: "fresh-development-single-scheduled",
        },
    ...
    ) throw new Error("Fresh terminal-objective runtime, source, baseline, or scheduler provenance drifted");
    const runtimeMap = manifest.inputs.maps[0];
    if (!runtimeMap?.exists || runtimeMap.sha256 !== plan.family.mapSha256) {
        throw new Error("Fresh terminal-objective runtime map bytes drifted from the plan");
    }
    const policyFiles = [
    ...
    if (policyFiles.some((filePath) => /\bgetAllUnits\s*\(/.test(fs.readFileSync(filePath, "utf8")))) {
        throw new Error("Fresh terminal-objective policy source or runtime violates the complete-state information boundary");
    }
    ...
    for (let launchIndex = 0; launchIndex < plan.episodes.length; launchIndex++) {
        ...
        const arm = armById.get(episode.armId) as TerminalFreshDevelopmentPlanArm;
    ...
    const summary = {
        schemaVersion: 2,
        status: technicalFailures === 0 ? "COMPLETE_TERMINAL_OBJECTIVE_FRESH_DEVELOPMENT_SHARD" : "FAILED_TERMINAL_OBJECTIVE_TECHNICAL_SHARD",
...
if (import.meta.url === invoked) {
    runTerminalFreshDevelopmentPlanFromEnvironment().catch((error) => {
        console.error(error);
        process.exitCode = 1;
    });
}
