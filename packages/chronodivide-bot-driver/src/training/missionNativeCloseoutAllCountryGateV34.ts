...
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_ENGINE_SEED_BASE = 4_294_800_000 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_RUNS_PER_COUNTRY_SLOT = 4 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES = Object.freeze(
...
export type MissionNativeCloseoutAllCountryV34CoverageRow = {
    country: Countries;
    candidateSlot: 0 | 1;
    passed: boolean;
...
export const validateMissionNativeCloseoutV34ObjectiveRaceTelemetry = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
): void => {
    ...
    if (allocations.length !== objectiveRace.length) {
        throw new Error("V34 objective-race allocation telemetry did not cover every live allocation");
    }
    for (const event of objectiveRace) {
        const allocation = allocations.find((candidate) =>
            candidate.tick === event.tick && candidate.targetId === event.targetId &&
            candidate.blockerId === event.blockerId,
        );
        if (
            !allocation || allocation.assignedAttackerCount !== event.assignedAttackerCount ||
            allocation.buildingAttackerCount !== event.buildingAttackerCount ||
            allocation.blockerAttackerCount !== event.blockerAttackerCount
        ) {
            throw new Error("V34 objective-race telemetry did not match its live allocation event");
        }
        if (event.allocationMode !== "boundedScreen") {
            throw new Error("V34 objective-race allocation did not execute boundedScreen mode");
        }
        if (
            ...
            throw new Error("V34 objective-race allocation partition or terminal state was inconsistent");
        }
        if (event.terminalPriorityActive && event.blockerId === null) {
            if (event.blockerAttackerCount !== 0 || event.buildingAttackerCount !== event.assignedAttackerCount) {
                throw new Error("V34 feasible terminal strike did not receive the full compatible force");
            }
            ...
            if (
                ...
                throw new Error("V34 blocker clearance consumed more than the bounded screen");
            }
        } else if (
            event.blockerAttackerCount !== 0 || event.buildingAttackerCount !== event.assignedAttackerCount
        ) {
            throw new Error("V34 direct building strike did not retain the full compatible force");
        }
    }
};

const convertedAfterCompositionBlock = (row: MissionNativeCloseoutAllCountryV34CoverageRow): boolean =>
...
export const validateMissionNativeCloseoutAllCountryV34Coverage = (
    rows: readonly MissionNativeCloseoutAllCountryV34CoverageRow[],
): string[] => {
    const errors: string[] = [];
    if (rows.length !== MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES.length * 2) {
        errors.push("all-country matrix does not contain exactly 18 country-slot cells");
    }
    for (const country of MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES) {
    ...
    const compositionExposure = (row: MissionNativeCloseoutAllCountryV34CoverageRow): boolean =>
        row.preterminalCompositionBlockedEvaluationCount > 0;
    for (const [label, predicate] of [
        ["Allied", (row: MissionNativeCloseoutAllCountryV34CoverageRow) => alliedCountries.has(row.country)],
        ["Soviet", (row: MissionNativeCloseoutAllCountryV34CoverageRow) => !alliedCountries.has(row.country)],
        ["slot 0", (row: MissionNativeCloseoutAllCountryV34CoverageRow) => row.candidateSlot === 0],
        ["slot 1", (row: MissionNativeCloseoutAllCountryV34CoverageRow) => row.candidateSlot === 1],
    ] as const) {
    ...
    if (rows.some((row) => row.productionReservationEventCount !== 0)) {
        errors.push("V34 emitted destructive production-reservation telemetry");
    }
    for (const [label, predicate, expectedName] of [
        ["Allied", (row: MissionNativeCloseoutAllCountryV34CoverageRow) => alliedCountries.has(row.country), "GAPILE"],
        ["Soviet", (row: MissionNativeCloseoutAllCountryV34CoverageRow) => !alliedCountries.has(row.country), "NAHAND"],
    ] as const) {
...
const summarizeCoverage = (
    ...
    candidateSlot: 0 | 1,
    passed: boolean,
): MissionNativeCloseoutAllCountryV34CoverageRow => {
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("All-country V34 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("All-country V34 gate map bytes drifted");
    }
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const disabledPolicy = buildMissionNativeCloseoutPolicyV34(false);
    const enabledPolicy = buildMissionNativeCloseoutPolicyV34(true);
    const rows: Array<Record<string, unknown>> = [];
    const coverageRows: MissionNativeCloseoutAllCountryV34CoverageRow[] = [];
    ...
    for (const country of MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES) {
        for (const candidateSlot of [0, 1] as const) {
            const requestedEngineSeed = derivePairedEngineSeed(
                MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_ENGINE_SEED_BASE,
                index++,
            );
            ...
            if (directDigest !== disabledDigest) validationErrors.push("disabled V34 adapter drifted");
            if (disabled.telemetry.length !== 0) validationErrors.push("disabled V34 adapter emitted telemetry");
            for (const [label, trace] of [["first", first], ["repeat", repeat]] as const) {
                ...
                try {
                    validateMissionNativeCloseoutV34ObjectiveRaceTelemetry(trace.telemetry);
                } catch (error) {
                    validationErrors.push(`${label}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            ...
            if (digest(first.actions) === digest(direct.actions)) {
                validationErrors.push("enabled V34 policy did not change commands");
            }
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-all-country-gate-v34-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-all-country-gate-v34",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_SCHEMA_VERSION,
            disabledPolicyId: missionNativeCloseoutPolicyV34Sha256(disabledPolicy),
            enabledPolicyId: missionNativeCloseoutPolicyV34Sha256(enabledPolicy),
            countries: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES,
            reciprocalSlots: [0, 1],
            runsPerCountrySlot: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_RUNS_PER_COUNTRY_SLOT,
            maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
            outcomeInspection: false,
        },
        baseline: factory.descriptor,
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("All-country V34 gate provenance or matrix coverage failed");
    const globalValidationErrors = validateMissionNativeCloseoutAllCountryV34Coverage(coverageRows);
    const passed = coverageRows.every((row) => row.passed) && globalValidationErrors.length === 0;
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34",
        ...
        disabledPolicyId: missionNativeCloseoutPolicyV34Sha256(disabledPolicy),
        enabledPolicyId: missionNativeCloseoutPolicyV34Sha256(enabledPolicy),
        countryCount: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES.length,
        reciprocalSlotCount: 2,
        gameCount: rows.length * MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_RUNS_PER_COUNTRY_SLOT,
        maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
    ...
    if (!passed) throw new Error("All-country V34 gate failed; preserved outcome-free artifact");
};
