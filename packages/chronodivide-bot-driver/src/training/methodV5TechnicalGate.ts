...
import { MethodV5EpisodeResult } from "./methodV5Episode.js";
import { parseMethodV5RunPlan, sha256File } from "./methodV5PlanRunner.js";
...
export const validateMethodV5Campaign = (value: unknown): MethodV5Campaign => {
    if (
        ...
        value.familyCount !== METHOD_V5_FAMILY_COUNT ||
        value.countryCount !== METHOD_V5_COUNTRIES.length || value.reciprocalSlotCount !== 2 ||
        value.policyCount !== METHOD_V5_CLOSEOUT_ARM_ORDER.length ||
        value.seedBlockCount !== METHOD_V5_SHARD_COUNT ||
        value.launchedGameCount !== METHOD_V5_LAUNCH_COUNT ||
        value.engineSeedBase !== METHOD_V5_ENGINE_SEED_BASE || value.maxTicks !== METHOD_V5_MAX_TICKS ||
        !Array.isArray(value.countries) || value.countries.join(",") !== METHOD_V5_COUNTRIES.join(",") ||
        !Array.isArray(value.rankingRule) || value.rankingRule.join("\0") !== METHOD_V5_RANKING_RULE.join("\0") ||
        !Array.isArray(value.advancementRule) || value.advancementRule.join("\0") !== METHOD_V5_ADVANCEMENT_RULE.join("\0") ||
    ...
    if (
        ...
        campaign.arms.some((arm, index) => {
            ...
            return arm.armId !== expected.armId || arm.policyId !== expected.policyId ||
                methodV5CloseoutPolicySha256(arm.policy) !== expected.policyId;
        }) ||
        new Set(campaign.selectedFamilies.map(({ familyId }) => familyId)).size !== METHOD_V5_FAMILY_COUNT ||
        new Set(campaign.selectedFamilies.map(({ mapSha256 }) => mapSha256)).size !== METHOD_V5_FAMILY_COUNT ||
        campaign.shards.some((shard, index) => {
            const family = campaign.selectedFamilies[Math.floor(index / METHOD_V5_COUNTRIES.length)];
            return shard.shardIndex !== index || shard.familyId !== family.familyId ||
                ...
                shard.country !== METHOD_V5_COUNTRIES[index % METHOD_V5_COUNTRIES.length] ||
                shard.seedBlockIndex !== index || shard.requestedEngineSeed !== METHOD_V5_ENGINE_SEED_BASE + index ||
                shard.launchedGameCount !== METHOD_V5_CLOSEOUT_ARM_ORDER.length * 2;
        }) ||
        new Set(campaign.shards.map(({ planFile }) => planFile)).size !== METHOD_V5_SHARD_COUNT ||
        new Set(campaign.shards.map(({ planSha256 }) => planSha256)).size !== METHOD_V5_SHARD_COUNT ||
        new Set(campaign.shards.map(({ runId }) => runId)).size !== METHOD_V5_SHARD_COUNT
    ) throw new Error("Method-v5 campaign schedule or evidence chain drifted");
    return campaign;
};

type SchedulerTask = { schedulerJobId: string; state: "COMPLETED"; exitCode: "0:0"; account: "pi_jss233" };
export const parseMethodV5Sacct = (raw: string, arrayJobId: string): Map<number, SchedulerTask> => {
    const tasks = new Map<number, SchedulerTask>();
    for (const [lineIndex, line] of raw.split("\n").entries()) {
        if (!line) continue;
        const fields = line.split("|");
        if (fields.length !== 5) throw new Error(`Method-v5 sacct line ${lineIndex + 1} is malformed`);
        ...
        if (
            !match || !/^\d+$/.test(schedulerJobId) || taskIndex < 0 || taskIndex >= METHOD_V5_SHARD_COUNT ||
            tasks.has(taskIndex) || state !== "COMPLETED" || exitCode !== "0:0" || account !== "pi_jss233"
        ) throw new Error(`Method-v5 scheduler row ${lineIndex + 1} is failed, duplicate, or unauthorized`);
        tasks.set(taskIndex, { schedulerJobId, state, exitCode, account });
    }
    if (tasks.size !== METHOD_V5_SHARD_COUNT) throw new Error(`Method-v5 sacct returned ${tasks.size}/${METHOD_V5_SHARD_COUNT} tasks`);
    return tasks;
};
...
export const validateMethodV5Telemetry = (value: unknown): MethodV5CloseoutTelemetry => {
    ...
    if (value.event === "capability_request") {
        ...
        const allowedStructures = unitName === "JUMPJET"
            ? new Set([null, "GAAIRC", "AMRADR", "GATECH"])
            : unitName === "ZEP"
              ? new Set([null, "NARADR", "NATECH"])
              : null;
        if (!allowedStructures || !allowedStructures.has(value.requestedStructure as string | null)) {
            throw new Error("Method-v5 capability telemetry has an invalid unit or structure request");
        }
    }
    return value as unknown as MethodV5CloseoutTelemetry;
};

export const validateMethodV5Result = (
    value: unknown,
    ...
    ) throw new Error(`Method-v5 completion ${expected.episodeId} is malformed or technically invalid`);
    value.policyTelemetry.forEach(validateMethodV5Telemetry);
    ...
    if (value.outcomeStatus === "tick_cap_draw") {
        exactKeys(terminal, ["endpointVersion", "endpointSha256", "tick", "status", "winner"],
            `Method-v5 cap terminal ${expected.episodeId}`);
        ...
        ], `Method-v5 physical terminal ${expected.episodeId}`);
        if (
            terminal.endpoint !== LITERAL_BUILDING_ELIMINATION_ENDPOINT ||
            terminal.tick !== value.ticks || terminal.engineFinishedSameUpdate !== value.engineFinished
        ) throw new Error(`Method-v5 physical terminal ${expected.episodeId} has invalid endpoint metadata`);
    }
    if (
    ...
    if (value.winner !== "draw") {
        if (!isRecord(terminal.evaluation) || !isRecord(terminal.evaluation.zeroingDispositions)) {
            throw new Error(`Method-v5 win ${expected.episodeId} lacks its endpoint ledger`);
        }
        ...
        const rows = validatePhysicalRows(
            ...
            `Method-v5 win ${expected.episodeId}`,
        );
        ...
        ) throw new Error(`Method-v5 win ${expected.episodeId} violates literal building destruction`);
    } else if (value.outcomeStatus === "simultaneous_draw") {
        if (!isRecord(terminal.evaluation) || !isRecord(terminal.evaluation.zeroingDispositions)) {
            throw new Error(`Method-v5 simultaneous draw ${expected.episodeId} lacks its endpoint ledger`);
        }
        const preCounts = isRecord(terminal.evaluation.preCounts) ? terminal.evaluation.preCounts : null;
        const postCounts = isRecord(terminal.evaluation.postCounts) ? terminal.evaluation.postCounts : null;
        for (const side of ["candidate", "baseline"] as const) {
            ...
            const rows = validatePhysicalRows(
                ...
                side === "candidate" ? candidateName : baselineName,
                side === "candidate" ? baselineName : candidateName,
                `Method-v5 simultaneous draw ${expected.episodeId} ${side}`,
            );
            if (
            ...
            ) throw new Error(`Method-v5 simultaneous draw ${expected.episodeId} violates literal building destruction`);
        }
    }
    return value as unknown as MethodV5EpisodeResult;
};

const parseEvents = (eventsPath: string): RecordValue[] => fs.readFileSync(eventsPath, "utf8")
    .split("\n").filter(Boolean).map((line, index) => {
        const value = JSON.parse(line) as unknown;
        if (!isRecord(value) || typeof value.event !== "string") throw new Error(`Malformed Method-v5 event ${index}`);
        return value;
    });

const validateShard = (
    campaign: MethodV5Campaign,
    resultsRoot: string,
    arrayJobId: string,
    task: SchedulerTask,
    shard: MethodV5Campaign["shards"][number],
): number => {
    ...
    for (const required of [manifestPath, summaryPath, eventsPath, shard.planFile]) {
        if (!fs.existsSync(required)) throw new Error(`Method-v5 shard ${shard.shardIndex} lacks ${required}`);
    }
    ...
    if (
        ...
        summary.complete !== true || summary.technicallyClean !== true || summary.outcomeAccess !== "open-training-only" ||
    ...
    ) throw new Error(`Method-v5 shard ${shard.shardIndex} summary is not one complete clean block`);
    ...
    if (!isRecord(outer) || outer.planSha256 !== shard.planSha256 || !isRecord(outer.plan) || !isRecord(outer.manifest)) {
        throw new Error(`Method-v5 shard ${shard.shardIndex} manifest is malformed`);
    }
    const plan = parseMethodV5RunPlan(outer.plan);
    if (
    ...
    ) throw new Error(`Method-v5 shard ${shard.shardIndex} plan commitments drifted`);
    const expectedEpisodes = buildMethodV5Episodes(campaign.arms);
    if (plan.episodes.some((episode, index) => JSON.stringify(episode) !== JSON.stringify(expectedEpisodes[index]))) {
        throw new Error(`Method-v5 shard ${shard.shardIndex} episode order drifted`);
    }
    ...
    ) throw new Error(`Method-v5 shard ${shard.shardIndex} scheduler or source provenance drifted`);
    const events = parseEvents(eventsPath);
    if (events[0]?.event !== "run_start" || events[events.length - 1]?.event !== "run_complete") {
        throw new Error(`Method-v5 shard ${shard.shardIndex} boundary events drifted`);
    }
    let cursor = 1;
    for (let launchIndex = 0; launchIndex < plan.episodes.length; launchIndex++) {
        ...
        ) throw new Error(`Method-v5 shard ${shard.shardIndex} launch ${launchIndex} drifted`);
        const completion = events[cursor++];
        if (completion?.event !== "episode_complete" || completion.launchIndex !== launchIndex) {
            throw new Error(`Method-v5 shard ${shard.shardIndex} launch ${launchIndex} did not complete cleanly`);
        }
        validateMethodV5Result(completion.result, {
    ...
    if (cursor !== events.length - 1 || !isRecord(events[cursor].summary)) {
        throw new Error(`Method-v5 shard ${shard.shardIndex} event accounting is incomplete`);
    }
    return plan.episodes.length;
};

export const methodV5ResultArtifactCommitmentSha256 = (
    campaign: MethodV5Campaign,
    resultsRoot: string,
    arrayJobId: string,
...
const main = (): void => {
    ...
    if (fs.existsSync(outputPath)) throw new Error(`Refusing to overwrite Method-v5 gate ${outputPath}`);
    const campaign = validateMethodV5Campaign(readJson(campaignPath));
    const schedulerTasks = parseMethodV5Sacct(execFileSync(
    ...
    if (accountedLaunches !== METHOD_V5_LAUNCH_COUNT) throw new Error("Method-v5 launch accounting is incomplete");
    ...
    const output = {
        ...
        resultsRoot,
        resultArtifactCommitmentSha256: methodV5ResultArtifactCommitmentSha256(campaign, resultsRoot, arrayJobId),
        arrayJobId,
        schedulerAccount: "pi_jss233",
        schedulerJobIds: [...schedulerTasks.values()].map(({ schedulerJobId }) => schedulerJobId),
        shardCount: METHOD_V5_SHARD_COUNT,
        requestedLaunches: METHOD_V5_LAUNCH_COUNT,
        accountedLaunches,
        technicalFailures: 0,
