...
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_ENGINE_SEED_BASE = 4_289_000_000 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_RUNS_PER_COUNTRY_SLOT = 4 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_COUNTRIES = Object.freeze(
...
export type MissionNativeCloseoutAllCountryV29CoverageRow = {
    country: Countries;
    candidateSlot: 0 | 1;
    passed: boolean;
...
const convertedAfterCompositionBlock = (row: MissionNativeCloseoutAllCountryV29CoverageRow): boolean =>
...
export const validateMissionNativeCloseoutAllCountryV29Coverage = (
    rows: readonly MissionNativeCloseoutAllCountryV29CoverageRow[],
): string[] => {
    const errors: string[] = [];
    if (rows.length !== MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_COUNTRIES.length * 2) {
        errors.push("all-country matrix does not contain exactly 18 country-slot cells");
    }
    for (const country of MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_COUNTRIES) {
    ...
    const compositionExposure = (row: MissionNativeCloseoutAllCountryV29CoverageRow): boolean =>
        row.preterminalCompositionBlockedEvaluationCount > 0;
    for (const [label, predicate] of [
        ["Allied", (row: MissionNativeCloseoutAllCountryV29CoverageRow) => alliedCountries.has(row.country)],
        ["Soviet", (row: MissionNativeCloseoutAllCountryV29CoverageRow) => !alliedCountries.has(row.country)],
        ["slot 0", (row: MissionNativeCloseoutAllCountryV29CoverageRow) => row.candidateSlot === 0],
        ["slot 1", (row: MissionNativeCloseoutAllCountryV29CoverageRow) => row.candidateSlot === 1],
    ] as const) {
...
const summarizeCoverage = (
    ...
    candidateSlot: 0 | 1,
    passed: boolean,
): MissionNativeCloseoutAllCountryV29CoverageRow => {
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("All-country V29 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("All-country V29 gate map bytes drifted");
    }
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const disabledPolicy = buildMissionNativeCloseoutPolicyV29(false);
    const enabledPolicy = buildMissionNativeCloseoutPolicyV29(true);
    const rows: Array<Record<string, unknown>> = [];
    const coverageRows: MissionNativeCloseoutAllCountryV29CoverageRow[] = [];
    ...
    for (const country of MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_COUNTRIES) {
        for (const candidateSlot of [0, 1] as const) {
            const requestedEngineSeed = derivePairedEngineSeed(
                MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_ENGINE_SEED_BASE,
                index++,
            );
            ...
            if (directDigest !== disabledDigest) validationErrors.push("disabled V29 adapter drifted");
            if (disabled.telemetry.length !== 0) validationErrors.push("disabled V29 adapter emitted telemetry");
            ...
            if (digest(first.actions) === digest(direct.actions)) {
                validationErrors.push("enabled V29 policy did not change commands");
            }
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-all-country-gate-v29-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-all-country-gate-v29",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_SCHEMA_VERSION,
            disabledPolicyId: missionNativeCloseoutPolicyV29Sha256(disabledPolicy),
            enabledPolicyId: missionNativeCloseoutPolicyV29Sha256(enabledPolicy),
            countries: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_COUNTRIES,
            reciprocalSlots: [0, 1],
            runsPerCountrySlot: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_RUNS_PER_COUNTRY_SLOT,
            maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
            outcomeInspection: false,
        },
        baseline: factory.descriptor,
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("All-country V29 gate provenance or matrix coverage failed");
    const globalValidationErrors = validateMissionNativeCloseoutAllCountryV29Coverage(coverageRows);
    const passed = coverageRows.every((row) => row.passed) && globalValidationErrors.length === 0;
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29",
        ...
        disabledPolicyId: missionNativeCloseoutPolicyV29Sha256(disabledPolicy),
        enabledPolicyId: missionNativeCloseoutPolicyV29Sha256(enabledPolicy),
        countryCount: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_COUNTRIES.length,
        reciprocalSlotCount: 2,
        gameCount: rows.length * MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V29_RUNS_PER_COUNTRY_SLOT,
        maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
    ...
    if (!passed) throw new Error("All-country V29 gate failed; preserved outcome-free artifact");
};
