...
const GENERATION0_SHA256 = "f3eb995c15d2f947bad6afcc1c24653aa18391e40c2026661cc4fd25eafa5a0f";
...
const MAX_UPDATES = 90_000, T_CRITICAL_90_DF35 = 1.30621;
const ALLIED = new Set<Countries>(V8_COUNTRIES.slice(0, 5));
export const V8_G1_RUN_COUNT = 3 as const;
export const V8_G1_CASE_COUNT = 36 as const;
export const V8_G1_CANDIDATE_COUNT = 16 as const;
export const V8_G1_ARM_COUNT = 18 as const;
export const V8_G1_TASKS_PER_RUN = 648 as const;
export const V8_G1_TASK_COUNT = 1_944 as const;
...
type PreviousAggregate = { kind: string; complete: boolean; passed: boolean; protocolSha256: string;
    ...
    runs: Array<{ runIndex: number; candidates: Array<{ policySha256: string }>; generation1: PolicyRow[] }> };
...
const sourceIdentity = () => { const repo = execFileSync("git", ["rev-parse", "--show-toplevel"], { encoding: "utf8" }).trim(),
    ...
    if (execFileSync("git", ["branch", "--show-current"], { encoding: "utf8" }).trim() !== "main" ||
        ...
        throw new Error("V8 G1 requires clean synchronized main"); return { repo, commit }; };
const commonInputs = () => { if (process.env.SLURM_JOB_ACCOUNT !== "pi_jss233") throw new Error("V8 G1 requires pi_jss233");
    ...
    if (hashFile(protocolPath) !== protocolSha256 || protocolSha256 !== ORIGINAL_PROTOCOL_SHA256 ||
        ...
        throw new Error("V8 G1 inputs drifted");
    ...
    if (!isRecord(manifest) || manifest.kind !== "private-ra2-snow-runtime" ||
        path.resolve(String(manifest.runtimeDirectory)) !== mixDir) throw new Error("V8 G1 runtime drifted");
...
const assertBaseline = (factory: BaselineFactory) => { if (factory.descriptor.kind !== "external-package" ||
    typeof factory.descriptor.packageRoot !== "string") throw new Error("V8 G1 baseline external required");
    ...
    if (execFileSync("git", ["-C", repo, "rev-parse", "HEAD"], { encoding: "utf8" }).trim() !== BASELINE_COMMIT ||
        ...
        throw new Error("V8 G1 baseline drifted"); };
const loadAdvanced = (root: string) => { const loaded = loadRa2WebOpponent(root, "ra2web_advanced_old_priest");
    if (loaded.bundleSha256 !== ADVANCED_SHA256 || loaded.freezeManifestSha256 !== RA2WEB_FREEZE_MANIFEST_SHA256)
        throw new Error("V8 G1 Advanced drifted"); return loaded; };
...
const loadSelection = (file: string, expectedHash: string, inputs: ReturnType<typeof commonInputs>): Selection => {
    if (hashFile(file) !== expectedHash) throw new Error("V8 G1 selection hash drifted"); const value = JSON.parse(fs.readFileSync(file, "utf8"));
    if (value.kind !== "hfo-advanced-v8-master-selection" || value.complete !== true || value.passed !== true ||
        ...
        throw new Error("V8 G1 selection ineligible"); return value as Selection; };
const parsePolicy = (row: PolicyRow): CanonicalV8Policy => { const parsed = JSON.parse(row.canonicalJson), policy = canonicalizeV8Policy(parsed);
    if (policy.sha256 !== row.sha256 || JSON.stringify(policy.complexity) !== JSON.stringify(row.complexity))
        throw new Error("V8 G1 policy identity drifted"); return policy; };
const loadPrevious = (file: string, expectedHash: string, selectionSha256: string,
    ...
    if (hashFile(file) !== expectedHash || expectedHash !== GENERATION0_SHA256)
        throw new Error("V8 G1 Generation-0 hash drifted");
    const value = JSON.parse(fs.readFileSync(file, "utf8"));
    if (value.kind !== "hfo-advanced-v8-generation-0" || value.complete !== true || value.passed !== true ||
        ...
        value.runs.some((run: any) => !Array.isArray(run.candidates) || run.candidates.length !== 32 ||
            new Set(run.candidates.map((row: any) => row.policySha256)).size !== 32 ||
            !Array.isArray(run.generation1) || run.generation1.length !== 16 ||
            new Set(run.generation1.map((row: any) => row.sha256)).size !== 16))
        throw new Error("V8 G1 Generation-0 artifact ineligible");
    return value as PreviousAggregate;
};
export const v8G1Arms = (previous: PreviousAggregate, runIndex: number): Arm[] => {
    if (runIndex < 0 || runIndex >= 3) throw new Error("V8 G1 run invalid");
    const rows = previous.runs.find((run) => run.runIndex === runIndex)?.generation1;
    if (!Array.isArray(rows) || rows.length !== 16 || new Set(rows.map((row) => row.sha256)).size !== 16)
        throw new Error("V8 G1 policy set invalid");
...
export const v8G1Assignment = (taskIndex: number) => { if (taskIndex < 0 || taskIndex >= V8_G1_TASK_COUNT)
    throw new Error("V8 G1 task invalid"); const runIndex = Math.floor(taskIndex / V8_G1_TASKS_PER_RUN),
        local = taskIndex % V8_G1_TASKS_PER_RUN, armIndex = Math.floor(local / V8_G1_CASE_COUNT),
        caseIndex = local % V8_G1_CASE_COUNT; return { runIndex, armIndex, caseIndex }; };
...
const runCell = async () => {
    ...
    if (process.env.SLURM_ARRAY_TASK_ID !== String(taskIndex) || hashFile(programPath) !== programSha256 || fs.existsSync(out))
        throw new Error("V8 G1 assignment drifted");
    const selection = loadSelection(selectionPath, selectionSha256, inputs),
        previous = loadPrevious(previousPath, previousSha256, selectionSha256, inputs), assignment = v8G1Assignment(taskIndex),
        populationId = `run-${assignment.runIndex}-generation-1`, cases = selection.cases.filter((row) => row.populationId === populationId)
            .sort((a, b) => a.populationCaseIndex - b.populationCaseIndex), caseSpec = cases[assignment.caseIndex],
        arms = v8G1Arms(previous, assignment.runIndex), arm = arms[assignment.armIndex];
    if (cases.length !== 36 || !caseSpec || !arm) throw new Error("V8 G1 case/arm missing");
    ...
    const advanced = loadAdvanced(inputs.freezeRoot), candidateName = `V8G1Candidate_${assignment.runIndex}_${assignment.caseIndex}`,
        opponentName = `V8G1Advanced_${assignment.runIndex}_${assignment.caseIndex}`;
    ...
    const result = await withSeededOfflineGame(cdapi, settings(candidate, opponent, caseSpec.candidateSlot),
        ...
        async (game) => { const api = candidate.lastGameApi as GameApi | null; if (!api ||
            ...
            throw new Error("V8 G1 selected start/API drifted");
            ...
            while (updates < MAX_UPDATES && !terminal && !failure) { adjudicator.beginUpdate(api); await game.update(); updates += 1;
                const stats = game.getPlayerStats(), candidateStats = stats.find((row) => row.name === candidateName),
                    opponentStats = stats.find((row) => row.name === opponentName); if (!candidateStats || !opponentStats)
                        throw new Error("V8 G1 stats missing"); const endpoint = adjudicator.completeUpdate(api,
            ...
            if (failure) throw new Error(`V8 G1 endpoint failure ${JSON.stringify(failure)}`);
    ...
    const artifact = { schemaVersion: 1, kind: "hfo-advanced-v8-generation-1-cell",
        status: "COMPLETE_HFO_ADVANCED_V8_GENERATION_1_CELL", complete: true, taskIndex,
...
export const deriveV8Generation2 = (survivors: any[], runIndex: number, priorHashes: Set<string>) => {
    const parents = survivors.map((row) => row.policy as CanonicalV8Policy), output: CanonicalV8Policy[] = parents.slice(0, 2),
    ...
    for (let child = 0; child < 4; child += 1) { let attempt = 0, candidate: CanonicalV8Policy;
        do { candidate = mutateV8Policy(parents[child % 4], seed + 3_000 + child * 100 + attempt++); }
        while (seen.has(candidate.sha256) && attempt < 100); if (seen.has(candidate.sha256)) throw new Error("V8 G2 mutation collision");
        seen.add(candidate.sha256); output.push(candidate); }
    for (let child = 0; child < 2; child += 1) { let attempt = 0, candidate: CanonicalV8Policy;
        do { candidate = crossoverV8Policy(parents[child], parents[child + 2], seed + 4_000 + child * 100 + attempt++); }
        while (seen.has(candidate.sha256) && attempt < 100); if (seen.has(candidate.sha256)) throw new Error("V8 G2 crossover collision");
        seen.add(candidate.sha256); output.push(candidate); }
    if (output.length !== 8 || new Set(output.map((row) => row.sha256)).size !== 8) throw new Error("V8 G2 generation invalid");
    return output;
};
...
const finalize = () => {
    const root = requiredPath("RESULTS_ROOT"), out = requiredPath("OUT_PATH"), arrayJobId = requiredText("ARRAY_JOB_ID", /^\d+$/),
        ...
        previousPath = requiredPath("PREVIOUS_AGGREGATE_PATH"),
        previousSha256 = requiredText("PREVIOUS_AGGREGATE_SHA256", SHA256), inputs = commonInputs();
    if (fs.existsSync(out)) throw new Error("V8 G1 aggregate exists"); const { commit } = sourceIdentity(),
    ...
    let tasks = new Map<number, string>(); for (let attempt = 0; attempt < 31; attempt += 1) { tasks = completedTasks(arrayJobId);
        if (tasks.size === V8_G1_TASK_COUNT) break; if (attempt < 30) execFileSync("sleep", ["2"]); }
    if (tasks.size !== V8_G1_TASK_COUNT) throw new Error(`V8 G1 scheduler ${tasks.size}/${V8_G1_TASK_COUNT}`);
    const rows: any[] = [];
    for (let taskIndex = 0; taskIndex < V8_G1_TASK_COUNT; taskIndex += 1) { const assignment = v8G1Assignment(taskIndex),
        ...
        if (hashFile(file) !== expected) throw new Error(`V8 G1 checksum ${taskIndex}`); const cell = JSON.parse(fs.readFileSync(file, "utf8"));
        if (cell.kind !== "hfo-advanced-v8-generation-1-cell" || cell.complete !== true || cell.taskIndex !== taskIndex ||
            ...
            throw new Error(`V8 G1 identity ${taskIndex}`); rows.push(cell.result); }
    const runs = [], priorEvaluatedHashes = new Set(previous.runs.flatMap((run) => [
        ...run.candidates.map((row) => row.policySha256), ...run.generation1.map((row) => row.sha256),
    ])), generation2Hashes = new Set<string>();
    for (let runIndex = 0; runIndex < 3; runIndex += 1) { const runRows = rows.filter((row) => row.runIndex === runIndex),
        ...
        if (deployed.length !== 36 || external.length !== 36) throw new Error(`V8 G1 controls ${runIndex}`);
        const initial = v8G1Arms(previous, runIndex).slice(2), candidates = initial.map((arm) => { const candidateRows = runRows.filter((row) =>
            row.policySha256 === arm.policy!.sha256); if (candidateRows.length !== 36) throw new Error("V8 G1 candidate coverage");
            const differences = candidateRows.map((row) => score(row.winner) - score(deployedByCase.get(row.caseIndex).winner)),
                pairedMean = mean(differences), sd = sampleSd(differences), pairedLower = pairedMean - T_CRITICAL_90_DF35 * sd / Math.sqrt(36),
            ...
            return { policySha256: arm.policy!.sha256, policy: arm.policy, summary, minimumSideSlotScore: Math.min(...groupScores),
                paired: { mean: pairedMean, sampleStandardDeviation: sd, tCritical: T_CRITICAL_90_DF35,
        ...
        const survivors = candidates.slice(0, 4), priorHashes = new Set([...priorEvaluatedHashes, ...generation2Hashes]),
            generation2 = deriveV8Generation2(survivors, runIndex, priorHashes);
        generation2.forEach((policy) => generation2Hashes.add(policy.sha256));
        runs.push({ runIndex, controls: { deployed: summarize(deployed), external: summarize(external) }, candidates,
            survivors: survivors.map((row, rank) => ({ rank, ...row })), generation2: generation2.map((policy, index) =>
                ({ index, sha256: policy.sha256, canonicalJson: policy.canonicalJson, complexity: policy.complexity })) }); }
    const artifact = { schemaVersion: 1, kind: "hfo-advanced-v8-generation-1",
        status: "PASS_HFO_ADVANCED_V8_GENERATION_1", complete: true, passed: true,
