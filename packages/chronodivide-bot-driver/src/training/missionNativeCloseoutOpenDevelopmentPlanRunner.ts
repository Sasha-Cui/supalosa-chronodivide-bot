...
import {
    MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_EPISODE_SCHEMA_VERSION,
    MissionNativeCloseoutOpenDevelopmentEpisodeSpec,
    runMissionNativeCloseoutOpenDevelopmentEpisode,
} from "./missionNativeCloseoutOpenDevelopmentEpisode.js";
import {
...
export const MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_PLAN_SCHEMA_VERSION = 2 as const;
export const MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_PLAN_KIND =
    "mission-native-closeout-open-development-literal-endpoint" as const;

export type MissionNativeCloseoutOpenDevelopmentPlanArm = {
    armId: string;
    policyId: string;
    policy: MissionNativeCloseoutExperimentPolicy;
};

export type MissionNativeCloseoutOpenDevelopmentPlanEpisode = {
    episodeId: string;
    armId: string;
    policyId: string;
    candidateSlot: 0 | 1;
...
export type MissionNativeCloseoutOpenDevelopmentRunPlan = {
    schemaVersion: typeof MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_PLAN_SCHEMA_VERSION;
    kind: typeof MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_PLAN_KIND;
    runId: string;
    ...
    maxTicks: number;
    arms: MissionNativeCloseoutOpenDevelopmentPlanArm[];
    episodes: MissionNativeCloseoutOpenDevelopmentPlanEpisode[];
};
...
export const parseMissionNativeCloseoutOpenDevelopmentRunPlan = (value: unknown): MissionNativeCloseoutOpenDevelopmentRunPlan => {
    if (!isRecord(value)) throw new Error("Mission-native closeout open-development run plan must be an object");
    exactKeys(value, [
    ...
    ], "Mission-native closeout open-development run plan");
    if (value.schemaVersion !== MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_PLAN_SCHEMA_VERSION || value.kind !== MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_PLAN_KIND) {
        throw new Error("Mission-native closeout open-development plan identity is invalid");
    }
    if (
    ...
    ) throw new Error("Mission-native closeout open-development plan endpoint binding drifted");
    if (!isRecord(value.family)) throw new Error("Mission-native closeout open-development plan family is invalid");
    exactKeys(value.family, ["familyId", "mapName", "mapSha256"], "Mission-native closeout open-development family");
    ...
    if (mapName !== mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(mapName)) {
        throw new Error("Mission-native closeout open-development mapName must be a map basename");
    }
    const country = value.country;
    if (typeof country !== "string" || !Object.values(Countries).includes(country as Countries)) {
        throw new Error("Mission-native closeout open-development plan country is invalid");
    }
    if (!Array.isArray(value.arms) || value.arms.length < 1) throw new Error("Mission-native closeout open-development plan has no arms");
    const arms = value.arms.map((raw, index): MissionNativeCloseoutOpenDevelopmentPlanArm => {
        if (!isRecord(raw)) throw new Error(`Mission-native closeout open-development arm ${index} is invalid`);
        exactKeys(raw, ["armId", "policyId", "policy"], `Mission-native closeout open-development arm ${index}`);
        ...
        const policy = validateMissionNativeCloseoutExperimentPolicy(raw.policy as MissionNativeCloseoutExperimentPolicy);
        if (missionNativeCloseoutExperimentPolicySha256(policy) !== policyId) {
            throw new Error(`Mission-native closeout open-development arm ${armId} policy hash drifted`);
        }
    ...
    ) throw new Error("Mission-native closeout open-development plan arm identities must be unique");
    if (!Array.isArray(value.episodes)) throw new Error("Mission-native closeout open-development plan episodes are invalid");
    const armById = new Map(arms.map((arm) => [arm.armId, arm]));
    const episodes = value.episodes.map((raw, index): MissionNativeCloseoutOpenDevelopmentPlanEpisode => {
        if (!isRecord(raw)) throw new Error(`Mission-native closeout open-development episode ${index} is invalid`);
        exactKeys(raw, ["episodeId", "armId", "policyId", "candidateSlot"], `Mission-native closeout open-development episode ${index}`);
        ...
        if ((raw.candidateSlot !== 0 && raw.candidateSlot !== 1) || armById.get(armId)?.policyId !== policyId) {
            throw new Error(`Mission-native closeout open-development episode ${index} has an invalid arm or slot binding`);
        }
    ...
    ) throw new Error("Mission-native closeout open-development plan episode count or identities are invalid");
    for (const arm of arms) {
        const slots = episodes.filter(({ armId }) => armId === arm.armId).map(({ candidateSlot }) => candidateSlot).sort();
        if (slots.join(",") !== "0,1") throw new Error(`Mission-native closeout open-development arm ${arm.armId} lacks reciprocal slots`);
    }
    ...
    if (derivePairedEngineSeed(engineSeedBase, seedBlockIndex) !== requestedEngineSeed) {
        throw new Error("Mission-native closeout open-development requested engine seed drifted from its paired derivation");
    }
    return {
...
export const serializeMissionNativeCloseoutOpenDevelopmentRunPlan = (plan: MissionNativeCloseoutOpenDevelopmentRunPlan): string =>
    `${JSON.stringify(parseMissionNativeCloseoutOpenDevelopmentRunPlan(plan), null, 2)}\n`;
...
export const runMissionNativeCloseoutOpenDevelopmentPlanFromEnvironment = async (): Promise<void> => {
    const repoRoot = gitRoot();
    const driverRoot = path.join(repoRoot, "packages", "chronodivide-bot-driver");
    if (path.resolve(process.cwd()) !== driverRoot) throw new Error(`Mission-native closeout open-development runner must start in ${driverRoot}`);
    const planPath = requireEnvPath("MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_PLAN");
    const outDir = requireEnvPath("OUT_DIR");
    if (fs.existsSync(outDir)) throw new Error(`Refusing to reuse Mission-native closeout open-development OUT_DIR ${outDir}`);
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Mission-native closeout open-development execution requires the pinned external baseline environment");
    }
    const planSha256 = sha256File(planPath);
    const plan = parseMissionNativeCloseoutOpenDevelopmentRunPlan(JSON.parse(fs.readFileSync(planPath, "utf8")));
    ...
    ) throw new Error("Mission-native closeout open-development runtime, source, baseline, or scheduler provenance drifted");
    const runtimeMap = manifest.inputs.maps[0];
    if (!runtimeMap?.exists || runtimeMap.sha256 !== plan.family.mapSha256) {
        throw new Error("Mission-native closeout open-development runtime map bytes drifted from the plan");
    }
    const policyFiles = [
        path.join(driverRoot, "src", "training", "missionNativeCloseoutCandidate.ts"),
        path.join(driverRoot, "dist", "training", "missionNativeCloseoutCandidate.js"),
        path.join(driverRoot, "src", "training", "missionNativeCloseoutPolicyV34.ts"),
    ...
    if (
        policyFiles.some((filePath) => /\bgetAllUnits\s*\(|LiteralBuildingElimination|ObjectDestroy|winner|outcome/i
            .test(fs.readFileSync(filePath, "utf8")))
    ) throw new Error("Mission-native closeout open-development wrapper violates its declared endpoint boundary");
    ...
    for (let launchIndex = 0; launchIndex < plan.episodes.length; launchIndex++) {
        ...
        const arm = armById.get(episode.armId) as MissionNativeCloseoutOpenDevelopmentPlanArm;
        const spec: MissionNativeCloseoutOpenDevelopmentEpisodeSpec = {
            schemaVersion: MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_EPISODE_SCHEMA_VERSION,
        ...
        try {
            const result = await runMissionNativeCloseoutOpenDevelopmentEpisode(spec, baselineFactory);
    ...
    const summary = {
        ...
        status: technicalFailures === 0
            ? "COMPLETE_MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_SHARD"
            : "FAILED_MISSION_NATIVE_CLOSEOUT_OPEN_DEVELOPMENT_TECHNICAL_SHARD",
...
if (import.meta.url === invoked) {
    runMissionNativeCloseoutOpenDevelopmentPlanFromEnvironment().catch((error) => {
        console.error(error);
        process.exitCode = 1;
    });
}
