...
const KIND = "stagnation-assault-complete-open-screen-v1" as const;
const STATUS = "FROZEN_STAGNATION_ASSAULT_COMPLETE_OPEN_SCREEN_V1" as const;
const COUNTRIES = [
...
type ArmId = "external_supalosa_control" | "unchanged_v5" |
    "v5_plus_stagnation_conservative" | "v5_plus_stagnation_early" |
    "v5_plus_stagnation_early_strong";
type Arm = {
    armId: ArmId;
    kind: "control" | "v5" | "assault";
...
const commitmentFor = (arm: Omit<Arm, "policySha256">) => {
    ...
    const assault = buildStagnationAssaultPolicy(arm.variant);
    return {
        architecture: arm.kind === "control" ? "exact_external_supalosa" :
            "exact_external_supalosa_then_optional_stagnation_assault_then_strict_v5",
        armId: arm.armId,
        v5: v5Policy,
        v5Sha256: progressCertifiedConversionPolicyV5Sha256(v5Policy),
        assault,
        assaultSha256: stagnationAssaultPolicySha256(assault),
    };
};
const buildArms = (): Arm[] => ([
    { armId: "external_supalosa_control", kind: "control", variant: "disabled" },
    { armId: "unchanged_v5", kind: "v5", variant: "disabled" },
    { armId: "v5_plus_stagnation_conservative", kind: "assault", variant: "conservative" },
    { armId: "v5_plus_stagnation_early", kind: "assault", variant: "early" },
    { armId: "v5_plus_stagnation_early_strong", kind: "assault", variant: "early_strong" },
...
const validateCampaign = (value: unknown): Campaign => {
    if (!isRecord(value) || value.schemaVersion !== 1 || value.kind !== KIND || value.status !== STATUS ||
        ...
        throw new Error("Stagnation-assault campaign schema is invalid");
    }
    ...
    if (JSON.stringify(campaign.arms) !== JSON.stringify(buildArms()) ||
        ...
        throw new Error("Stagnation-assault campaign commitments drifted");
    }
    return campaign;
};
...
type EpisodeResult = {
    ...
    assaultTelemetry: StagnationAssaultTelemetry[];
};
...
const createCandidate = (
    ...
    return createStagnationAssaultCandidate(
        factory, name, country, buildProgressCertifiedConversionPolicyV5(),
        buildStagnationAssaultPolicy(arm.variant),
        { v5: (event) => v5Telemetry.push(event), assault: (event) => assaultTelemetry.push(event) },
    );
};
...
const runEpisode = async (campaign: Campaign, shard: Shard, arm: Arm, slot: 0 | 1,
    ...
    const candidateName = `StagnationCandidate_${shard.familyOrdinal}_${shard.countryOrdinal}_${slot}`;
    const baselineName = `StagnationBaseline_${shard.familyOrdinal}_${shard.countryOrdinal}_${slot}`;
    const v5Telemetry: TerminalObjectiveTelemetry[] = [];
    const assaultTelemetry: StagnationAssaultTelemetry[] = [];
    const candidate = createCandidate(factory, candidateName, shard.country, arm, v5Telemetry, assaultTelemetry);
...
const generate = async (): Promise<void> => {
    ...
    if (!isRecord(compatibility) || compatibility.kind !==
        "stagnation-assault-outcome-blind-live-compatibility-gate" || compatibility.passed !== true ||
        compatibility.outcomeFree !== true || compatibility.schedulerAccount !== "pi_jss233" ||
        !/^\d+$/.test(String(compatibility.schedulerJobId)) ||
        compatibility.policySha256 !== stagnationAssaultPolicySha256(buildStagnationAssaultPolicy("early_strong"))) {
        throw new Error("Compatibility gate did not pass the exact strongest intervention interface");
    }
    ...
    const manifest = createExperimentManifest({ runId: "stagnation-assault-open-screen-generator",
    ...
    const campaign: Campaign = {
        ...
        compatibilityGate: { path: compatibilityPath, sha256: compatibilitySha256,
            schedulerJobId: String(compatibility.schedulerJobId) },
...
const cell = async (): Promise<void> => {
    ...
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const manifest = createExperimentManifest({ runId: `stagnation-open-${taskIndex}-${process.env.SLURM_JOB_ID}`,
    ...
    const summary = { schemaVersion: 1, status: technicalFailures === 0 ? "COMPLETE_STAGNATION_ASSAULT_OPEN_SHARD" :
        "FAILED_STAGNATION_ASSAULT_OPEN_SHARD", campaignSha256, taskIndex, requestedLaunches: 10,
...
type Observation = {
    ...
    assault: StagnationAssaultTelemetry[];
};
const validateResult = (value: unknown, campaign: Campaign, shard: Shard, arm: Arm, slot: 0 | 1): Observation => {
    if (!isRecord(value) || value.schemaVersion !== 1 || value.familyId !== shard.familyId ||
        ...
        !Array.isArray(value.v5Telemetry) || !Array.isArray(value.assaultTelemetry)) {
    ...
    return { armId: arm.armId, familyId: shard.familyId, country: shard.country, candidateSlot: slot,
        ...
        nonterminalDraw: winner === "draw" && ["tick_cap_draw", "engine_nonliteral_termination_draw"]
            .includes(String(value.outcomeStatus)), assault };
};
...
const finalize = (): void => {
    ...
    for (const shard of campaign.shards) {
        ...
        if (!isRecord(summary) || summary.status !== "COMPLETE_STAGNATION_ASSAULT_OPEN_SHARD" ||
    ...
    const evaluations = campaign.arms.filter(({ kind }) => kind === "assault").map((arm) => {
        ...
        if (missionCreations === 0) eligibilityFailures.push("No assault mission was created");
    ...
    const artifact = { schemaVersion: 1, kind: "stagnation-assault-complete-open-screen-finalizer-v1",
        ...
        artifactRows: observations.map(({ assault: _assault, ...row }) => row) };
