...
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_ENGINE_SEED_BASE = 4_293_500_000 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_RUNS_PER_COUNTRY_SLOT = 4 as const;
export const MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_COUNTRIES = Object.freeze(
...
export type MissionNativeCloseoutAllCountryV30CoverageRow = {
    country: Countries;
    candidateSlot: 0 | 1;
    passed: boolean;
...
const convertedAfterCompositionBlock = (row: MissionNativeCloseoutAllCountryV30CoverageRow): boolean =>
...
export const validateMissionNativeCloseoutAllCountryV30Coverage = (
    rows: readonly MissionNativeCloseoutAllCountryV30CoverageRow[],
): string[] => {
    const errors: string[] = [];
    if (rows.length !== MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_COUNTRIES.length * 2) {
        errors.push("all-country matrix does not contain exactly 18 country-slot cells");
    }
    for (const country of MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_COUNTRIES) {
    ...
    const compositionExposure = (row: MissionNativeCloseoutAllCountryV30CoverageRow): boolean =>
        row.preterminalCompositionBlockedEvaluationCount > 0;
    for (const [label, predicate] of [
        ["Allied", (row: MissionNativeCloseoutAllCountryV30CoverageRow) => alliedCountries.has(row.country)],
        ["Soviet", (row: MissionNativeCloseoutAllCountryV30CoverageRow) => !alliedCountries.has(row.country)],
        ["slot 0", (row: MissionNativeCloseoutAllCountryV30CoverageRow) => row.candidateSlot === 0],
        ["slot 1", (row: MissionNativeCloseoutAllCountryV30CoverageRow) => row.candidateSlot === 1],
    ] as const) {
    ...
    if (rows.some((row) => row.productionReservationEventCount !== 0)) {
        errors.push("V30 emitted destructive production-reservation telemetry");
    }
    for (const [label, predicate, expectedName] of [
        ["Allied", (row: MissionNativeCloseoutAllCountryV30CoverageRow) => alliedCountries.has(row.country), "GAPILE"],
        ["Soviet", (row: MissionNativeCloseoutAllCountryV30CoverageRow) => !alliedCountries.has(row.country), "NAHAND"],
    ] as const) {
    ...
    const sovietRows = rows.filter((row) => !alliedCountries.has(row.country));
    if (!sovietRows.some((row) => row.screenInfrastructureRequestedCount > 0)) {
        errors.push("Soviet rows never requested missing screen infrastructure");
    }
    return errors;
};
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("All-country V30 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("All-country V30 gate map bytes drifted");
    }
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const disabledPolicy = buildMissionNativeCloseoutPolicyV30(false);
    const enabledPolicy = buildMissionNativeCloseoutPolicyV30(true);
    const rows: Array<Record<string, unknown>> = [];
    const coverageRows: MissionNativeCloseoutAllCountryV30CoverageRow[] = [];
    ...
    for (const country of MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_COUNTRIES) {
        for (const candidateSlot of [0, 1] as const) {
            ...
            if (directDigest !== disabledDigest) validationErrors.push("disabled V30 adapter drifted");
            if (disabled.telemetry.length !== 0) validationErrors.push("disabled V30 adapter emitted telemetry");
            ...
            if (digest(first.actions) === digest(direct.actions)) {
                validationErrors.push("enabled V30 policy did not change commands");
            }
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-all-country-gate-v30-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-all-country-gate-v30",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_SCHEMA_VERSION,
            disabledPolicyId: missionNativeCloseoutPolicyV30Sha256(disabledPolicy),
            enabledPolicyId: missionNativeCloseoutPolicyV30Sha256(enabledPolicy),
            countries: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_COUNTRIES,
            reciprocalSlots: [0, 1],
            runsPerCountrySlot: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_RUNS_PER_COUNTRY_SLOT,
            maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
            outcomeInspection: false,
        },
        baseline: factory.descriptor,
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("All-country V30 gate provenance or matrix coverage failed");
    const globalValidationErrors = validateMissionNativeCloseoutAllCountryV30Coverage(coverageRows);
    const passed = coverageRows.every((row) => row.passed) && globalValidationErrors.length === 0;
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30",
        ...
        disabledPolicyId: missionNativeCloseoutPolicyV30Sha256(disabledPolicy),
        enabledPolicyId: missionNativeCloseoutPolicyV30Sha256(enabledPolicy),
        countryCount: MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_COUNTRIES.length,
        reciprocalSlotCount: 2,
        gameCount: rows.length * MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V30_RUNS_PER_COUNTRY_SLOT,
        maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
    ...
    if (!passed) throw new Error("All-country V30 gate failed; preserved outcome-free artifact");
};
