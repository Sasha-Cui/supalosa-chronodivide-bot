...
import { createExternalBaselineOverlayCandidate } from "./researchEpisode.js";
import { buildMethodV4LifecycleArms } from "./methodV4LifecyclePolicies.js";
import { sha256File } from "./researchPlanRunner.js";

export const METHOD_V4_EQUIVALENCE_MAX_TICKS = 1_200 as const;
export const METHOD_V4_EQUIVALENCE_ENGINE_SEED_BASE = 3_490_000_000 as const;
...
const snapshot = (
    bot: ReturnType<Awaited<ReturnType<typeof loadBaselineFactory>>["create"]>,
    tick: number,
): Snapshot => {
...
const buildSettings = (
    mapName: string,
    candidate: ReturnType<Awaited<ReturnType<typeof loadBaselineFactory>>["create"]>,
    baseline: ReturnType<Awaited<ReturnType<typeof loadBaselineFactory>>["create"]>,
    candidateSlot: 0 | 1,
): CreateOfflineOpts => ({
    buildOffAlly: false,
    cratesAppear: false,
...
const runTrial = async (
    ...
    control: ReturnType<typeof buildMethodV4LifecycleArms>[number],
): Promise<TrialTrace> => {
    const candidateName = `Candidate_${country}_${candidateSlot}`;
    const opponentName = `Opponent_${country}_${candidateSlot}`;
    const candidate = wrapCandidate
        ? createExternalBaselineOverlayCandidate(
            factory,
            candidateName,
            country,
            control.policy,
            () => { throw new Error("Disabled baseline control emitted lifecycle telemetry"); },
        )
    ...
    await withSeededOfflineGame(
        ...
        [
            ...
            { agent: opponent, identity: "baseline" },
        ],
        async (game) => {
            for (let tick = 1; tick <= METHOD_V4_EQUIVALENCE_MAX_TICKS; tick++) {
...
const main = async (): Promise<void> => {
    ...
    if (fs.existsSync(outFile)) throw new Error(`Refusing to overwrite equivalence gate ${outFile}`);
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Method-v4 equivalence gate requires the external baseline package");
    }
    ...
    const control = buildMethodV4LifecycleArms().find(({ armId }) => armId === "baseline_control");
    ...
    for (const country of Object.values(Countries)) {
        for (const candidateSlot of [0, 1] as const) {
            const requestedEngineSeed = derivePairedEngineSeed(METHOD_V4_EQUIVALENCE_ENGINE_SEED_BASE, seedBlockIndex++);
            const wrapped = await runTrial(
                factory,
                mapName,
                country,
                candidateSlot,
            ...
            const unwrapped = await runTrial(
                factory,
                mapName,
                country,
                candidateSlot,
            ...
            if (!equal) throw new Error(`Wrapped external baseline drifted for ${country} slot ${candidateSlot}`);
            rows.push({
                country,
                candidateSlot,
                requestedEngineSeed,
                snapshotTicks: wrapped.candidate.map(({ tick }) => tick),
                wrapped,
                unwrapped,
                equal,
            });
    ...
    const manifest = createExperimentManifest({
        runId: `method-v4-baseline-equivalence-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "method-v4-outcome-free-baseline-equivalence",
            countries: Object.values(Countries),
            reciprocalSlots: [0, 1],
            maxTicks: METHOD_V4_EQUIVALENCE_MAX_TICKS,
            overlayEnabled: false,
            candidateRuntime: "same external BaselineFactory object, returned without callback mutation",
        },
        baseline: factory.descriptor,
        gameSeedBase: METHOD_V4_EQUIVALENCE_ENGINE_SEED_BASE,
    });
    if (
        ...
        throw new Error("Method-v4 equivalence provenance or coverage failed");
    }
    const output = {
        schemaVersion: 1,
        ...
        implementationInvariant:
            "disabled preserveBaselineCore adapter returns the exact external BaselineFactory bot object without mutation",
        rows,
    };
    fs.mkdirSync(path.dirname(outFile), { recursive: true, mode: 0o700 });
    fs.writeFileSync(outFile, JSON.stringify(output, null, 2), { flag: "wx", mode: 0o600 });
    console.log(JSON.stringify({ outFile, sha256: sha256File(outFile), status: output.status, gameCount: rows.length * 2 }));
};
