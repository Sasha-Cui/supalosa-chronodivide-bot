import { Bot, CreateOfflineOpts, GameApi, ObjectType, OrderType, ProductionApi, QueueType, UnitData, cdapi } from "@chronodivide/game-api";
...
const SEED_BASE = 4_243_000_000, MAX_OFFSETS = 400, MAX_TICKS = 12_000, SNAPSHOT_INTERVAL = 600;
...
const selectCases = async (): Promise<void> => {
                ...
                for (const candidateSlot of [0, 1] as const) {
                    ...
                    if (starts.candidateStart === desiredStart && starts.baselineStart === OPPOSITE[desiredStart]) {
                        selected = { caseIndex: cases.length, countryOrdinal, country, startOrdinal, desiredStart,
                            ...
                            ...starts, expectedActive: ALLIED.has(country) && desiredStart === "39,82" };
                        break;
                    }
                }
            }
    ...
    if (cases.length !== CASE_COUNT || new Set(cases.map((entry) =>
        ...
        cases.filter((entry) => entry.expectedActive).length !== 5) throw new Error("Isolation selection coverage drifted");
    const artifact = { schemaVersion: 1, kind: "hfo-allied-west-isolation-selection",
        status: "PASS_HFO_ALLIED_WEST_ISOLATION_SELECTION", complete: true, passed: true, outcomeFree: true,
        ...
        selectedCaseCount: cases.length, activeCaseCount: 5, inactiveCaseCount: 31, forbiddenOutcomeFields: [], cases };
...
const loadSelection = (selectionPath: string, selectionSha256: string, inputs: ReturnType<typeof commonInputs>): CaseSpec[] => {
    ...
    if (!isRecord(selection) || selection.kind !== "hfo-allied-west-isolation-selection" ||
        selection.status !== "PASS_HFO_ALLIED_WEST_ISOLATION_SELECTION" || selection.complete !== true ||
        selection.passed !== true || selection.outcomeFree !== true || selection.updateCount !== 0 ||
        selection.selectedCaseCount !== CASE_COUNT || selection.activeCaseCount !== 5 || selection.inactiveCaseCount !== 31 ||
...
const runArm = async (args: { factory: Awaited<ReturnType<typeof loadBaselineFactory>>; caseSpec: CaseSpec;
    arm: "default" | "winner_conditional" }) => {
    ...
    const winner = arm === "winner_conditional";
    const candidate = createDeployedStrongBotCandidate(candidateName, caseSpec.country,
        winner ? { hfoAlliedWestProfile: true } : {}, winner ? { hfoWestHomeGuard: {
    ...
    return withSeededOfflineGame(cdapi, settings(candidate, baseline, caseSpec.candidateSlot), caseSpec.requestedEngineSeed,
        [{ agent: candidate, identity: "candidate" }, { agent: baseline, identity: "baseline" }], async (game) => {
            ...
            const guardAnchorOrders = actions.filter((row) => row.tick <= 9_600 &&
                row.orderType === OrderType.AttackMove && typeof row.args[2] === "number" && typeof row.args[3] === "number" &&
                GUARD_ANCHORS.has(`${row.args[2]},${row.args[3]}`)).length;
            return { arm, observedTicks: ticks, engineFinished: game.isFinished(),
                ...
                snapshotSha256: sha256Value(snapshots), guardAnchorOrders };
        });
};
...
const runCell = async (): Promise<void> => {
    ...
    await cdapi.init(inputs.mixDir);
    const factory = await loadBaselineFactory(path.join(repo, "packages", "chronodivide-bot"));
    const defaultTrace = await runArm({ factory, caseSpec, arm: "default" });
    const winnerTrace = await runArm({ factory, caseSpec, arm: "winner_conditional" });
    ...
    const passed = caseSpec.expectedActive
        ? defaultTrace.actionSha256 !== winnerTrace.actionSha256 && winnerTrace.guardAnchorOrders > 0
        : inactiveEqual && winnerTrace.guardAnchorOrders === 0;
    const artifact = { schemaVersion: 1, kind: "hfo-allied-west-isolation-cell",
        status: passed ? "PASS_HFO_ALLIED_WEST_ISOLATION_CELL" : "FAIL_HFO_ALLIED_WEST_ISOLATION_CELL",
...
const finalize = (): void => {
    ...
    for (let taskIndex = 0; taskIndex < CASE_COUNT; taskIndex += 1) {
        ...
        if (cell.kind !== "hfo-allied-west-isolation-cell" || cell.status !== "PASS_HFO_ALLIED_WEST_ISOLATION_CELL" ||
    ...
    const passed = active.length === 5 && inactive.length === 31 && countries.size === 9 && starts.size === 4 && sources.size === 1;
    const artifact = { schemaVersion: 1, kind: "hfo-allied-west-activation-isolation-gate",
        status: passed ? "PASS_HFO_ALLIED_WEST_ACTIVATION_ISOLATION" : "FAIL_HFO_ALLIED_WEST_ACTIVATION_ISOLATION",
