...
import { validateMissionNativeCloseoutV35ObjectiveRaceTelemetry } from
    "./missionNativeCloseoutAllCountryGateV35.js";
import {
...
export const MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_SEED_BASE = 4_294_940_000 as const;
export const MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_MAX_TICKS = 7_200 as const;
export const MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_COUNTRIES = [
    Countries.USA,
    Countries.KOREA,
    Countries.FRANCE,
    Countries.GERMANY,
...
export const MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_CELL_COUNT =
    MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_COUNTRIES.length * 2;
...
const requiredTaskIndex = (): number => {
    ...
    if (!Number.isSafeInteger(value) || value < 0 || value >= MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_CELL_COUNT) {
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("V37-R2 literal-endpoint probe requires the pinned external baseline");
    }
    ...
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("V37-R2 literal-endpoint probe map bytes drifted");
    }
    const country = MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_COUNTRIES[Math.floor(taskIndex / 2)];
    const candidateSlot = taskIndex % 2 as 0 | 1;
    const requestedEngineSeed = MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_SEED_BASE + taskIndex;
    ...
    const args = {
        ...
        candidateSlot,
        policy,
        maxTicks: MISSION_NATIVE_CLOSEOUT_V37_R2_EXPOSURE_MAX_TICKS,
    ...
    if (firstSha256 !== repeatSha256) validationErrors.push("same-seed V37-R2 traces differ");
    if (
        first.observedTicks !== repeat.observedTicks ||
        first.engineFinishObservedAtTick !== repeat.engineFinishObservedAtTick
    ) validationErrors.push("same-seed V37-R2 trace boundaries differ");
    let control: {
        directSha256: string;
    ...
    if (taskIndex === 0) {
        ...
        if (directSha256 !== disabledSha256 || disabled.telemetry.length !== 0) {
            validationErrors.push("disabled V37-R2 control drifted from exact Supalosa");
        }
        control = {
            directSha256,
            disabledSha256,
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-v37-r2-exposure-${process.env.SLURM_ARRAY_JOB_ID ?? "local"}-${taskIndex}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-v37-r2-literal-endpoint-exposure",
            taskIndex,
            country,
            candidateSlot,
            requestedEngineSeed,
    ...
    ) throw new Error("V37-R2 literal-endpoint probe provenance failed");
    const passed = validationErrors.length === 0;
    const output = {
        schemaVersion: 1,
        gateRevision: "V37-R2",
        status: passed
            ? "PASS_OUTCOME_FREE_V37_R2_LITERAL_ENDPOINT_CELL"
            : "FAIL_OUTCOME_FREE_V37_R2_LITERAL_ENDPOINT_CELL",
        passed,
        outcomeFree: true,
    ...
    if (!passed) throw new Error("V37-R2 cell failed; preserved complete outcome-free artifact");
};
