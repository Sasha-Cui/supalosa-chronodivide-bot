...
const KIND = "passive-capability-handoff-complete-open-screen-v1" as const;
const STATUS = "FROZEN_PASSIVE_CAPABILITY_HANDOFF_COMPLETE_OPEN_SCREEN_V1" as const;
const COUNTRIES = [
...
type ArmId = "external_supalosa_control" | "unchanged_v5" | "v5_plus_early_distance" |
    "v5_plus_early_distance_air2" | "v5_plus_conservative_distance_air2" |
    "v5_plus_conservative_distance_air4";
type Arm = {
    armId: ArmId;
...
const commitmentFor = (arm: Omit<Arm, "policySha256">) => {
    ...
    const capability = buildProgressTriggeredCapabilityPolicy(arm.airTargetCount);
    return {
        architecture: arm.kind === "control" ? "exact_external_supalosa" :
            "exact_external_supalosa_then_optional_passive_capability_handoff_then_strict_v5",
        armId: arm.armId,
        v5: v5Policy,
        ...
        capability,
        capabilitySha256: progressTriggeredCapabilityPolicySha256(capability),
    };
};
const buildArms = (): Arm[] => ([
    { armId: "external_supalosa_control", kind: "control", priority: "distance", replacementEnabled: false,
        activationNotBeforeTick: 12_000, stagnationWindowTicks: 3_600, airTargetCount: 0 },
    ...
    { armId: "v5_plus_early_distance", kind: "replacement", priority: "distance", replacementEnabled: true,
        activationNotBeforeTick: 9_000, stagnationWindowTicks: 3_000, airTargetCount: 0 },
    { armId: "v5_plus_early_distance_air2", kind: "replacement", priority: "distance", replacementEnabled: true,
        activationNotBeforeTick: 9_000, stagnationWindowTicks: 3_000, airTargetCount: 2 },
    { armId: "v5_plus_conservative_distance_air2", kind: "replacement", priority: "distance", replacementEnabled: true,
        activationNotBeforeTick: 12_000, stagnationWindowTicks: 3_600, airTargetCount: 2 },
    { armId: "v5_plus_conservative_distance_air4", kind: "replacement", priority: "distance", replacementEnabled: true,
        activationNotBeforeTick: 12_000, stagnationWindowTicks: 3_600, airTargetCount: 4 },
...
type EpisodeResult = {
    ...
    capabilityTelemetry: ProgressTriggeredCapabilityTelemetry[];
};
...
const createCandidate = (
    ...
    return createProgressTriggeredCapabilityCandidate(
        ...
        buildProgressTriggeredReplacementPolicy({ enabled: arm.replacementEnabled,
            activationNotBeforeTick: arm.activationNotBeforeTick, stagnationWindowTicks: arm.stagnationWindowTicks,
            targetPriority: arm.priority }), buildProgressTriggeredCapabilityPolicy(arm.airTargetCount),
        { v5: (event) => v5Telemetry.push(event), replacement: (event) => replacementTelemetry.push(event),
            ...
            capability: (event) => capabilityTelemetry.push(event) },
    );
};
...
const runEpisode = async (campaign: Campaign, shard: Shard, arm: Arm, slot: 0 | 1,
    ...
    const capabilityTelemetry: ProgressTriggeredCapabilityTelemetry[] = [];
    const candidate = createCandidate(factory, candidateName, shard.country, arm, v5Telemetry,
        replacementTelemetry, attackFactoryTelemetry, capabilityTelemetry);
    ...
    return withSeededOfflineGame(cdapi, settings(shard.mapName, candidate, baseline, slot),
        ...
        async (game) => {
            ...
            return {
                ...
                v5Telemetry, replacementTelemetry, attackFactoryTelemetry, capabilityTelemetry,
            };
        });
};
...
const generate = async (): Promise<void> => {
    ...
    const requiredVariants = ["early_distance", "early_distance_air2", "conservative_distance_air2",
        "conservative_distance_air4"] as const;
    const completeExposure = exposure !== null && requiredVariants.every((id) => {
        ...
        return isRecord(row) && Number(row.replacement) > 0 && Number(row.missions) > 0 &&
            (id === "early_distance" || Number(row.capabilityRequests) > 0 && Number(row.capabilityStaged) > 0 &&
                Number(row.capabilityReleases) === 0);
    });
    if (!isRecord(compatibility) || compatibility.kind !==
        "passive-capability-handoff-outcome-blind-gate" || compatibility.passed !== true ||
        ...
        throw new Error("Compatibility gate did not pass every passive capability variant");
    }
    ...
    const manifest = createExperimentManifest({ runId: "passive-capability-handoff-open-screen-generator",
...
const validateResult = (value: unknown, campaign: Campaign, shard: Shard, arm: Arm, slot: 0 | 1): Observation => {
    if (!isRecord(value) || value.schemaVersion !== 1 || value.familyId !== shard.familyId ||
        ...
        !Array.isArray(value.attackFactoryTelemetry) || !Array.isArray(value.capabilityTelemetry)) {
    ...
    const capabilityEvents = value.capabilityTelemetry as ProgressTriggeredCapabilityTelemetry[];
    const v5 = value.v5Telemetry as TerminalObjectiveTelemetry[];
    if (arm.kind !== "replacement" && (replacement.length !== 0 || factoryEvents.length !== 0 || capabilityEvents.length !== 0)) {
    ...
    const capabilityPolicy = buildProgressTriggeredCapabilityPolicy(arm.airTargetCount);
    for (const event of capabilityEvents) if (event.schemaVersion !== 1 ||
        event.event === "capability_unit_release" || event.informationBoundary !== "public_complete_state" || event.country !== shard.country ||
        event.unitName !== progressTriggeredAirUnit(shard.country) ||
        event.targetCount !== capabilityPolicy.airTargetCount || !Array.isArray(event.releasedUnitIds) ||
        !Array.isArray(event.forbiddenFieldsEmitted) || event.forbiddenFieldsEmitted.length !== 0 ||
        JSON.stringify(event).match(/winner|loser|score|outcome|endpoint|resignation|evaluator/i)) {
        throw new Error("Capability telemetry validation failed");
    }
    return { armId: arm.armId, familyId: shard.familyId, country: shard.country, candidateSlot: slot,
        ...
        nonterminalDraw: winner === "draw" && ["tick_cap_draw", "engine_nonliteral_termination_draw"]
            .includes(String(value.outcomeStatus)), replacement, factory: factoryEvents, capability: capabilityEvents };
};
...
const finalize = (): void => {
    ...
    const evaluations = campaign.arms.filter(({ kind }) => kind === "replacement").map((arm) => {
        ...
        const capabilityRequests = armObservations.flatMap((row) => row.capability).filter((event) =>
            event.event === "capability_structure_request" || event.event === "capability_unit_request").length;
        const capabilityStaged = armObservations.flatMap((row) => row.capability)
            .filter((event) => event.event === "capability_unit_staged").length;
        const capabilityReleases = armObservations.flatMap((row) => row.capability)
            .filter((event) => event.event === "capability_unit_release").length;
        ...
        if (arm.airTargetCount > 0 && capabilityRequests === 0) eligibilityFailures.push("No capability request occurred");
        if (arm.airTargetCount > 0 && capabilityStaged === 0) eligibilityFailures.push("No capability unit was staged");
        if (arm.airTargetCount > 0 && capabilityReleases !== 0) eligibilityFailures.push("Explicit capability release occurred");
        return { arm, absolute, versusSupalosa, versusV5, replacementCount, missionCreations,
            capabilityRequests, capabilityStaged, capabilityReleases,
    ...
    const artifact = { schemaVersion: 1, kind: "passive-capability-handoff-complete-open-screen-finalizer-v1",
        ...
        artifactRows: observations.map(({ replacement: _replacement, factory: _factory, capability: _capability, ...row }) => row) };
