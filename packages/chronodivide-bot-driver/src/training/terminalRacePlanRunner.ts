...
import {
    TERMINAL_RACE_EPISODE_SCHEMA_VERSION,
    TerminalRaceEpisodeSpec,
    runTerminalRaceEpisode,
} from "./terminalRaceEpisode.js";
import {
    TerminalRacePolicy,
...
export const TERMINAL_RACE_PLAN_SCHEMA_VERSION = 2 as const;
export const TERMINAL_RACE_PLAN_KIND = "terminal-race-open-development-literal-endpoint" as const;

export type TerminalRacePlanArm = {
    armId: string;
    policyId: string;
    policy: TerminalRacePolicy;
};

export type TerminalRacePlanEpisode = {
    episodeId: string;
    armId: string;
    policyId: string;
    candidateSlot: 0 | 1;
...
export type TerminalRaceRunPlan = {
    schemaVersion: typeof TERMINAL_RACE_PLAN_SCHEMA_VERSION;
    kind: typeof TERMINAL_RACE_PLAN_KIND;
    runId: string;
    ...
    maxTicks: number;
    arms: TerminalRacePlanArm[];
    episodes: TerminalRacePlanEpisode[];
};
...
export const parseTerminalRaceRunPlan = (value: unknown): TerminalRaceRunPlan => {
    if (!isRecord(value)) throw new Error("Terminal-race run plan must be an object");
    exactKeys(value, [
    ...
    ], "Terminal-race run plan");
    if (value.schemaVersion !== TERMINAL_RACE_PLAN_SCHEMA_VERSION || value.kind !== TERMINAL_RACE_PLAN_KIND) {
        throw new Error("Terminal-race plan identity is invalid");
    }
    if (
    ...
    ) throw new Error("Terminal-race plan endpoint binding drifted");
    if (!isRecord(value.family)) throw new Error("Terminal-race plan family is invalid");
    exactKeys(value.family, ["familyId", "mapName", "mapSha256"], "Terminal-race family");
    ...
    if (mapName !== mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(mapName)) {
        throw new Error("Terminal-race mapName must be a map basename");
    }
    const country = value.country;
    if (typeof country !== "string" || !Object.values(Countries).includes(country as Countries)) {
        throw new Error("Terminal-race plan country is invalid");
    }
    if (!Array.isArray(value.arms) || value.arms.length < 1) throw new Error("Terminal-race plan has no arms");
    const arms = value.arms.map((raw, index): TerminalRacePlanArm => {
        if (!isRecord(raw)) throw new Error(`Terminal-race arm ${index} is invalid`);
        exactKeys(raw, ["armId", "policyId", "policy"], `Terminal-race arm ${index}`);
        ...
        const policy = validateTerminalRacePolicy(raw.policy as TerminalRacePolicy);
        if (terminalRacePolicySha256(policy) !== policyId) {
            throw new Error(`Terminal-race arm ${armId} policy hash drifted`);
        }
    ...
    ) throw new Error("Terminal-race plan arm identities must be unique");
    if (!Array.isArray(value.episodes)) throw new Error("Terminal-race plan episodes are invalid");
    const armById = new Map(arms.map((arm) => [arm.armId, arm]));
    const episodes = value.episodes.map((raw, index): TerminalRacePlanEpisode => {
        if (!isRecord(raw)) throw new Error(`Terminal-race episode ${index} is invalid`);
        exactKeys(raw, ["episodeId", "armId", "policyId", "candidateSlot"], `Terminal-race episode ${index}`);
        ...
        if ((raw.candidateSlot !== 0 && raw.candidateSlot !== 1) || armById.get(armId)?.policyId !== policyId) {
            throw new Error(`Terminal-race episode ${index} has an invalid arm or slot binding`);
        }
    ...
    ) throw new Error("Terminal-race plan episode count or identities are invalid");
    for (const arm of arms) {
        const slots = episodes.filter(({ armId }) => armId === arm.armId).map(({ candidateSlot }) => candidateSlot).sort();
        if (slots.join(",") !== "0,1") throw new Error(`Terminal-race arm ${arm.armId} lacks reciprocal slots`);
    }
    ...
    if (derivePairedEngineSeed(engineSeedBase, seedBlockIndex) !== requestedEngineSeed) {
        throw new Error("Terminal-race requested engine seed drifted from its paired derivation");
    }
    return {
...
export const serializeTerminalRaceRunPlan = (plan: TerminalRaceRunPlan): string =>
    `${JSON.stringify(parseTerminalRaceRunPlan(plan), null, 2)}\n`;
...
export const runTerminalRacePlanFromEnvironment = async (): Promise<void> => {
    const repoRoot = gitRoot();
    const driverRoot = path.join(repoRoot, "packages", "chronodivide-bot-driver");
    if (path.resolve(process.cwd()) !== driverRoot) throw new Error(`Terminal-race runner must start in ${driverRoot}`);
    const planPath = requireEnvPath("TERMINAL_RACE_PLAN");
    const outDir = requireEnvPath("OUT_DIR");
    if (fs.existsSync(outDir)) throw new Error(`Refusing to reuse Terminal-race OUT_DIR ${outDir}`);
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Terminal-race execution requires the pinned external baseline environment");
    }
    const planSha256 = sha256File(planPath);
    const plan = parseTerminalRaceRunPlan(JSON.parse(fs.readFileSync(planPath, "utf8")));
    ...
    const manifest = createExperimentManifest({
        ...
        effectiveConfig: {
            runner: "terminalRacePlanRunner-v1",
            planPath,
            planSha256,
    ...
    ) throw new Error("Terminal-race runtime, source, baseline, or scheduler provenance drifted");
    const runtimeMap = manifest.inputs.maps[0];
    if (!runtimeMap?.exists || runtimeMap.sha256 !== plan.family.mapSha256) {
        throw new Error("Terminal-race runtime map bytes drifted from the plan");
    }
    const policyFiles = [
    ...
    ) throw new Error("Terminal-race policy violates its declared complete-state or endpoint boundary");
    ...
    for (let launchIndex = 0; launchIndex < plan.episodes.length; launchIndex++) {
        ...
        const arm = armById.get(episode.armId) as TerminalRacePlanArm;
        const spec: TerminalRaceEpisodeSpec = {
        ...
        try {
            const result = await runTerminalRaceEpisode(spec, baselineFactory);
    ...
    const summary = {
        schemaVersion: 2,
        status: technicalFailures === 0 ? "COMPLETE_TERMINAL_RACE_OPEN_DEVELOPMENT_SHARD" : "FAILED_TERMINAL_RACE_TECHNICAL_SHARD",
...
if (import.meta.url === invoked) {
    runTerminalRacePlanFromEnvironment().catch((error) => {
        console.error(error);
        process.exitCode = 1;
    });
}
