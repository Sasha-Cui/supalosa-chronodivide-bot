...
export const PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_TECHNICAL_GATE_SCHEMA_VERSION = 1 as const;
...
export const parseProgressCertifiedV5RepairConfirmatorySacct = (
    raw: string,
    arrayJobId: string,
    ...
    for (const [lineIndex, line] of raw.split("\n").entries()) {
        ...
        if (
            ...
            taskIndex >= PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_SHARD_COUNT ||
    ...
    if (tasks.size !== PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_SHARD_COUNT) {
        throw new Error(
            `Confirmatory sacct returned ${tasks.size}/` +
            `${PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_SHARD_COUNT} tasks`,
        );
    }
    return tasks;
};
 ...
 */
export const validateProgressCertifiedV5RepairConfirmatoryEndpointTechnicalState = (
    value: unknown,
    expected: {
...
export const progressCertifiedV5RepairConfirmatoryResultCommitmentSha256 = (
...
const validateShard = (
    ...
    shard: ProgressCertifiedV5RepairConfirmatoryCampaign["shards"][number],
): string => {
    ...
    for (let launchIndex = 0; launchIndex < plan.episodes.length; launchIndex++) {
        ...
        validateProgressCertifiedV5RepairConfirmatoryEndpointTechnicalState(result, {
...
const main = (): void => {
    ...
    const schedulerTasks = parseProgressCertifiedV5RepairConfirmatorySacct(execFileSync(
    ...
    if (
        schedulerJobIds.length !== PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_SHARD_COUNT ||
        new Set(schedulerJobIds).size !== PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_SHARD_COUNT
    ) {
    ...
    const output = {
        schemaVersion: PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_TECHNICAL_GATE_SCHEMA_VERSION,
        status: "PASSED_PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_TECHNICAL_GATE_NO_OUTCOMES_INSPECTED",
        ...
        shardCount: PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_SHARD_COUNT,
        requestedLaunches: PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_LAUNCH_COUNT,
        accountedLaunches: PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_LAUNCH_COUNT,
        completedLaunches: PROGRESS_CERTIFIED_V5_REPAIR_CONFIRMATORY_LAUNCH_COUNT,
        technicalFailures: 0,
