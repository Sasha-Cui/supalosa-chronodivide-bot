...
const selectCases = async (): Promise<void> => {
                ...
                for (const candidateSlot of [0, 1] as const) {
                    ...
                    if (starts.candidateStart === desiredStart && starts.baselineStart === OPPOSITE[desiredStart]) {
                        selected = { caseIndex: cases.length, countryOrdinal, country, startOrdinal, desiredStart,
                            ...
                            ...starts, expectedActive: SOVIET.has(country) && desiredStart === "39,82" };
                        break;
                    }
                }
            }
    ...
    if (cases.length !== CASE_COUNT || new Set(cases.map((entry) =>
        ...
        cases.filter((entry) => entry.expectedActive).length !== 4) throw new Error("Isolation selection coverage drifted");
    const artifact = { schemaVersion: 1, kind: "hfo-soviet-west-isolation-selection",
        status: "PASS_HFO_SOVIET_WEST_ISOLATION_SELECTION", complete: true, passed: true, outcomeFree: true,
        ...
        selectedCaseCount: cases.length, activeCaseCount: 4, inactiveCaseCount: 32, forbiddenOutcomeFields: [], cases };
...
const loadSelection = (selectionPath: string, selectionSha256: string, inputs: ReturnType<typeof commonInputs>): CaseSpec[] => {
    ...
    if (!isRecord(selection) || selection.kind !== "hfo-soviet-west-isolation-selection" ||
        selection.status !== "PASS_HFO_SOVIET_WEST_ISOLATION_SELECTION" || selection.complete !== true ||
        selection.passed !== true || selection.outcomeFree !== true || selection.updateCount !== 0 ||
        selection.selectedCaseCount !== CASE_COUNT || selection.activeCaseCount !== 4 || selection.inactiveCaseCount !== 32 ||
...
const runArm = async (args: { factory: Awaited<ReturnType<typeof loadBaselineFactory>>; caseSpec: CaseSpec;
    ...
    const candidate = createDeployedStrongBotCandidate(candidateName, caseSpec.country,
        winner ? { hfoSovietWestProfile: true } : {}, winner ? { hfoWestHomeGuard: {
...
const finalize = (): void => {
    ...
    for (let taskIndex = 0; taskIndex < CASE_COUNT; taskIndex += 1) {
        ...
        if (cell.kind !== "hfo-soviet-west-isolation-cell" || cell.status !== "PASS_HFO_SOVIET_WEST_ISOLATION_CELL" ||
    ...
    const passed = active.length === 4 && inactive.length === 32 && countries.size === 9 && starts.size === 4 && sources.size === 1;
    const artifact = { schemaVersion: 1, kind: "hfo-soviet-west-activation-isolation-gate",
        status: passed ? "PASS_HFO_SOVIET_WEST_ACTIVATION_ISOLATION" : "FAIL_HFO_SOVIET_WEST_ACTIVATION_ISOLATION",
