...
export const validateMissionNativeCloseoutFocusedGateV25Telemetry = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
    country: Countries,
): void => {
    ...
    if (capability.length !== 1 || capability.some((event) => {
        ...
        const blocker = event.launchMode === "progressive_blocker" ||
        ...
        return event.schemaVersion !== 19 || event.targetId < 0 || event.compatibleAttackerCount < 2 ||
            ...
            (blocker && (event.blockerId === null || event.blockerName === null ||
                ...
                event.estimatedBlockerRemovalTicks < 0 || event.estimatedBlockerRemovalTicks > survival)) ||
            (!direct && !blocker);
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Focused V25 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("Focused V25 gate map bytes drifted");
    }
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const policy = buildMissionNativeCloseoutPolicyV25(true);
    await cdapi.init(path.join(process.cwd(), "data"));
    const rows: Array<Record<string, unknown>> = [];
    for (const [index, country] of MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V25_COUNTRIES.entries()) {
        ...
        for (const [label, trace] of [["first", first], ["repeat", repeat]] as const) {
            try {
                validateMissionNativeCloseoutFocusedGateV25Telemetry(trace.telemetry, country);
            } catch (error) {
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-focused-gate-v25-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-focused-gate-v25",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V25_SCHEMA_VERSION,
            policyId: missionNativeCloseoutPolicyV25Sha256(policy),
            countries: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V25_COUNTRIES,
            candidateSlot: 0,
        ...
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V25_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("Focused V25 gate provenance or coverage failed");
    const passed = rows.every((row) => row.passed === true);
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V25_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V25"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V25",
    ...
    if (!passed) throw new Error("Focused V25 gate failed its outcome-blind technical contract");
};
