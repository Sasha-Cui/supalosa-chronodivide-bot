...
export const TERMINAL_OBJECTIVE_EPISODE_SCHEMA_VERSION = 2 as const;

export type TerminalObjectiveEpisodeSpec = {
    schemaVersion: typeof TERMINAL_OBJECTIVE_EPISODE_SCHEMA_VERSION;
    episodeId: string;
    familyId: string;
    mapName: string;
    mapSha256: string;
...
export type TerminalObjectiveWinner = "candidate" | "baseline" | "draw";
export type TerminalObjectiveOutcomeStatus =
    | "candidate_win"
    | "baseline_win"
    | "simultaneous_draw"
...
export type TerminalObjectiveEpisodeResult = {
    schemaVersion: typeof TERMINAL_OBJECTIVE_EPISODE_SCHEMA_VERSION;
    episodeId: string;
    familyId: string;
    mapName: string;
    ...
    policyId: string;
    policySha256: string;
    policyInformationBoundary: typeof TERMINAL_OBJECTIVE_INFORMATION_BOUNDARY;
    ...
    wallTimeMs: number;
    shortGame: false;
    outcomeStatus: TerminalObjectiveOutcomeStatus;
    winner: TerminalObjectiveWinner | null;
...
export const validateTerminalObjectiveEpisodeSpec = (spec: TerminalObjectiveEpisodeSpec): TerminalObjectiveEpisodeSpec => {
    if (spec.schemaVersion !== TERMINAL_OBJECTIVE_EPISODE_SCHEMA_VERSION) {
        throw new Error(`Terminal-objective episode schemaVersion must be ${TERMINAL_OBJECTIVE_EPISODE_SCHEMA_VERSION}`);
    }
    ...
    if (spec.mapName !== spec.mapName.split(/[\\/]/).pop() || !/\.(map|mpr)$/i.test(spec.mapName)) {
        throw new Error(`Terminal-objective mapName must be a .map or .mpr basename; got ${spec.mapName}`);
    }
    if (!SHA256_PATTERN.test(spec.mapSha256)) throw new Error("Terminal-objective mapSha256 must be lowercase SHA-256");
    const policy = validateTerminalObjectivePolicy(spec.policy);
    if (terminalObjectivePolicySha256(policy) !== spec.policyId) {
        throw new Error("Terminal-objective policyId does not equal the canonical policy SHA-256");
    }
    if (!Number.isSafeInteger(spec.seedBlockIndex) || spec.seedBlockIndex < 0) {
        throw new Error("Terminal-objective seedBlockIndex must be a nonnegative integer");
    }
    if (
        !Number.isSafeInteger(spec.requestedEngineSeed) ||
        spec.requestedEngineSeed < 0 ||
        spec.requestedEngineSeed > 0xffff_ffff
    ) throw new Error("Terminal-objective requestedEngineSeed must be uint32");
    if (spec.candidateSlot !== 0 && spec.candidateSlot !== 1) {
        throw new Error("Terminal-objective candidateSlot must be 0 or 1");
    }
    if (!Object.values(Countries).includes(spec.candidateCountry)) {
        throw new Error("Terminal-objective candidateCountry is invalid");
    }
    if (!Object.values(Countries).includes(spec.baselineCountry)) {
        throw new Error("Terminal-objective baselineCountry is invalid");
    }
    if (!Number.isSafeInteger(spec.maxTicks) || spec.maxTicks < 1 || spec.maxTicks > 100_000) {
        throw new Error("Terminal-objective maxTicks must be in [1, 100000]");
    }
    return { ...spec, policy };
};
...
const score = (winner: TerminalObjectiveWinner | null): 0 | 0.5 | 1 | null =>
    winner === "candidate" ? 1 : winner === "baseline" ? 0 : winner === "draw" ? 0.5 : null;

/**
 * Runs one predeclared Terminal-objective episode. There is no retry or outcome shaping.
 ...
 */
export const runTerminalObjectiveEpisode = async (
    rawSpec: TerminalObjectiveEpisodeSpec,
    baselineFactory: BaselineFactory,
): Promise<TerminalObjectiveEpisodeResult> => {
    const spec = validateTerminalObjectiveEpisodeSpec(rawSpec);
    const startedAt = Date.now();
    const candidateName = `TerminalObjectiveCandidate_${spec.seedBlockIndex}_${spec.candidateSlot}`;
    const baselineName = `TerminalObjectiveBaseline_${spec.seedBlockIndex}_${spec.candidateSlot}`;
        ...
        async (game) => {
            ...
            const capDraw: LiteralEndpointCapDraw | null =
                !terminal && !technicalFailure && ticks >= spec.maxTicks
                    ...
                    : null;
            const outcomeStatus: TerminalObjectiveOutcomeStatus = technicalFailure
                ? "technical_failure"
                : terminal
                  ? terminal.status
                  : "tick_cap_draw";
            const winner: TerminalObjectiveWinner | null = technicalFailure
                ? null
                : terminal
                  ? terminal.winner
                  : "draw";
            ...
            return {
                ...
                policySha256: terminalObjectivePolicySha256(spec.policy),
                policyInformationBoundary: TERMINAL_OBJECTIVE_INFORMATION_BOUNDARY,
                endpointVersion: LITERAL_BUILDING_ELIMINATION_ENDPOINT_VERSION,
                endpointSha256: LITERAL_BUILDING_ELIMINATION_ENDPOINT_SHA256,
