...
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_ENGINE_SEED_BASE = 4_265_000_000 as const;
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_COUNTRIES = [Countries.USA, Countries.LIBYA] as const;
...
export const validateMissionNativeCloseoutFocusedGateV26Telemetry = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
    country: Countries,
): void => {
    ...
    if (capability[0].tick !== activated[0].tick || capability[0].tick < firstPhysical.tick) {
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Focused V26 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("Focused V26 gate map bytes drifted");
    }
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const policy = buildMissionNativeCloseoutPolicyV26(true);
    await cdapi.init(path.join(process.cwd(), "data"));
    const rows: Array<Record<string, unknown>> = [];
    for (const [index, country] of MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_COUNTRIES.entries()) {
        const requestedEngineSeed = derivePairedEngineSeed(
            MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_ENGINE_SEED_BASE,
            index,
        );
        ...
        for (const [label, trace] of [["first", first], ["repeat", repeat]] as const) {
            try {
                validateMissionNativeCloseoutFocusedGateV26Telemetry(trace.telemetry, country);
            } catch (error) {
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-focused-gate-v26-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-focused-gate-v26",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_SCHEMA_VERSION,
            policyId: missionNativeCloseoutPolicyV26Sha256(policy),
            countries: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_COUNTRIES,
            candidateSlot: 0,
        ...
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("Focused V26 gate provenance or coverage failed");
    const passed = rows.every((row) => row.passed === true);
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26",
    ...
    if (!passed) throw new Error("Focused V26 gate failed its outcome-blind technical contract");
};
