...
import {
    progressCertifiedV5ConfirmatoryResultCommitmentSha256,
} from "./progressCertifiedV5ConfirmatoryTechnicalGate.js";

export const PROGRESS_CERTIFIED_V5_CONFIRMATORY_UNBLINDING_SCHEMA_VERSION = 1 as const;

type RecordValue = Record<string, unknown>;
type ArmId = "external_supalosa_control" | "final_building_hybrid_v4" | "visibility_aware_final_building_v5";
export type ProgressCertifiedV5ConfirmatoryAnalysisRow = {
    familyId: string;
    country: string;
...
type Row = ProgressCertifiedV5ConfirmatoryAnalysisRow;
...
const oneSidedFamilyInference = (values: number[], estimand: string) => {
    if (values.length !== 56) throw new Error(`${estimand} requires exactly 56 family observations`);
    ...
    const lowerBound = standardError === null
        ? null
        : estimate - PROGRESS_CERTIFIED_V5_CONFIRMATORY_ONE_SIDED_95_T_CRITICAL_DF55 * standardError;
    return {
        estimand,
        unit: "family",
        familyCount: 56,
...
export const analyzeProgressCertifiedV5Confirmation = (rows: Row[]) => {
    if (rows.length !== 3_024) throw new Error("V5 confirmation requires exactly 3024 validated outcomes");
    const families = [...new Set(rows.map(({ familyId }) => familyId))];
    const countries = [...new Set(rows.map(({ country }) => country))];
    if (families.length !== 56 || countries.length !== 9) throw new Error("V5 confirmation population is incomplete");
    ...
    return {
        status: primaryPassed
            ? "PASSED_PROGRESS_CERTIFIED_V5_CONFIRMATORY_SUCCESS_GATE"
            : "FAILED_PROGRESS_CERTIFIED_V5_CONFIRMATORY_SUCCESS_GATE",
        primaryPassed,
...
const validateTechnicalGate = (
    ...
    if (
        ...
        value.status !== "PASSED_PROGRESS_CERTIFIED_V5_CONFIRMATORY_TECHNICAL_GATE_NO_OUTCOMES_INSPECTED" ||
        ...
        value.shardCount !== 504 || value.requestedLaunches !== 3_024 ||
        value.accountedLaunches !== 3_024 || value.completedLaunches !== 3_024 ||
        ...
        value.resultArtifactCommitmentSha256 !== progressCertifiedV5ConfirmatoryResultCommitmentSha256(campaign, resultsRoot, arrayJobId) ||
        !Array.isArray(value.schedulerJobIds) || value.schedulerJobIds.length !== 504 ||
        new Set(value.schedulerJobIds).size !== 504 ||
...
const main = (): void => {
    ...
    const output = {
        schemaVersion: PROGRESS_CERTIFIED_V5_CONFIRMATORY_UNBLINDING_SCHEMA_VERSION,
