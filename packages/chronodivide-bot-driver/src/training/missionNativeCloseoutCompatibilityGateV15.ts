...
export const MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_ENGINE_SEED_BASE = 4_110_000_000 as const;
export const MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_RUNS_PER_COUNTRY_SLOT = 4 as const;
...
export const summarizeMissionNativeCloseoutV15 = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
    ...
    const readinessBlocks = eventsOf(telemetry, "activation_blocked")
        .filter(({ reason }) => reason === "no_viable_staged_blocker_clearance");
    ...
    return {
        ...
        stagingShortfallEvaluationCount: activationEvaluations.filter((event) =>
            Number(event.totalCompatibleAttackerCount ?? event.compatibleAttackerCount) >
...
export const validateMissionNativeCloseoutV15Exposure = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
    country: Countries,
    slot: 0 | 1,
): void => {
    ...
    ) throw new Error(`V15 activation contract drifted for ${country} slot ${slot}`);
    ...
    if (passingEvaluationIndex < 0 || passingEvaluationIndex > activationIndex) {
        throw new Error(`V15 activation lacked a preceding certificate for ${country} slot ${slot}`);
    }
    for (const evaluation of evaluations) {
        ...
        ) throw new Error(`V15 blocker-readiness certificate drifted for ${country} slot ${slot}`);
        if (
            evaluation.phase === "building_ready" && evaluation.blockerId !== null
        ) throw new Error(`V15 building-readiness certificate drifted for ${country} slot ${slot}`);
        if (
            evaluation.schemaVersion !== 9 ||
            ...
            evaluation.compatibleAttackerCount !== stagedCompatibleAttackerCount
        ) throw new Error(`V15 staging certificate drifted for ${country} slot ${slot}`);
    }
    const activationBlocks = eventsOf(telemetry, "activation_blocked") as ActivationBlocked[];
    if (activationBlocks.some(({ reason, tick }) =>
        (reason !== "no_viable_staged_blocker_clearance" && reason !== "insufficient_own_combatants") ||
        tick > activations[0].tick)) {
        throw new Error(`V15 emitted an unfrozen activation block for ${country} slot ${slot}`);
    }
    ...
    ) throw new Error(`V15 staging lifecycle drifted for ${country} slot ${slot}`);
    ...
    ) throw new Error(`V15 target focus drifted for ${country} slot ${slot}`);
    if (orders.some(({ tick }) => tick < activations[0].tick)) {
        throw new Error(`V15 issued closeout orders before takeover for ${country} slot ${slot}`);
    }
    const handoffs = eventsOf(telemetry, "launch_handoff") as LaunchHandoff[];
    if (handoffs.length !== 1) throw new Error(`V15 launch handoff count drifted for ${country} slot ${slot}`);
    ...
    ) throw new Error(`V15 launch handoff did not reconcile for ${country} slot ${slot}`);
    ...
    if (decisions.length === 0 || allocations.length === 0) {
        throw new Error(`V15 engagement was not exposed for ${country} slot ${slot}`);
    }
    const contactCommitted = new Set<string>();
    for (const decision of decisions) {
        if (decision.reason === "route_interception_wins") {
            const key = `${decision.targetId}:${decision.blockerId}`;
            if (
                ...
                decision.earliestRouteThreatInterceptTicks === null ||
                (decision.earliestRouteThreatInterceptTicks > 0 && !contactCommitted.has(key))
            ) throw new Error(`V15 blocker certificate drifted for ${country} slot ${slot}`);
            if (decision.earliestRouteThreatInterceptTicks === 0) contactCommitted.add(key);
        } else if (decision.reason === "objective_advance") {
            if (
                decision.phase !== "building_strike" || decision.blockerId !== null ||
                decision.routeThreatCount <= 0 || decision.earliestRouteThreatInterceptTicks === null ||
                decision.earliestRouteThreatInterceptTicks <= 0 ||
                decision.estimatedBuildingCompletionTicks === null ||
                decision.estimatedForceSurvivalTicks === null ||
                decision.estimatedBuildingCompletionTicks <= decision.estimatedForceSurvivalTicks
            ) throw new Error(`V15 objective-advance certificate drifted for ${country} slot ${slot}`);
        } else if (
            decision.phase === "blocker_clear" || decision.blockerId !== null ||
            (decision.reason === "building_completion_race" &&
                (decision.estimatedBuildingCompletionTicks === null ||
                    decision.estimatedForceSurvivalTicks === null ||
                    decision.estimatedBuildingCompletionTicks > decision.estimatedForceSurvivalTicks))
        ) throw new Error(`V15 building certificate drifted for ${country} slot ${slot}`);
    }
    for (const allocation of allocations) {
        if (
            ...
            (allocation.blockerId !== null && allocation.blockerAttackerCount <= 0)
        ) throw new Error(`V15 phase allocation drifted for ${country} slot ${slot}`);
    }
    ...
    if (!heartbeats.some(({ buildingAttackerCount }) => buildingAttackerCount > 0)) {
        throw new Error(`V15 never advanced the building objective for ${country} slot ${slot}`);
    }
    const progress = eventsOf(telemetry, "target_progress") as Progress[];
    if (!progress.some(({ damage }) => damage > 0)) {
        throw new Error(`V15 caused no physical building damage for ${country} slot ${slot}`);
    }
    if (telemetry.some(({ event }) => event === "sweep_orders" || event === "capability_production")) {
        throw new Error(`Unfrozen V15 mechanism executed for ${country} slot ${slot}`);
    }
    if (telemetry.some(({ tick }) => !Number.isSafeInteger(tick) || tick < 0)) {
        throw new Error(`V15 telemetry tick drifted for ${country} slot ${slot}`);
    }
};
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Mission-native V15 compatibility requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("Mission-native V15 compatibility map bytes drifted");
    }
    const factory = await loadBaselineFactory(path.join(repoRoot, "packages", "chronodivide-bot"));
    const disabledPolicy = buildMissionNativeCloseoutPolicyV15(false);
    const enabledPolicy = buildMissionNativeCloseoutPolicyV15(true);
    ...
    for (const country of Object.values(Countries)) for (const candidateSlot of [0, 1] as const) {
        const requestedEngineSeed = derivePairedEngineSeed(
            MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_ENGINE_SEED_BASE,
            index++,
        );
        ...
        if (directDigest !== disabledDigest) {
            validationErrors.push(`Disabled V15 adapter drifted for ${country} slot ${candidateSlot}`);
        }
        if (disabled.telemetry.length !== 0) {
            validationErrors.push(`Disabled V15 adapter emitted telemetry for ${country} slot ${candidateSlot}`);
        }
        for (const [label, trace] of [["first", first], ["repeat", repeat]] as const) {
            try {
                validateMissionNativeCloseoutV15Exposure(trace.telemetry, country, candidateSlot);
            } catch (error) {
        ...
        if (firstDigest !== repeatDigest) {
            validationErrors.push(`V15 trace was not deterministic for ${country} slot ${candidateSlot}`);
        }
        if (digest(first.actions) === digest(direct.actions)) {
            validationErrors.push(`V15 did not change commands for ${country} slot ${candidateSlot}`);
        }
        rows.push({
            country,
            candidateSlot,
            ...
            summary: summarizeMissionNativeCloseoutV15(first.telemetry),
            repeatSummary: summarizeMissionNativeCloseoutV15(repeat.telemetry),
            quitAttempts: {
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-compatibility-v15-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-contact-clearance-closeout-v15",
            gateSchemaVersion: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_SCHEMA_VERSION,
            disabledPolicyId: missionNativeCloseoutPolicyV15Sha256(disabledPolicy),
            enabledPolicyId: missionNativeCloseoutPolicyV15Sha256(enabledPolicy),
            countries: Object.values(Countries),
            reciprocalSlots: [0, 1],
            runsPerCountrySlot: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_RUNS_PER_COUNTRY_SLOT,
            maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
            outcomeInspection: false,
        },
        baseline: factory.descriptor,
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("Mission-native V15 compatibility provenance or coverage failed");
    ...
    if (totals("pureBlockerAllocationCount") <= 0) {
        globalValidationErrors.push("V15 never exposed a phase-pure blocker allocation");
    }
    if (totals("pureBuildingAllocationCount") <= 0) {
        globalValidationErrors.push("V15 never exposed a phase-pure building allocation");
    }
    if (totals("objectiveAdvanceDecisionCount") <= 0) {
        globalValidationErrors.push("V15 never exposed a positive-distance objective advance");
    }
    if (totals("contactClearDecisionCount") <= 0) {
        globalValidationErrors.push("V15 never exposed a zero-intercept contact clearance");
    }
    if (totals("persistentBlockerHeartbeats") <= 0) {
        globalValidationErrors.push("V15 never preserved a blocker across consecutive heartbeats");
    }
    if (totals("readinessBlockedCount") <= 0) {
        globalValidationErrors.push("V15 never exposed a readiness-blocked takeover event");
    }
    if (totals("readinessReserveCreatedCount") <= 0) {
        globalValidationErrors.push("V15 never created a staging mission");
    }
    if (totals("readinessReserveReleasedCount") <= 0) {
        globalValidationErrors.push("V15 never released staging before takeover");
    }
    if (totals("readinessReservePositiveGrowth") <= 0) {
        globalValidationErrors.push("V15 never accumulated a positive staged force");
    }
    if (totals("blockerReadyEvaluationCount") <= 0) {
        globalValidationErrors.push("V15 never activated through certified first-blocker clearance");
    }
    if (totals("stagingShortfallEvaluationCount") <= 0) {
        globalValidationErrors.push("V15 never exposed a staging-ownership shortfall");
    }
    if (!reinforcementTargetNames.some((name) => name === "GAPILE" || name === "GAWEAP")) {
        globalValidationErrors.push("V15 never selected an Allied reinforcement source");
    }
    if (!reinforcementTargetNames.some((name) => name === "NAHAND" || name === "NAWEAP")) {
        globalValidationErrors.push("V15 never selected a Soviet reinforcement source");
    }
    const passed = rows.every((row) => row.passed === true) && globalValidationErrors.length === 0;
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15"
        ...
        gameCount: rows.length * MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_V15_RUNS_PER_COUNTRY_SLOT,
        maxTicks: MISSION_NATIVE_CLOSEOUT_COMPATIBILITY_MAX_TICKS,
        aggregate: {
    ...
    if (!passed) throw new Error("Mission-native compatibility-v15 failed; preserved outcome-free artifact");
};
