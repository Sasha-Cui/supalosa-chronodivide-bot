...
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_ENGINE_SEED_BASE = 4_240_000_000 as const;
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_COUNTRIES = [Countries.USA, Countries.LIBYA] as const;
...
export const validateMissionNativeCloseoutFocusedGateV23Telemetry = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
    country: Countries,
): void => {
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Focused V23 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("Focused V23 gate map bytes drifted");
    }
    ...
    for (const [index, country] of MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_COUNTRIES.entries()) {
        const requestedEngineSeed = derivePairedEngineSeed(
            MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_ENGINE_SEED_BASE,
            index,
        );
        ...
        for (const [label, trace] of [["first", first], ["repeat", repeat]] as const) {
            try {
                validateMissionNativeCloseoutFocusedGateV23Telemetry(trace.telemetry, country);
            } catch (error) {
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-focused-gate-v23-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-focused-gate-v23",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_SCHEMA_VERSION,
            policyId: missionNativeCloseoutPolicyV23Sha256(policy),
            countries: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_COUNTRIES,
            candidateSlot: 0,
        ...
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("Focused V23 gate provenance or coverage failed");
    const passed = rows.every((row) => row.passed === true);
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V23",
    ...
    if (!passed) throw new Error("Focused V23 gate failed its outcome-blind technical contract");
};
