...
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2_SCHEMA_VERSION = 1 as const;
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2_ENGINE_SEED_BASE = 4_291_000_000 as const;
export const MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2_COUNTRIES = [Countries.USA, Countries.LIBYA] as const;
...
export const validateMissionNativeCloseoutFocusedGateV28R2Telemetry = (
    telemetry: readonly BuildingEliminationTelemetryEvent[],
    country: Countries,
): void => {
...
const main = async (): Promise<void> => {
    ...
    if (!process.env.BASELINE_PACKAGE_ROOT || process.env.REQUIRE_EXTERNAL_BASELINE !== "true") {
        throw new Error("Focused V28-R2 gate requires the pinned external baseline");
    }
    const mapName = "simple-1v1-no-preview.map";
    if (sha256File(path.join(process.cwd(), "data", mapName)) !== METHOD_V5_EQUIVALENCE_MAP_SHA256) {
        throw new Error("Focused V28-R2 gate map bytes drifted");
    }
    ...
    for (const [index, country] of MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2_COUNTRIES.entries()) {
        ...
        for (const [label, trace] of [["first", first], ["repeat", repeat]] as const) {
            try {
                validateMissionNativeCloseoutFocusedGateV28R2Telemetry(trace.telemetry, country);
            } catch (error) {
    ...
    const manifest = createExperimentManifest({
        runId: `mission-native-closeout-focused-gate-v28-r2-${process.env.SLURM_JOB_ID ?? "local"}`,
        mixDir: path.join(process.cwd(), "data"),
        maps: [mapName],
        effectiveConfig: {
            purpose: "outcome-free-mission-native-closeout-focused-gate-v28-r2",
            schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2_SCHEMA_VERSION,
            policyId: missionNativeCloseoutPolicyV28Sha256(policy),
            countries: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2_COUNTRIES,
            candidateSlot: 0,
        ...
        gameSeedBase: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2_ENGINE_SEED_BASE,
    });
    if (
    ...
    ) throw new Error("Focused V28-R2 gate provenance or coverage failed");
    ...
    const output = {
        schemaVersion: MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2_SCHEMA_VERSION,
        status: passed
            ? "PASS_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2"
            : "FAIL_OUTCOME_FREE_MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R2",
    ...
    if (!passed) throw new Error("Focused V28-R2 gate failed its outcome-blind technical contract");
};
