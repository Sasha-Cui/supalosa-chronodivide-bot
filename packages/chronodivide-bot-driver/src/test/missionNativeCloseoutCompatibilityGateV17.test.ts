...
import {
    summarizeMissionNativeCloseoutV17,
    validateMissionNativeCloseoutV17Exposure,
} from "../training/missionNativeCloseoutCompatibilityGateV17.js";
...
describe("mission-native closeout compatibility v17", () => {
    it("validates blocker persistence followed by a physical building phase", () => {
        const telemetry = validTelemetry();
        expect(() => validateMissionNativeCloseoutV17Exposure(
            telemetry,
            Countries.USA,
            0,
        )).not.toThrow();
        expect(summarizeMissionNativeCloseoutV17(telemetry)).toMatchObject({
            buildingDamage: 10,
