...
} from "../training/missionNativeCloseoutAllCountryGateV35.js";

const validRows = (): MissionNativeCloseoutAllCountryV35CoverageRow[] =>
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V35_COUNTRIES.flatMap((country) =>
        ([0, 1] as const).map((candidateSlot) => ({
            ...
            physicalProgressEventCount: 2,
            buildingPhysicalProgressEventCount: 1,
            blockerPhysicalProgressEventCount: 1,
            fallbackStartedCount: 1,
            fallbackActiveCount: 1,
            replanStartedCount: 1,
            activePredecessorFallbackEventCount: 1,
            overlaySuspensionEventCount: 1,
        })),
    );
...
describe("mission-native closeout all-country gate v35", () => {
    it("freezes all nine countries and both reciprocal slots", () => {
        ...
        expect(validateMissionNativeCloseoutAllCountryV35Coverage(validRows())).toEqual([]);
    });
    ...
    it("fails closed on destructive reservation or missing ready screen infrastructure", () => {
        const reservation = validRows().map((row, index) => index === 0
            ...
            : row);
        expect(validateMissionNativeCloseoutAllCountryV35Coverage(reservation)).toContain(
            "V35 emitted destructive production-reservation telemetry",
        );
        ...
        expect(validateMissionNativeCloseoutAllCountryV35Coverage(noSovietInfrastructure)).toContain(
    ...
    it("accepts a full-force terminal strike and a bounded nonterminal blocker screen", () => {
        expect(() => validateMissionNativeCloseoutV35ObjectiveRaceTelemetry([
            {
    ...
    it("permits matched control resignations but forbids enabled-policy resignations", () => {
        expect(validateMissionNativeCloseoutV35QuitAudits(
    ...
    it("accepts certified damage followed by exact bounded fallback and replanning", () => {
        expect(() => validateMissionNativeCloseoutV35ProgressTelemetry([
            {
                schemaVersion: 28, event: "objective_physical_progress", tick: 100,
                progressKind: "building_damage", targetId: 10, blockerId: null,
                objectId: 10, damage: 20, lastCertifiedProgressTick: 100,
            },
            {
            ...
            },
            {
                ...
                blockerId: null, lastCertifiedProgressTick: 100, deadlineTicks: 300,
                fallbackUntilTick: 580, releasedUnitIds: [1, 2],
                suspendedOverlayMissionNames: ["buildingEliminationAssaultProduction"],
                activePredecessorMissionNames: ["attack"],
            },
            {
                ...
                blockerId: null, lastCertifiedProgressTick: 100, deadlineTicks: 300,
                fallbackUntilTick: 580, releasedUnitIds: [1, 2],
                suspendedOverlayMissionNames: [], activePredecessorMissionNames: ["attack"],
            },
        ] as any)).not.toThrow();
    });

    it("rejects command-only delay and an early or missing replan", () => {
        expect(() => validateMissionNativeCloseoutV35ProgressTelemetry([
            {
                schemaVersion: 29, event: "objective_progress_deadline", tick: 339,
                phase: "fallback_started", reason: "blocker_no_progress", targetId: 10,
                blockerId: 20, lastCertifiedProgressTick: 100, deadlineTicks: 240,
                fallbackUntilTick: 519, releasedUnitIds: [1],
                suspendedOverlayMissionNames: [], activePredecessorMissionNames: [],
            },
        ] as any)).toThrow("frozen deadline contract");
        expect(() => validateMissionNativeCloseoutV35ProgressTelemetry([
            {
                schemaVersion: 29, event: "objective_progress_deadline", tick: 340,
                phase: "fallback_started", reason: "blocker_no_progress", targetId: 10,
                blockerId: 20, lastCertifiedProgressTick: 100, deadlineTicks: 240,
                fallbackUntilTick: 520, releasedUnitIds: [1],
                suspendedOverlayMissionNames: [], activePredecessorMissionNames: [],
            },
            {
                schemaVersion: 29, event: "objective_progress_deadline", tick: 340,
                phase: "fallback_active", reason: "blocker_no_progress", targetId: 10,
                blockerId: 20, lastCertifiedProgressTick: 100, deadlineTicks: 240,
                fallbackUntilTick: 520, releasedUnitIds: [1],
                suspendedOverlayMissionNames: ["buildingElimination"],
                activePredecessorMissionNames: ["attack"],
            },
