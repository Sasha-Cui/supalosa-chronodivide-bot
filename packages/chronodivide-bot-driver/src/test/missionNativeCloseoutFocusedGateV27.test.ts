...
import {
    MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V27_COUNTRIES,
    MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V27_ENGINE_SEED_BASE,
    validateMissionNativeCloseoutFocusedGateV27Telemetry,
} from
    "../training/missionNativeCloseoutFocusedGateV27.js";
...
const valid = (): BuildingEliminationTelemetryEvent[] => [
    ...
    {
        schemaVersion: 21, event: "activation_evaluation", tick: 3_620,
        phase: "blocker_ready", targetId: 10, targetName: "NAPOWR",
        blockerId: 20, blockerName: "E1", compatibleAttackerCount: 3,
        totalCompatibleAttackerCount: 3, transferCertifiedAttackerCount: 3,
        stagedCompatibleAttackerCount: 2, vanguardCompatibleAttackerCount: 1,
        assaultTankCount: 1, assaultScreenCount: 2, transferredCapabilityReady: true,
        enemyBuildingCount: 5, activationScopeLatched: true, routeThreatCount: 2,
        estimatedBuildingCompletionTicks: 80, estimatedForceSurvivalTicks: 30,
        estimatedBlockerRemovalTicks: 2, estimatedRouteClearanceTicks: 100,
    },
    {
...
describe("mission-native closeout focused gate v27", () => {
    it("keeps every focused seed inside the engine uint32 domain", () => {
        for (const index of MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V27_COUNTRIES.keys()) {
    ...
    it("requires factory-triggered readiness ownership, persistent tanks, and building damage", () => {
        expect(() => validateMissionNativeCloseoutFocusedGateV27Telemetry(valid(), Countries.USA)).not.toThrow();
    });
    ...
    it("fails closed if production telemetry ends on tank acquisition", () => {
        ...
        expect(() => validateMissionNativeCloseoutFocusedGateV27Telemetry(
    ...
    it("accepts a heartbeat sampling gap when the same-tick certificate owns the screen", () => {
    ...
    it("fails closed when screen production exceeds the bounded target allowance", () => {
        const telemetry = valid().map((event) => event.event === "assault_screen_production" &&
            event.tick === 3_610
            ? { ...event, currentCount: 6 }
            : event);
        expect(() => validateMissionNativeCloseoutFocusedGateV27Telemetry(
            telemetry, Countries.USA,
        )).toThrow(/Unbounded E1/);
    });

    it("fails closed when a readiness certificate has no transferred tank", () => {
        ...
        expect(() => validateMissionNativeCloseoutFocusedGateV27Telemetry(
            telemetry, Countries.USA,
        )).toThrow(/combined-arms certificate/);
    });

    it("fails closed when a readiness certificate has no transferred screen", () => {
        const telemetry = valid().map((event) => event.event === "activation_evaluation"
            ? {
                ...event,
                assaultScreenCount: 0,
                transferredCapabilityReady: false,
            }
            : event);
        expect(() => validateMissionNativeCloseoutFocusedGateV27Telemetry(
    ...
    it("fails closed when activation scope is not latched", () => {
        const telemetry = valid().map((event) => event.event === "activation_evaluation"
            ? { ...event, activationScopeLatched: false }
            : event);
        expect(() => validateMissionNativeCloseoutFocusedGateV27Telemetry(
            telemetry, Countries.USA,
        )).toThrow(/schema-21/);
    ...
    it("accepts a matched positive-progress sortie when one wave cannot finish the blocker", () => {
        const telemetry = valid()
            .filter((event) => event.event !== "progressive_blocker_launch")
            .map((event) => event.event === "assault_capability_launch"
                ? {
                    ...event,
                    launchMode: "attritional_blocker" as const,
                    estimatedBlockerRemovalTicks: 80,
                }
                : event);
        telemetry.push({
            schemaVersion: 20,
            ...
            blockerId: 20,
            blockerName: "E1",
            blockerIsStatic: false,
            compatibleAttackerCount: 3,
            readinessTankCount: 1,
            readinessScreenCount: 2,
            estimatedBlockerApproachTicks: 20,
            estimatedBlockerRemovalTicks: 80,
