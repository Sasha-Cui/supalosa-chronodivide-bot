...
import {
    summarizeMissionNativeCloseoutV12,
    validateMissionNativeCloseoutV12Exposure,
} from "../training/missionNativeCloseoutCompatibilityGateV12.js";
...
const validTelemetry = (): BuildingEliminationTelemetryEvent[] => [
    ...
    },
    {
        ...
        tick: 2_700,
        reason: "no_viable_transferable_route_clearance",
        ownCombatants: 2,
        enemyCombatants: 8,
...
describe("mission-native closeout compatibility v12", () => {
    it("validates blocker persistence followed by a physical building phase", () => {
        const telemetry = validTelemetry();
        expect(() => validateMissionNativeCloseoutV12Exposure(
            telemetry,
            Countries.USA,
            0,
        )).not.toThrow();
        expect(summarizeMissionNativeCloseoutV12(telemetry)).toMatchObject({
            buildingDamage: 10,
            ...
            transferCertificationShortfallEvaluationCount: 1,
            maximumTransferCertificationShortfall: 1,
    ...
    it("fails closed when transfer-certified force exceeds the visible compatible force", () => {
        const telemetry = validTelemetry();
        const evaluation = telemetry.find((event) =>
            event.event === "activation_evaluation" && event.phase === "blocker_ready",
        );
        ...
        evaluation.transferCertifiedAttackerCount = 3;
        expect(() => validateMissionNativeCloseoutV12Exposure(
            telemetry,
            Countries.USA,
            0,
        )).toThrow(/transfer certificate/);
    });
});
