...
import {
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V32_COUNTRIES,
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V32_ENGINE_SEED_BASE,
    MissionNativeCloseoutAllCountryV32CoverageRow,
    validateMissionNativeCloseoutAllCountryV32Coverage,
} from "../training/missionNativeCloseoutAllCountryGateV32.js";

const validRows = (): MissionNativeCloseoutAllCountryV32CoverageRow[] =>
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V32_COUNTRIES.flatMap((country) =>
...
describe("mission-native closeout all-country gate v32", () => {
    it("freezes all nine countries and both reciprocal slots", () => {
        expect(MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V32_COUNTRIES).toEqual([
            Countries.USA,
            Countries.KOREA,
            Countries.FRANCE,
            Countries.GERMANY,
        ...
        expect(validateMissionNativeCloseoutAllCountryV32Coverage(validRows())).toEqual([]);
    });
    ...
    it("fails closed on a missing reciprocal cell", () => {
        ...
        expect(validateMissionNativeCloseoutAllCountryV32Coverage(rows)).toEqual(expect.arrayContaining([
    ...
    it("fails closed on destructive reservation or missing ready screen infrastructure", () => {
        const reservation = validRows().map((row, index) => index === 0
            ...
            : row);
        expect(validateMissionNativeCloseoutAllCountryV32Coverage(reservation)).toContain(
            "V32 emitted destructive production-reservation telemetry",
        );
        ...
        expect(validateMissionNativeCloseoutAllCountryV32Coverage(noSovietInfrastructure)).toContain(
    ...
    it("does not require construction when side-correct screen infrastructure already exists", () => {
        ...
        expect(validateMissionNativeCloseoutAllCountryV32Coverage(rows)).toEqual(
            [],
        );
    });
});
