...
import {
    summarizeMissionNativeCloseoutV14,
    validateMissionNativeCloseoutV14Exposure,
} from "../training/missionNativeCloseoutCompatibilityGateV14.js";
...
describe("mission-native closeout compatibility v14", () => {
    it("validates blocker persistence followed by a physical building phase", () => {
        const telemetry = validTelemetry();
        expect(() => validateMissionNativeCloseoutV14Exposure(
            telemetry,
            Countries.USA,
            0,
        )).not.toThrow();
        expect(summarizeMissionNativeCloseoutV14(telemetry)).toMatchObject({
            buildingDamage: 10,
