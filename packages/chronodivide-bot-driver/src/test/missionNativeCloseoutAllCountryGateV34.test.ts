...
import {
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES,
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_ENGINE_SEED_BASE,
    MissionNativeCloseoutAllCountryV34CoverageRow,
    validateMissionNativeCloseoutAllCountryV34Coverage,
    validateMissionNativeCloseoutV34ObjectiveRaceTelemetry,
} from "../training/missionNativeCloseoutAllCountryGateV34.js";

const validRows = (): MissionNativeCloseoutAllCountryV34CoverageRow[] =>
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES.flatMap((country) =>
...
describe("mission-native closeout all-country gate v34", () => {
    it("freezes all nine countries and both reciprocal slots", () => {
        expect(MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V34_COUNTRIES).toEqual([
            Countries.USA,
            Countries.KOREA,
            Countries.FRANCE,
            Countries.GERMANY,
        ...
        expect(validateMissionNativeCloseoutAllCountryV34Coverage(validRows())).toEqual([]);
    });
    ...
    it("fails closed on a missing reciprocal cell", () => {
        ...
        expect(validateMissionNativeCloseoutAllCountryV34Coverage(rows)).toEqual(expect.arrayContaining([
    ...
    it("fails closed on destructive reservation or missing ready screen infrastructure", () => {
        const reservation = validRows().map((row, index) => index === 0
            ...
            : row);
        expect(validateMissionNativeCloseoutAllCountryV34Coverage(reservation)).toContain(
            "V34 emitted destructive production-reservation telemetry",
        );
        ...
        expect(validateMissionNativeCloseoutAllCountryV34Coverage(noSovietInfrastructure)).toContain(
    ...
    it("accepts a full-force terminal strike and a bounded nonterminal blocker screen", () => {
        expect(() => validateMissionNativeCloseoutV34ObjectiveRaceTelemetry([
            {
    ...
    it("does not require construction when side-correct screen infrastructure already exists", () => {
        ...
        expect(validateMissionNativeCloseoutAllCountryV34Coverage(rows)).toEqual(
            [],
        );
    });
});
