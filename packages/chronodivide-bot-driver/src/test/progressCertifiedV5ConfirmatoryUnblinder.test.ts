...
import {
    ProgressCertifiedV5ConfirmatoryAnalysisRow,
    analyzeProgressCertifiedV5Confirmation,
} from "../training/progressCertifiedV5ConfirmatoryUnblinder.js";
import { PROGRESS_CERTIFIED_V5_CONFIRMATORY_COUNTRIES } from
    "../training/progressCertifiedV5ConfirmatoryCampaign.js";
...
const rows = (positive: boolean): ProgressCertifiedV5ConfirmatoryAnalysisRow[] =>
    Array.from({ length: 56 }, (_, familyIndex) => `family_${familyIndex}`).flatMap((familyId, familyIndex) =>
        PROGRESS_CERTIFIED_V5_CONFIRMATORY_COUNTRIES.flatMap((country) =>
...
describe("progress-certified V5 confirmatory unblinding", () => {
    it("requires both paired improvement and absolute above-even evidence", () => {
        ...
        expect(analysis.status).toBe("PASSED_PROGRESS_CERTIFIED_V5_CONFIRMATORY_SUCCESS_GATE");
        ...
        expect(analysis.transitions.pairedCellCount).toBe(1_008);
    ...
    it("does not claim success when V5 merely ties both controls", () => {
        ...
        expect(analysis.status).toBe("FAILED_PROGRESS_CERTIFIED_V5_CONFIRMATORY_SUCCESS_GATE");
