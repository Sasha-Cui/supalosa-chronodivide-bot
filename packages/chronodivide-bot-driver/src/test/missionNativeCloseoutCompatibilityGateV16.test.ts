...
import {
    summarizeMissionNativeCloseoutV16,
    validateMissionNativeCloseoutV16Exposure,
} from "../training/missionNativeCloseoutCompatibilityGateV16.js";
...
describe("mission-native closeout compatibility v16", () => {
    it("validates blocker persistence followed by a physical building phase", () => {
        const telemetry = validTelemetry();
        expect(() => validateMissionNativeCloseoutV16Exposure(
            telemetry,
            Countries.USA,
            0,
        )).not.toThrow();
        expect(summarizeMissionNativeCloseoutV16(telemetry)).toMatchObject({
            buildingDamage: 10,
