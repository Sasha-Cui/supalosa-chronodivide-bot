...
import {
    summarizeMissionNativeCloseoutV13,
    validateMissionNativeCloseoutV13Exposure,
} from "../training/missionNativeCloseoutCompatibilityGateV13.js";
...
const validTelemetry = (): BuildingEliminationTelemetryEvent[] => [
    ...
    },
    {
        ...
        tick: 2_700,
        reason: "no_viable_staged_route_clearance",
        ownCombatants: 2,
        enemyCombatants: 8,
...
describe("mission-native closeout compatibility v13", () => {
    it("validates blocker persistence followed by a physical building phase", () => {
        const telemetry = validTelemetry();
        expect(() => validateMissionNativeCloseoutV13Exposure(
            telemetry,
            Countries.USA,
            0,
        )).not.toThrow();
        expect(summarizeMissionNativeCloseoutV13(telemetry)).toMatchObject({
            buildingDamage: 10,
    ...
    it("fails closed when aggregate route clearance exceeds force survival", () => {
        const telemetry = validTelemetry();
        ...
        expect(() => validateMissionNativeCloseoutV13Exposure(
            ...
            Countries.USA,
            0,
        )).toThrow(/route-readiness certificate/);
    });
    ...
    it("fails closed when first-heartbeat ownership is smaller than the staged certificate", () => {
        const telemetry = validTelemetry();
        const passing = telemetry.find((event) =>
            event.event === "activation_evaluation" && event.phase === "blocker_ready",
        );
        if (!passing || passing.event !== "activation_evaluation") {
            throw new Error("Missing passing activation fixture");
        }
        passing.compatibleAttackerCount = 3;
        passing.totalCompatibleAttackerCount = 3;
        passing.transferCertifiedAttackerCount = 3;
        passing.stagedCompatibleAttackerCount = 3;
        expect(() => validateMissionNativeCloseoutV13Exposure(
            telemetry,
            Countries.USA,
            0,
        )).toThrow(/launch ownership/);
    });
});
