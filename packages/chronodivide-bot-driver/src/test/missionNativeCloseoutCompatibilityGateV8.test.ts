...
import {
    summarizeMissionNativeCloseoutV8,
    validateMissionNativeCloseoutV8Exposure,
} from "../training/missionNativeCloseoutCompatibilityGateV8.js";
...
const validTelemetry = (): BuildingEliminationTelemetryEvent[] => [
    {
        ...
        tick: 2_700,
        reason: "no_viable_building_race",
        ownCombatants: 2,
        enemyCombatants: 8,
...
describe("mission-native closeout compatibility v8", () => {
    it("validates blocker persistence followed by a physical building phase", () => {
        const telemetry = validTelemetry();
        expect(() => validateMissionNativeCloseoutV8Exposure(
            telemetry,
            Countries.USA,
            0,
        )).not.toThrow();
        expect(summarizeMissionNativeCloseoutV8(telemetry)).toMatchObject({
            buildingDamage: 10,
