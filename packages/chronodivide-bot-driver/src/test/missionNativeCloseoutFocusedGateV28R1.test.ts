...
import {
    MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R1_COUNTRIES,
    MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R1_ENGINE_SEED_BASE,
    validateMissionNativeCloseoutFocusedGateV28R1Telemetry,
} from "../training/missionNativeCloseoutFocusedGateV28R1.js";
...
describe("mission-native closeout focused gate v28-r1", () => {
    it("keeps every focused seed inside the engine uint32 domain", () => {
        for (const index of MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V28_R1_COUNTRIES.keys()) {
    ...
    it("accepts an objective-feasible direct launch despite zero readiness composition", () => {
        expect(() => validateMissionNativeCloseoutFocusedGateV28R1Telemetry(
    ...
    it("fails closed when screen production exceeds its bounded allowance", () => {
        const telemetry = blockedPreterminal().map((event) =>
