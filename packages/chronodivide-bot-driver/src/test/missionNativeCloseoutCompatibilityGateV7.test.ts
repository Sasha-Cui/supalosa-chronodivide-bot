...
import {
    summarizeMissionNativeCloseoutV7,
    validateMissionNativeCloseoutV7Exposure,
} from "../training/missionNativeCloseoutCompatibilityGateV7.js";
...
describe("mission-native closeout compatibility v7", () => {
    it("validates blocker persistence followed by a physical building phase", () => {
        const telemetry = validTelemetry();
        expect(() => validateMissionNativeCloseoutV7Exposure(
            telemetry,
            Countries.USA,
            0,
        )).not.toThrow();
        expect(summarizeMissionNativeCloseoutV7(telemetry)).toMatchObject({
            buildingDamage: 10,
