...
import {
    MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_COUNTRIES,
    MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_ENGINE_SEED_BASE,
    validateMissionNativeCloseoutFocusedGateV26Telemetry,
} from
    "../training/missionNativeCloseoutFocusedGateV26.js";
...
describe("mission-native closeout focused gate v26", () => {
    it("keeps every focused seed inside the engine uint32 domain", () => {
        for (const index of MISSION_NATIVE_CLOSEOUT_FOCUSED_GATE_V26_COUNTRIES.keys()) {
    ...
    it("requires factory-triggered readiness ownership, persistent tanks, and building damage", () => {
        expect(() => validateMissionNativeCloseoutFocusedGateV26Telemetry(valid(), Countries.USA)).not.toThrow();
    });
    ...
    it("fails closed if production telemetry ends on tank acquisition", () => {
        ...
        expect(() => validateMissionNativeCloseoutFocusedGateV26Telemetry(
