...
describe("persistent objective-completion strategy", () => {
    ...
    it("discounts a distant threat until its earliest interception time", () => {
        const tick = { value: 0 };
        const routeBlocker = combatant(200, 19, 0, "enemy");
        const target = { ...building(100, 20, "enemy"), hitPoints: 100 };
        const units = [
            combatant(1, 0),
            building(10, 0, "candidate"),
            target,
