...
import {
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V31_COUNTRIES,
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V31_ENGINE_SEED_BASE,
    MissionNativeCloseoutAllCountryV31CoverageRow,
    validateMissionNativeCloseoutAllCountryV31Coverage,
} from "../training/missionNativeCloseoutAllCountryGateV31.js";

const validRows = (): MissionNativeCloseoutAllCountryV31CoverageRow[] =>
    MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V31_COUNTRIES.flatMap((country) =>
...
describe("mission-native closeout all-country gate v31", () => {
    it("freezes all nine countries and both reciprocal slots", () => {
        expect(MISSION_NATIVE_CLOSEOUT_ALL_COUNTRY_GATE_V31_COUNTRIES).toEqual([
            Countries.USA,
            Countries.KOREA,
            Countries.FRANCE,
            Countries.GERMANY,
        ...
        expect(validateMissionNativeCloseoutAllCountryV31Coverage(validRows())).toEqual([]);
    });
    ...
    it("fails closed on a missing reciprocal cell", () => {
        ...
        expect(validateMissionNativeCloseoutAllCountryV31Coverage(rows)).toEqual(expect.arrayContaining([
    ...
    it("fails closed on destructive reservation or missing ready screen infrastructure", () => {
        const reservation = validRows().map((row, index) => index === 0
            ...
            : row);
        expect(validateMissionNativeCloseoutAllCountryV31Coverage(reservation)).toContain(
            "V31 emitted destructive production-reservation telemetry",
        );
        ...
        expect(validateMissionNativeCloseoutAllCountryV31Coverage(noSovietInfrastructure)).toContain(
    ...
    it("does not require construction when side-correct screen infrastructure already exists", () => {
        ...
        expect(validateMissionNativeCloseoutAllCountryV31Coverage(rows)).toEqual(
            [],
        );
    });
});
