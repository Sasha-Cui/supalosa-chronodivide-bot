...
import {
    summarizeMissionNativeCloseoutV9,
    validateMissionNativeCloseoutV9Exposure,
} from "../training/missionNativeCloseoutCompatibilityGateV9.js";
...
describe("mission-native closeout compatibility v9", () => {
    it("validates blocker persistence followed by a physical building phase", () => {
        const telemetry = validTelemetry();
        expect(() => validateMissionNativeCloseoutV9Exposure(
            telemetry,
            Countries.USA,
            0,
        )).not.toThrow();
        expect(summarizeMissionNativeCloseoutV9(telemetry)).toMatchObject({
            buildingDamage: 10,
